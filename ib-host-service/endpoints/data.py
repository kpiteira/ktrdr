    ...
    try:
        ...
        if instrument_type is None:
            ...
            if validation_result.contract_info:
                ...
                if (
                    hasattr(validation_result.contract_info, "head_timestamp")
                    and validation_result.contract_info.head_timestamp
                ):
                    from datetime import datetime

                    head_timestamp = datetime.fromisoformat(
                        validation_result.contract_info.head_timestamp.replace(
                            "Z", "+00:00"
                        )
                    )
                ...
                logger.warning(
                    f"No contract info returned for {request.symbol}, defaulting to STK"
                )
                instrument_type = "STK"
        ...
        return HistoricalDataResponse(success=True, data=data_json, rows=len(data))

    except Exception as e:
        logger.error(f"Error fetching historical data: {str(e)}")
        return HistoricalDataResponse(success=False, error=str(e))
...
async def validate_symbol(request: ValidationRequest):
    ...
    try:
        ...
        validation_result = await validator.validate_symbol_with_metadata(
            symbol=request.symbol, timeframes=request.timeframes or []
        )
        ...
        return ValidationResponse(
            ...
            contract_info=(
                validation_result.contract_info.__dict__
                if validation_result.contract_info
                else None
            ),
...
async def get_head_timestamp(
    symbol: str, timeframe: str, instrument_type: Optional[str] = None
):
    """
    ...
    try:
        ...
        return HeadTimestampResponse(
            success=True, timestamp=head_timestamp if head_timestamp else None
        )

    except Exception as e:
        logger.error(f"Error getting head timestamp: {str(e)}")
        return HeadTimestampResponse(success=False, error=str(e))
