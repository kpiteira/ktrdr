...
class IbPaceManager:
    ...
    async def _enforce_historical_limits(
        self, now: float, contract_key: Optional[str], is_bid_ask: bool
    ):
    ...
    async def _enforce_ten_minute_limit(self, now: float, is_bid_ask: bool):
        ...
        while (
            ...
            and self.historical_requests[0].timestamp < cutoff_time
        ):
    ...
    async def _enforce_identical_request_limit(self, now: float, contract_key: str):
        ...
        if contract_key in self.identical_requests:
            ...
            if elapsed < 15.0:
                ...
                logger.debug(
                    f"Identical request limit ({contract_key}): waiting {wait_time:.2f}s"
                )
                await asyncio.sleep(wait_time)
        ...
        with self.lock:
            ...
            if is_historical:
                ...
                if contract_key:
                    ...
                    contract_requests = self.contract_requests[contract_key]
                    recent_requests = [t for t in contract_requests if t >= now - 2.0]
