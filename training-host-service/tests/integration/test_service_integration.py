...
class TestServiceIntegration:
    ...
    def test_training_session_lifecycle(self, integration_config, sample_training_config):
        ...
        try:
            ...
            while time.time() - start_time < training_timeout:
                ...
                if current_epoch > last_epoch:
                    print(f"Training progress: Epoch {current_epoch}/{status_data['progress']['total_epochs']}")
            ...
            # Cleanup session
            try:
                requests.delete(f"{service_url}/training/sessions/{session_id}", timeout=30)
            except:
    ...
    def test_concurrent_session_limit(self, integration_config, sample_training_config):
        ...
        try:
            ...
            if response2.status_code == 500:
                ...
                requests.delete(f"{service_url}/training/sessions/{session_id2}", timeout=30)
            ...
            try:
                ...
                time.sleep(2)
                requests.delete(f"{service_url}/training/sessions/{session_id1}", timeout=30)
            except:
                pass
    ...
    def test_model_evaluation(self, integration_config):
        ...
        response = requests.post(
            f"{service_url}/training/evaluate",
    ...
    def test_service_resource_usage(self, integration_config):
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                # Stop training
                stop_data = {"session_id": session_id, "save_checkpoint": False}
                requests.post(f"{service_url}/training/stop", json=stop_data, timeout=30)
                time.sleep(2)
...
class TestServicePerformance:
    ...
    def test_response_times(self, integration_config, performance_config):
        ...
        assert response.status_code == 200
        assert response_time < max_response_time, f"Health check took {response_time:.3f}s, expected < {max_response_time:.3f}s"
        ...
        assert response.status_code == 200
        assert response_time < max_response_time * 2, f"Detailed health check took {response_time:.3f}s"
    ...
    def test_concurrent_requests(self, integration_config, performance_config):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_concurrent) as executor:
            futures = [executor.submit(make_health_request) for _ in range(num_concurrent)]
            results = [future.result() for future in concurrent.futures.as_completed(futures)]
        
        success_rate = sum(results) / len(results)
        acceptable_success_rate = 1.0 - performance_config["acceptable_error_rate"]
        
        assert success_rate >= acceptable_success_rate, f"Success rate {success_rate:.2%} below acceptable {acceptable_success_rate:.2%}"