...
class TestTrainingService:
    ...
    @pytest.mark.asyncio
    async def test_create_session_success(self, mock_gpu_available, sample_training_config):
        """Test successful session creation."""
        from services.training_service import TrainingService
        
        with patch.object(TrainingService, '_initialize_global_resources'):
            with patch.object(TrainingService, '_initialize_session_resources', new_callable=AsyncMock):
                with patch.object(TrainingService, '_run_training_session', new_callable=AsyncMock):
    ...
    @pytest.mark.asyncio
    async def test_create_session_with_custom_id(self, mock_gpu_available, sample_training_config):
        ...
        with patch.object(TrainingService, '_initialize_global_resources'):
            with patch.object(TrainingService, '_initialize_session_resources', new_callable=AsyncMock):
                with patch.object(TrainingService, '_run_training_session', new_callable=AsyncMock):
                    ...
                    session_id = await service.create_session(sample_training_config, custom_id)
    ...
    @pytest.mark.asyncio
    async def test_create_session_exceeds_limit(self, mock_gpu_available, sample_training_config):
    ...
    @pytest.mark.asyncio
    async def test_create_duplicate_session_id(self, mock_gpu_available, sample_training_config):
        ...
        with patch.object(TrainingService, '_initialize_global_resources'):
            with patch.object(TrainingService, '_initialize_session_resources', new_callable=AsyncMock):
                with patch.object(TrainingService, '_run_training_session', new_callable=AsyncMock):
                    service = TrainingService(max_concurrent_sessions=2)  # Allow multiple sessions
    ...
    @pytest.mark.asyncio
    async def test_stop_session_success(self, mock_gpu_available, sample_training_config):
...
class TestTrainingSession:
    ...
    def test_training_session_initialization(self, sample_training_config):
        ...
        assert session.total_epochs == sample_training_config["training_config"]["epochs"]
    ...
    def test_update_progress(self, sample_training_config):
        ...
        assert session.best_metrics["train_accuracy"] == 0.8  # Accuracy - higher is better
