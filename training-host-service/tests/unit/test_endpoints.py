...
class TestTrainingEndpoints:
    ...
    def test_start_training_success(self, client, mock_training_service):
        ...
        with patch('services.training_service.get_training_service', return_value=mock_training_service):
    ...
    def test_stop_training_service_error(self, client):
        ...
        with patch('services.training_service.get_training_service') as mock_get_service:
            ...
            request_data = {"session_id": "nonexistent-session", "save_checkpoint": False}
            response = client.post("/training/stop", json=request_data)
    ...
    def test_cleanup_session_service_error(self, client):
        ...
        with patch('services.training_service.get_training_service') as mock_get_service:
            mock_service = Mock()
            mock_service.cleanup_session.side_effect = Exception("Cannot cleanup running session")
            mock_get_service.return_value = mock_service
