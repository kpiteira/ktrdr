...
class TrainingStartRequest(BaseModel):
    ...
    gpu_configuration: Optional[Dict[str, Any]] = Field(default=None, description="GPU-specific configuration")

class TrainingStopRequest(BaseModel):
    """Request to stop a training session."""
    session_id: str = Field(description="Session ID to stop")
    save_checkpoint: bool = Field(default=True, description="Whether to save final checkpoint")
...
class EvaluationRequest(BaseModel):
    ...
    metrics: List[str] = Field(default=["accuracy", "loss"], description="Metrics to compute")
