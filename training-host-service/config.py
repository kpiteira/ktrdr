...
class HostServiceConfig(BaseModel):
    ...
    max_concurrent_sessions: int = Field(
        default=3, description="Maximum concurrent training sessions"
    )
    session_timeout_minutes: int = Field(
        default=60, description="Training session timeout in minutes"
    )
...
def get_host_service_config() -> TrainingHostServiceConfig:
    ...
    if _service_config is None:
        try:
            ...
            config_path = (
                Path(__file__).parent.parent / "config" / "training_host_service.yaml"
            )
            if config_path.exists():
                _service_config = _config_loader.load(
                    config_path, TrainingHostServiceConfig
                )
            else:
