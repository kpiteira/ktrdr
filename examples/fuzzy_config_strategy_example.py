...
def main():
    ...
    logger.info(f"Loaded default fuzzy configuration with indicators: {list(default_config.root.keys())}")
    ...
    for strategy in strategies:
        try:
            ...
            logger.info(f"Loaded '{strategy}' configuration with indicators: {list(strategy_config.root.keys())}")
            
            # Count the total number of fuzzy sets across all indicators
            total_sets = sum(len(ind_config.root) for ind_config in strategy_config.root.values())
            logger.info(f"'{strategy}' has {len(strategy_config.root)} indicators with {total_sets} total fuzzy sets")
    ...
    for strategy, config in strategy_configs.items():
        plt2 = plot_membership_functions(config.root, f"{strategy.replace('_', ' ').title()}", "rsi")
    ...
    for strategy, config in strategy_configs.items():
        ...
        if unique_indicators:
            ...
            if unique_indicators:
                ...
                plt3 = plot_membership_functions(config.root, f"{strategy.replace('_', ' ').title()}", unique_ind)
                plt3.savefig(PROJECT_ROOT / "output" / f"{strategy}_{unique_ind}_fuzzy_sets.png")
    
    logger.info("Fuzzy configuration demonstration completed. Check the 'output' directory for visualization results.")
