...
def plot_data_comparison(original, repaired, title="Data Comparison"):
    ...
    # Plot volumes
    axes[1].bar(original.index, original['volume'], width=0.8, alpha=0.4, color='blue', label='Original Volume')
    axes[1].bar(repaired.index, repaired['volume'], width=0.4, alpha=0.6, color='red', label='Repaired Volume')
...
def demonstrate_data_manager():
    ...
    print(f"  - Resampled from daily ({len(standard_repaired)} rows) to weekly ({len(weekly_data)} rows)")
