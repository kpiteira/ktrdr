...
async def get_market_data(symbol: str, timeframe: str = "1h", 
                         ...
                         limit_bars: int = 50) -> Dict[str, Any]:
    ...
    try:
        async with get_api_client() as client:
            ...
            data = await client.get_cached_data(
                symbol, timeframe, start_date, end_date, trading_hours_only, 
                limit=effective_limit
            )
            logger.info("Cached market data retrieved", 
                       symbol=symbol, timeframe=timeframe, limit=effective_limit)
            return data
    except Exception as e:
...
@mcp.tool()
async def load_data_from_source(symbol: str, timeframe: str = "1h",
                               mode: str = "tail",
                               start_date: Optional[str] = None, 
    ...
    try:
        async with get_api_client() as client:
            result = await client.load_data_operation(symbol, timeframe, mode, start_date, end_date)
...
@mcp.tool()
async def save_strategy(name: str, config: Dict[str, Any], 
...
async def add_knowledge(topic: str, content: str, 
    ...
    try:
        storage = await get_storage()
        knowledge_id = await storage.add_knowledge(topic, content, source_type, tags=tags)
...
@mcp.tool()
async def search_knowledge(topic: Optional[str] = None, 
...
@mcp.tool()
async def start_model_training(experiment_id: str, 
                              symbol: str,
                              timeframe: str = "1h",
...
async def get_training_status(task_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info("Training status retrieved", task_id=task_id, status=result["status"])
        return result
...
@mcp.tool()
async def list_training_tasks(experiment_id: Optional[str] = None, 
    ...
    try:
        ...
        logger.info("Training tasks listed", count=len(tasks), experiment_id=experiment_id)
        return tasks
...
@mcp.tool()
async def save_trained_model(task_id: str, model_name: str, 
                            description: str = "") -> Dict[str, Any]:
    ...
    try:
        ...
        async with get_api_client() as client:
            save_result = await client.save_trained_model(task_id, model_name, description)
...
@mcp.tool()
async def test_model_prediction(model_name: str, symbol: str, 
                               timeframe: str = "1h",
                               test_date: Optional[str] = None) -> Dict[str, Any]:
...
async def run_strategy_backtest(experiment_id: str,
                               ...
                               backtest_name: Optional[str] = None) -> Dict[str, Any]:
...
async def get_backtest_results(backtest_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error("Failed to get backtest results", backtest_id=backtest_id, error=str(e))
        raise
    ...
    try:
        ...
        for metric in metrics:
            ...
            if values:
                ...
                reverse_sort = metric not in ['volatility', 'max_drawdown']  # Lower is better for these
...
@mcp.tool()
async def run_walk_forward_analysis(experiment_id: str,
    ...
    try:
        ...
        logger.info("Walk-forward analysis planned", symbol=symbol, periods=len(periods))
        return {
            "experiment_id": experiment_id,
            "symbol": symbol,
...
@mcp.tool()
async def get_backtest_performance_summary(symbol: Optional[str] = None,
