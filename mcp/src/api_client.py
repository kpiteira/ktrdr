...
class KTRDRAPIClient:
    ...
    async def _request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        if not self.client:
            raise KTRDRAPIError("API client not initialized. Use async context manager.")
        ...
        try:
            ...
            data = response.json()
            logger.debug("API response", status=response.status_code, data_keys=list(data.keys()) if isinstance(data, dict) else type(data).__name__)
            return data
    ...
    async def get_cached_data(self, symbol: str, timeframe: str = "1h", 
        ...
        response = await self._request("GET", f"/data/{symbol}/{timeframe}", params=params)
        ...
        if limit and 'data' in response and 'dates' in response['data']:
            ...
            if len(data['dates']) > limit:
                logger.info(f"Limiting response from {len(data['dates'])} to {limit} data points")
    ...
    async def load_data_operation(self, symbol: str, timeframe: str = "1h",
                                 mode: str = "local",
                                 start_date: Optional[str] = None, 
    ...
    async def compute_indicator(self, symbol: str, indicator_type: str, 
    ...
    async def compute_fuzzy_signals(self, symbol: str, 
    ...
    async def save_strategy(self, strategy_name: str, 
