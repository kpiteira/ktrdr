...
class ExperimentStorage:
    ...
    def __init__(self, db_path: Optional[str] = None):
        self.db_path = db_path or get_config("EXPERIMENT_DB_PATH", "/app/experiments/experiments.db")
    ...
    async def update_experiment(self, experiment_id: int, status: Optional[str] = None,
    ...
    async def list_experiments(self, status: Optional[str] = None, 
    ...
    # Strategy Management
    async def save_strategy(self, name: str, config: Dict, 
                           description: str = "") -> int:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if cursor.rowcount == 0:
                ...
                cursor = await db.execute("SELECT id FROM strategies WHERE name = ?", (name,))
                row = await cursor.fetchone()
                strategy_id = row[0]
    ...
    # Backtest Management
    async def save_backtest(self, strategy_id: int, symbol: str, 
                           start_date: str, end_date: str,
                           results: Dict, metrics: Dict) -> int:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute(
                ...
                (strategy_id, symbol, start_date, end_date, 
                 json.dumps(results), json.dumps(metrics))
            )
            await db.commit()
            backtest_id = cursor.lastrowid
            logger.info("Backtest saved", id=backtest_id, strategy_id=strategy_id, symbol=symbol)
            return backtest_id
    
    async def get_backtest_history(self, strategy_id: Optional[int] = None,
    ...
    async def add_knowledge(self, topic: str, content: str, 
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute(
                ...
                (topic, content, source_type, source_id, json.dumps(tags) if tags else None)
            )
            await db.commit()
    ...
    async def search_knowledge(self, topic: Optional[str] = None,
    ...
    async def create_training_task(self, experiment_id: int, symbol: str, 
                                  timeframe: str, config: Dict,
        ...
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                """INSERT INTO training_tasks 
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (task_id, experiment_id, symbol, timeframe, json.dumps(config), start_date, end_date)
            )
            await db.commit()
            logger.info("Training task created", task_id=task_id, experiment_id=experiment_id)
            return task_id
    
    async def update_training_task(self, task_id: str, status: Optional[str] = None,
        ...
        if status:
            updates.append("status = ?")
            values.append(status)
            if status == "training" and not updates.__contains__("started_at = CURRENT_TIMESTAMP"):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                f"UPDATE training_tasks SET {', '.join(updates)} WHERE id = ?",
                values
            )
            await db.commit()
    ...
    async def list_training_tasks(self, experiment_id: Optional[int] = None,
    ...
    async def save_model_record(self, name: str, task_id: str, description: str,
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute(
                """INSERT INTO model_records 
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (name, task_id, description, json.dumps(config), symbol, timeframe,
