...
def test_service_manager_functionality():
    ...
    success, output, error = run_command(
        [
            ...
            "run",
            "python",
            "services/management/service_manager.py",
            "status",
            "--service",
            "training-host",
        ]
...
def test_health_monitoring():
    ...
    success, output, error = run_command(
        [
            ...
            "run",
            "python",
            "services/management/health_monitor.py",
            "status",
            "--service",
            "training-host",
        ]
...
def test_cross_platform_configurations():
    ...
    install_script = (
        PROJECT_ROOT / "services" / "management" / "install_macos_service.sh"
    )
    ...
    install_script = (
        PROJECT_ROOT / "services" / "management" / "install_linux_service.sh"
    )
    ...
    linux_wrapper = (
        ...
        / "management"
        / "systemd"
        / "systemd_service_wrapper.sh"
    )

    if (
        macos_wrapper.exists()
        and os.access(macos_wrapper, os.X_OK)
        and linux_wrapper.exists()
        and os.access(linux_wrapper, os.X_OK)
    ):
    ...
    macos_uninstall = (
        PROJECT_ROOT / "services" / "management" / "uninstall_macos_service.sh"
    )
    linux_uninstall = (
        PROJECT_ROOT / "services" / "management" / "uninstall_linux_service.sh"
    )

    if (
        macos_uninstall.exists()
        and os.access(macos_uninstall, os.X_OK)
        and linux_uninstall.exists()
        and os.access(linux_uninstall, os.X_OK)
    ):
...
def main():
    ...
    test_results.append(
        ("Service Manager Functionality", test_service_manager_functionality())
    )
    test_results.append(("Health Monitoring", test_health_monitoring()))
    test_results.append(("Unified Scripts", test_unified_scripts()))
    test_results.append(
        ("Cross-Platform Configurations", test_cross_platform_configurations())
    )
