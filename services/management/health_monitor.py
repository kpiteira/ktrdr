...
class HealthMonitor:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return GPUMetrics(
                    ...
                    memory_utilization_percent=data.get("gpu_memory_utilization_percent", 0.0),
                    temperature_celsius=data.get("gpu_temperature_celsius"),
                    power_usage_watts=data.get("gpu_power_usage_watts"),
                    compute_utilization_percent=data.get("gpu_compute_utilization_percent"),
                    manager_status=data.get("gpu_manager_status", {}).get("reason", "unknown"),
    ...
    def get_service_health(self, service_name: str) -> ServiceHealthMetrics:
        ...
        try:
            ...
            for conn in psutil.net_connections():
                if hasattr(conn, 'laddr') and conn.laddr and conn.laddr.port == config["port"]:
                    if conn.pid:
            ...
            active_connections = sum(
                ...
                if hasattr(conn, 'raddr') and conn.raddr and conn.raddr.port == config["port"]
            )
    ...
    def update_baseline(self, service_name: str, metrics_window_hours: int = 24) -> None:
        ...
        try:
            ...
            if len(recent_metrics) < 10:
                logger.warning(f"Insufficient data for baseline calculation: {len(recent_metrics)} samples")
                return
            ...
            total_count = len([
                entry["metrics"][service_name] for entry in history
            ...
            logger.info(f"Updated baseline for {service_name}: "
    ...
    def check_anomalies(self, metrics: Dict[str, ServiceHealthMetrics]) -> List[Dict[str, Any]]:
        ...
        for service_name, metric in metrics.items():
            ...
            if metric.response_time_ms > baseline.avg_response_time_ms * 2:
                anomalies.append({
                    ...
                    "severity": "warning" if metric.response_time_ms < baseline.max_response_time_ms else "critical"
                })
    ...
    def print_health_dashboard(self) -> None:
        ...
        for service_name, metrics in report["services"].items():
            ...
            service_name_display = service_config.get('name', service_name) if isinstance(service_config, dict) else service_name
            ...
            if metrics.get("gpu_metrics") and metrics["gpu_metrics"]["available"]:
                ...
                print(f"      Memory: {gpu['memory_allocated_mb']:.1f}/{gpu['memory_total_mb']:.1f}MB "
        ...
        if report["baselines"]:
            ...
            for service_name, baseline in report["baselines"].items():
                print(f"📊 {self.services.get(service_name, {}).get('name', service_name)}")
    ...
    def continuous_monitoring(self, interval: int = 30, update_baseline_hours: int = 24) -> None:
        ...
        try:
            while True:
                ...
                for service_name in self.services:
                    ...
                    if datetime.now() - last_update > timedelta(hours=update_baseline_hours):
                ...
                if critical_anomalies:
                    for anomaly in critical_anomalies:
                        logger.error(f"CRITICAL ANOMALY: {anomaly['type']} in {anomaly['service']}")
...
def main():
    ...
    parser.add_argument("command", choices=[
        "status", "dashboard", "monitor", "baseline", "report"
    ], help="Command to execute")
    parser.add_argument("--service", help="Specific service to monitor")
    parser.add_argument("--interval", type=int, default=30, help="Monitoring interval in seconds")
    parser.add_argument("--hours", type=int, default=24, help="Hours for baseline calculation")
