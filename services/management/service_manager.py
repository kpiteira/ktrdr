...
class ServiceManager:
    ...
    def __init__(self, project_root: Optional[str] = None):
        ...
        self.status_file = self.project_root / "services" / "management" / "service_status.json"
        self.pid_file = self.project_root / "services" / "management" / "service_manager.pid"
    ...
    def start_service(self, service_name: str, wait: bool = True) -> bool:
        ...
        try:
            ...
            if wait:
                # Wait for startup
                logger.info(f"Waiting {config.startup_delay}s for {config.display_name} to start...")
                ...
                if status == ServiceStatus.RUNNING:
                    ...
                    logger.error(f"❌ {config.display_name} failed to start (status: {status.value})")
                    return False
            else:
        ...
        try:
            ...
            if wait:
                ...
                if status == ServiceStatus.STOPPED:
                    ...
                    logger.warning(f"⚠️  {config.display_name} may not have stopped cleanly (status: {status.value})")
                    return False
            else:
                    ...
                    try:
                        ...
                        if status != ServiceStatus.RUNNING:
                            ...
                            if failure_counts[service_name] <= max_failures:
                                logger.warning(f"Service {service_name} is {status.value}, attempting restart ({failure_counts[service_name]}/{max_failures})")
                                ...
                                logger.error(f"Service {service_name} has failed {max_failures} times, giving up")
                        else:
...
def main():
    ...
    parser.add_argument("command", choices=[
        "start", "stop", "restart", "status", "monitor", "daemon"
    ...
    if args.command == "start":
        ...
        for service_name, info in status_info.items():
            ...
            print(f"   Health: {health_emoji} {'OK' if info['health_ok'] else 'FAILED'}")
