...
class TestResearchDatabaseService:
    ...
    @pytest.mark.asyncio
    async def test_health_check_with_details(
    ...
    async def test_create_session_success(
        ...
        session_id = await clean_database.create_session(
            session_name=session_name, description="Test session for unit testing"
        )
    ...
    @pytest.mark.asyncio
    async def test_create_session_duplicate_name(
        self, clean_database: ResearchDatabaseService
    ):
        ...
        await clean_database.create_session(
            session_name=session_name, description="First session"
        )

        # Attempt to create duplicate should fail
        with pytest.raises(DatabaseError, match="already exists"):
            await clean_database.create_session(
                session_name=session_name, description="Duplicate session"
            )
    ...
    async def test_get_active_agents(self, clean_database: ResearchDatabaseService):
        ...
        agents_data = [
            {
                "agent_id": f"test-agent-active-{i}",
    ...
    async def test_experiment_lifecycle(
        ...
        assert (
            experiment["experiment_name"] == sample_experiment_data["experiment_name"]
        )
        ...
        await clean_database.complete_experiment(
            experiment_id=experiment_id, results=results, status="completed"
        )
    ...
    @pytest.mark.asyncio
    async def test_get_experiments_by_session(
        self, clean_database: ResearchDatabaseService
    ):
    ...
    async def test_knowledge_base_operations(
        ...
        await clean_database.update_knowledge_entry(
            entry_id=entry_id, quality_score=new_quality_score
        )
    ...
    @pytest.mark.asyncio
    async def test_search_knowledge_by_tags(
        self, clean_database: ResearchDatabaseService
    ):
        ...
        combined_results = await clean_database.search_knowledge_by_tags(
            ["machine_learning", "trading_strategy"]
        )
    ...
    @pytest.mark.asyncio
    async def test_get_top_quality_knowledge(
        self, clean_database: ResearchDatabaseService
    ):
    ...
    async def test_database_error_handling(
        ...
        with pytest.raises(DatabaseError):
            await clean_database.update_agent_state(
                agent_id="non-existent-agent", status="active"
            )

    @pytest.mark.asyncio
    async def test_connection_pool_management(
        self, test_database: ResearchDatabaseService
    ):
    ...
    @pytest.mark.asyncio
    async def test_agent_heartbeat_tracking(
        self, clean_database: ResearchDatabaseService
    ):
    ...
    async def test_experiment_statistics(self, clean_database: ResearchDatabaseService):
        ...
        for i, data in enumerate(experiment_data):
            ...
            if data["results"]:
                await clean_database.complete_experiment(
                    experiment_id=exp_id, results=data["results"], status=data["status"]
                )
