...
class TestServiceInitialization:
    ...
    def test_initialization_with_custom_params(self):
        ...
        assert service.api_base_url == "https://custom-api:9000"  # Trailing slash removed
    ...
    async def test_initialize_success(self, integration_service):
        ...
        setup_mock_session_with_response(mock_session, 'get', 200, {"status": "healthy"})
...
class TestHealthCheck:
    ...
    async def test_health_check_success(self, integration_service):
        ...
        setup_mock_session_with_response(mock_session, 'get', 200, {"status": "healthy", "version": "1.0"})
...
class TestTrainingOperations:
    ...
    async def test_submit_training_success(self, integration_service, sample_training_config):
        ...
        mock_session.post = MagicMock(return_value=create_mock_context_manager(mock_response))
    ...
    async def test_submit_training_bad_request(self, integration_service, sample_training_config):
        ...
        mock_response.json = AsyncMock(return_value={"detail": "Missing required field"})
        mock_session.post = MagicMock(return_value=create_mock_context_manager(mock_response))
    ...
    async def test_get_training_status_success(self, integration_service):
        ...
        mock_session.get = MagicMock(return_value=create_mock_context_manager(mock_response))
    ...
    async def test_wait_for_training_completion_timeout(self, integration_service):
        ...
        mock_response.status = 200
        mock_response.json = AsyncMock(return_value={
            "status": "running", "epochs_completed": 50
        })
        mock_session.get = MagicMock(return_value=create_mock_context_manager(mock_response))
        
        # Mock time to simulate timeout
        with patch('research_agents.services.ktrdr_integration.datetime') as mock_datetime:
            # Set up time progression to exceed max_wait
            start_time = datetime.now(timezone.utc)
            times = [
                start_time,  # Initial time
                start_time,  # First check
                start_time.replace(minute=start_time.minute + 2)  # Exceed 1 minute limit
            ]
...
class TestBacktestingOperations:
    ...
    async def test_get_backtest_results_success(self, integration_service):
        ...
        mock_response.json = AsyncMock(return_value={
            ...
            "trade_details": [{"entry": "2024-01-01", "exit": "2024-01-02", "pnl": 0.005}],
            "equity_curve": [{"date": "2024-01-01", "equity": 100000}],
            "drawdown_periods": [{"start": "2024-02-01", "end": "2024-02-05", "drawdown": -0.03}],
...
class TestPerformanceAndConfiguration:
    ...
    def test_endpoint_url_construction(self):
        ...
        test_cases = [
            ...
            ("https://api.example.com/ktrdr", "https://api.example.com/ktrdr/api/training"),
        ]
    ...
    async def test_concurrent_requests(self, integration_service):
        ...
        tasks = [
            integration_service.get_training_status(f"train-{i}")
            for i in range(5)
        ]
