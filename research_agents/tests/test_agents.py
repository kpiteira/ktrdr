...
class TestBaseResearchAgent:
    ...
    def mock_database_service(self):
        ...
        db.get_agent_state = AsyncMock(
            return_value=None
        )  # Default to no existing state
    ...
    async def test_agent_initialize_creates_state(self, mock_agent: MockResearchAgent):
        ...
        mock_agent.db.create_agent_state.assert_called_once_with(
            "test-agent-001", "assistant", "idle", "Agent registered", {}, {}
        )

    @pytest.mark.asyncio
    async def test_agent_initialize_handles_existing_state(
        self, mock_agent: MockResearchAgent
    ):
    ...
    async def test_agent_configuration_management(self, mock_agent: MockResearchAgent):
        ...
        # Set configuration
        config = {"max_cycles": 100, "heartbeat_interval": 30, "memory_limit": 1000}
...
class TestResearcherAgent:
    ...
    async def test_researcher_initialization(
        self, mock_researcher_agent: ResearcherAgent
    ):
    ...
    @pytest.mark.asyncio
    async def test_researcher_hypothesis_generation(
        self, mock_researcher_agent: ResearcherAgent
    ):
    ...
    @pytest.mark.asyncio
    async def test_researcher_knowledge_search(
        self, mock_researcher_agent: ResearcherAgent
    ):
    ...
    @pytest.mark.asyncio
    async def test_researcher_experiment_design(
        self, mock_researcher_agent: ResearcherAgent
    ):
...
class TestAssistantAgent:
    ...
    @pytest.mark.asyncio
    async def test_assistant_experiment_execution(
        self, mock_assistant_agent: AssistantAgent
    ):
    ...
    @pytest.mark.asyncio
    async def test_assistant_training_monitoring(
        self, mock_assistant_agent: AssistantAgent
    ):
    ...
    @pytest.mark.asyncio
    async def test_assistant_result_analysis(
        self, mock_assistant_agent: AssistantAgent
    ):
    ...
    @pytest.mark.asyncio
    async def test_assistant_knowledge_extraction(
        self, mock_assistant_agent: AssistantAgent
    ):
        ...
        experiment_results = {
            "fitness_score": 0.85,
            "profit_factor": 1.25,
            "strategy_parameters": {"ma_period": 20, "rsi_threshold": 30},
            "performance_metrics": {"total_trades": 150, "win_rate": 0.62},
        }
    ...
    async def test_assistant_error_handling(self, mock_assistant_agent: AssistantAgent):
        ...
        mock_assistant_agent.ktrdr_client.start_training.side_effect = Exception(
            "Training failed"
        )
...
class TestAgentIntegration:
    ...
    @pytest.mark.asyncio
    async def test_agent_database_integration(
        self, clean_database: ResearchDatabaseService
    ):
    ...
    async def test_multiple_agents_coordination(
        ...
        researcher_state = await clean_database.get_agent_state(
            "researcher-integration-001"
        )
        assistant_state = await clean_database.get_agent_state(
            "assistant-integration-001"
        )
