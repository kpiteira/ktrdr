...
class TestExperimentLifecycle:
    ...
    @pytest.mark.asyncio
    async def test_create_experiment_success(self, orchestrator, sample_experiment_config, mock_db_service):
        ...
        with patch('research_agents.services.research_orchestrator.uuid4', return_value=expected_experiment_id):
    ...
    @pytest.mark.asyncio
    async def test_create_experiment_not_initialized(self, mock_db_service, sample_experiment_config):
    ...
    @pytest.mark.asyncio
    async def test_create_experiment_database_error(self, orchestrator, sample_experiment_config, mock_db_service):
...
class TestExperimentExecution:
    ...
    async def test_start_experiment_success(self, orchestrator, mock_db_service):
        ...
        with patch.object(orchestrator, '_get_experiment_config', new_callable=AsyncMock) as mock_get_config:
            mock_get_config.return_value = mock_experiment
            
            # Mock experiment execution to prevent actual execution
            with patch.object(orchestrator, '_execute_experiment', new_callable=AsyncMock) as mock_execute:
    ...
    async def test_cancel_experiment_success(self, orchestrator):
        ...
        # Mock status update
        with patch.object(orchestrator, '_update_experiment_status', new_callable=AsyncMock) as mock_update:
            await orchestrator.cancel_experiment(experiment_id)
            
            # Verify cancellation
            assert task.cancelled()
            mock_update.assert_called_once_with(experiment_id, ExperimentStatus.CANCELLED)
...
class TestExperimentListingAndSearch:
    ...
    @pytest.mark.asyncio
    async def test_list_experiments_with_session_filter(self, orchestrator, mock_db_service):
    ...
    @pytest.mark.asyncio
    async def test_list_experiments_with_status_filter(self, orchestrator, mock_db_service):
    ...
    @pytest.mark.asyncio
    async def test_list_experiments_with_multiple_filters(self, orchestrator, mock_db_service):
...
class TestErrorHandlingAndRecovery:
    ...
    async def test_experiment_execution_error_handling(self, orchestrator):
        ...
        # Mock database calls
        with patch.object(orchestrator, '_update_experiment_status', new_callable=AsyncMock) as mock_update:
            with patch.object(orchestrator, '_fail_experiment', new_callable=AsyncMock) as mock_fail:
                # Mock initialization to raise an error
                with patch.object(orchestrator, '_initialize_experiment', side_effect=Exception("Test error")):
                    
                    # Execute experiment (should handle the error)
                    try:
                        await orchestrator._execute_experiment(experiment_id, experiment_config)
                    except Exception:
                        pass  # Expected to raise
                    
                    # Verify error handling calls
                    assert mock_update.call_count >= 1  # Should be called at least once (initializing status)
...
class TestShutdownAndCleanup:
    ...
    @pytest.mark.asyncio
    async def test_graceful_shutdown_no_running_experiments(self, orchestrator, mock_db_service):
    ...
    @pytest.mark.asyncio
    async def test_graceful_shutdown_with_running_experiments(self, orchestrator, mock_db_service):
        ...
        # Mock status update
        with patch.object(orchestrator, '_update_experiment_status', new_callable=AsyncMock):
            await orchestrator.shutdown()
...
class TestExperimentWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_complete_experiment_lifecycle(self, orchestrator, sample_experiment_config, mock_db_service):
        ...
        experiment_id = await orchestrator.create_experiment(session_id, sample_experiment_config)
        assert orchestrator._total_experiments == 1
    ...
    @pytest.mark.asyncio
    async def test_concurrent_experiment_management(self, orchestrator, sample_experiment_config, mock_db_service):
        ...
        exp1_id = await orchestrator.create_experiment(session_id, sample_experiment_config)
        exp2_id = await orchestrator.create_experiment(session_id, sample_experiment_config)
        ...
        exp3_id = await orchestrator.create_experiment(session_id, sample_experiment_config)
...
class TestPerformanceCharacteristics:
    ...
    @pytest.mark.asyncio
    async def test_experiment_creation_performance(self, orchestrator, sample_experiment_config, mock_db_service):
