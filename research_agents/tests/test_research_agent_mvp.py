...
async def mock_db_service():
    ...
    db_service.get_knowledge_base_statistics = AsyncMock(return_value={"total_entries": 10})
    return db_service
...
async def mock_orchestrator():
    ...
    orchestrator.get_experiment_status = AsyncMock(return_value={
        "status": "completed",
        "fitness_score": 1.5,
...
async def mock_researcher():
    ...
    researcher.generate_novel_hypotheses = AsyncMock(return_value=[
        {"hypothesis": "Test hypothesis 1", "confidence": 0.8},
        {"hypothesis": "Test hypothesis 2", "confidence": 0.7}
    ])
    researcher.generate_focused_hypotheses = AsyncMock(return_value=[
        {"hypothesis": "Focused hypothesis", "confidence": 0.9}
    ])
    researcher.design_experiment = AsyncMock(return_value=ExperimentConfig(
...
async def mock_assistant():
    ...
    assistant.analyze_experiment_results = AsyncMock(return_value={
...
class TestResearchAgentMVPInitialization:
    ...
    @pytest.mark.asyncio
    async def test_full_initialization_success(self, research_agent_mvp, mock_db_service, mock_orchestrator, mock_researcher, mock_assistant):
        """Test successful full initialization"""
        # Patch the base class database creation and MVP-specific imports
        with patch('research_agents.agents.base.create_database_service', return_value=mock_db_service):
            with patch('research_agents.services.database.create_database_service', return_value=mock_db_service):
                with patch('research_agents.services.research_orchestrator.create_research_orchestrator', new_callable=AsyncMock, return_value=mock_orchestrator):
                    with patch('research_agents.agents.research_agent_mvp.ResearcherAgent', return_value=mock_researcher):
                        with patch('research_agents.agents.research_agent_mvp.AssistantAgent', return_value=mock_assistant):
...
class TestResearchSessionManagement:
    ...
    @pytest.mark.asyncio
    async def test_start_research_session_success(self, research_agent_mvp, mock_db_service):
    ...
    @pytest.mark.asyncio
    async def test_start_research_session_database_error(self, research_agent_mvp, mock_db_service):
...
class TestResearchCycleExecution:
    ...
    @pytest.mark.asyncio
    async def test_execute_research_cycle_complete_workflow(self, research_agent_mvp, mock_db_service, mock_orchestrator, mock_researcher, mock_assistant):
        ...
        research_agent_mvp.research_progress = ResearchProgress(
            total_cycles=0, completed_cycles=0, active_experiments=0,
            best_fitness_score=0.0, avg_fitness_score=0.0, successful_strategies=[],
            failed_experiments=0, knowledge_base_size=10, research_velocity=0.0
        )
        
        # Mock knowledge base methods
        with patch.object(research_agent_mvp, '_get_recent_insights', return_value=[]):
            with patch.object(research_agent_mvp, '_get_knowledge_base_size', return_value=10):
    ...
    @pytest.mark.asyncio
    async def test_execute_research_cycle_auto_session_creation(self, research_agent_mvp):
        """Test research cycle with automatic session creation"""
        with patch.object(research_agent_mvp, 'start_research_session') as mock_start_session:
            with patch.object(research_agent_mvp, '_phase_hypothesis_generation'):
                with patch.object(research_agent_mvp, '_phase_experiment_design'):
                    with patch.object(research_agent_mvp, '_phase_experiment_execution'):
                        with patch.object(research_agent_mvp, '_phase_results_analysis'):
                            with patch.object(research_agent_mvp, '_phase_knowledge_integration'):
                                with patch.object(research_agent_mvp, '_phase_strategy_optimization'):
                                    with patch.object(research_agent_mvp, '_complete_research_cycle'):
                                        ...
                                        assert "Auto-Session" in call_args["session_name"]
                                        assert call_args["strategy"] == ResearchStrategy.EXPLORATORY
    ...
    async def test_execute_research_cycle_error_handling(self, research_agent_mvp):
        ...
        with patch.object(research_agent_mvp, '_phase_hypothesis_generation', side_effect=Exception("Hypothesis error")):
            with patch.object(research_agent_mvp, '_record_cycle_failure') as mock_record_failure:
...
class TestResearchPhases:
    ...
    @pytest.mark.asyncio
    async def test_phase_hypothesis_generation_exploratory(self, research_agent_mvp, mock_researcher):
        """Test exploratory hypothesis generation phase"""
        research_agent_mvp.researcher = mock_researcher
        research_agent_mvp.current_session_id = uuid4()
        research_agent_mvp.current_cycle = ResearchCycle(
            cycle_id=uuid4(), session_id=uuid4(), strategy=ResearchStrategy.EXPLORATORY,
            phase=ResearchPhase.IDLE, hypotheses=[], experiments=[], insights=[],
            fitness_scores=[], started_at=datetime.now(timezone.utc)
        )
        ...
        with patch.object(research_agent_mvp, '_get_recent_insights', return_value=[]):
            with patch.object(research_agent_mvp, '_should_explore', return_value=True):
                ...
                assert research_agent_mvp.current_cycle.phase == ResearchPhase.HYPOTHESIS_GENERATION
                assert len(research_agent_mvp.current_cycle.hypotheses) > 0
    
    @pytest.mark.asyncio
    async def test_phase_hypothesis_generation_focused(self, research_agent_mvp, mock_researcher):
        """Test focused hypothesis generation phase"""
        research_agent_mvp.researcher = mock_researcher
        research_agent_mvp.current_session_id = uuid4()
        research_agent_mvp.current_cycle = ResearchCycle(
            cycle_id=uuid4(), session_id=uuid4(), strategy=ResearchStrategy.FOCUSED,
            phase=ResearchPhase.IDLE, hypotheses=[], experiments=[], insights=[],
            fitness_scores=[], started_at=datetime.now(timezone.utc)
        )
        
        with patch.object(research_agent_mvp, '_get_recent_insights', return_value=[]):
            with patch.object(research_agent_mvp, '_should_explore', return_value=False):
                with patch.object(research_agent_mvp, '_get_successful_patterns', return_value=[]):
    ...
    @pytest.mark.asyncio
    async def test_phase_experiment_design(self, research_agent_mvp, mock_researcher, mock_orchestrator):
        ...
        research_agent_mvp.current_cycle = ResearchCycle(
            ...
            hypotheses=[{"hypothesis": "Test hypothesis 1"}, {"hypothesis": "Test hypothesis 2"}],
            experiments=[], insights=[], fitness_scores=[], started_at=datetime.now(timezone.utc)
        )
    ...
    @pytest.mark.asyncio
    async def test_phase_experiment_execution(self, research_agent_mvp, mock_orchestrator, mock_assistant):
        ...
        research_agent_mvp.current_cycle = ResearchCycle(
            ...
            hypotheses=[], experiments=[uuid4(), uuid4()], insights=[], fitness_scores=[],
        ...
        assert research_agent_mvp.current_cycle.phase == ResearchPhase.EXPERIMENT_EXECUTION
    
    @pytest.mark.asyncio
    async def test_phase_results_analysis(self, research_agent_mvp, mock_orchestrator, mock_assistant):
        ...
        research_agent_mvp.current_cycle = ResearchCycle(
            ...
            hypotheses=[], experiments=[uuid4()], insights=[], fitness_scores=[],
            started_at=datetime.now(timezone.utc)
        )
    ...
    @pytest.mark.asyncio
    async def test_phase_knowledge_integration(self, research_agent_mvp, mock_db_service):
        """Test knowledge integration phase"""
        research_agent_mvp.db_service = mock_db_service
        research_agent_mvp.current_cycle = ResearchCycle(
            cycle_id=uuid4(), session_id=uuid4(), strategy=ResearchStrategy.EXPLORATORY,
            phase=ResearchPhase.RESULTS_ANALYSIS,
            hypotheses=[], experiments=[uuid4()], 
            insights=["Insight 1", "Insight 2"], fitness_scores=[1.5, 1.8],
            started_at=datetime.now(timezone.utc)
        )
        ...
        with patch.object(research_agent_mvp, '_refresh_knowledge_cache'):
            ...
            assert research_agent_mvp.current_cycle.phase == ResearchPhase.KNOWLEDGE_INTEGRATION
    ...
    async def test_phase_strategy_optimization(self, research_agent_mvp):
        ...
        research_agent_mvp.current_cycle = ResearchCycle(
            ...
            hypotheses=[], experiments=[], insights=[], fitness_scores=[1.5, 1.8],
            started_at=datetime.now(timezone.utc)
        )
        ...
        with patch.object(research_agent_mvp, '_adapt_research_strategy'):
            ...
            assert ResearchStrategy.EXPLORATORY in research_agent_mvp.strategy_performance
            assert len(research_agent_mvp.strategy_performance[ResearchStrategy.EXPLORATORY]) == 1
            assert research_agent_mvp.current_cycle.phase == ResearchPhase.STRATEGY_OPTIMIZATION
...
class TestResearchProgress:
    ...
    async def test_complete_research_cycle_success(self, research_agent_mvp):
        ...
        research_agent_mvp.current_cycle = ResearchCycle(
            ...
            hypotheses=[], experiments=[uuid4(), uuid4()], 
            insights=["Insight 1"], fitness_scores=[1.2, 1.8],
            started_at=datetime.now(timezone.utc)
        )
    ...
    async def test_initialize_research_progress(self, research_agent_mvp):
        ...
        with patch.object(research_agent_mvp, '_get_knowledge_base_size', return_value=25):
...
class TestHelperMethods:
    ...
    async def test_get_knowledge_base_size(self, research_agent_mvp, mock_db_service):
        ...
        research_agent_mvp.db_service = mock_db_service
        mock_db_service.get_knowledge_base_statistics.return_value = {"total_entries": 42}
        
        size = await research_agent_mvp._get_knowledge_base_size()
        assert size == 42
    
    @pytest.mark.asyncio
    async def test_get_knowledge_base_size_error(self, research_agent_mvp, mock_db_service):
        """Test getting knowledge base size with error"""
        research_agent_mvp.db_service = mock_db_service
        mock_db_service.get_knowledge_base_statistics.side_effect = Exception("DB error")
    ...
    async def test_refresh_knowledge_cache(self, research_agent_mvp):
        ...
        with patch.object(research_agent_mvp, '_get_recent_insights', return_value=[{"insight": "test"}]):
...
class TestStatusAndMonitoring:
    ...
    async def test_get_research_status_complete(self, research_agent_mvp):
        ...
        research_agent_mvp.research_progress = ResearchProgress(
            total_cycles=5, completed_cycles=4, active_experiments=1,
            best_fitness_score=2.1, avg_fitness_score=1.5, successful_strategies=["focused"],
            failed_experiments=1, knowledge_base_size=50, research_velocity=0.8
        )
        
        research_agent_mvp.current_cycle = ResearchCycle(
            cycle_id=uuid4(), session_id=uuid4(), strategy=ResearchStrategy.FOCUSED,
            phase=ResearchPhase.EXPERIMENT_EXECUTION,
            hypotheses=[{}, {}], experiments=[uuid4()], insights=["insight"],
            fitness_scores=[], started_at=datetime.now(timezone.utc)
        )
        ...
        assert status["current_session_id"] == str(research_agent_mvp.current_session_id)
...
class TestCleanupAndShutdown:
    ...
    @pytest.mark.asyncio
    async def test_cleanup_success(self, research_agent_mvp, mock_researcher, mock_assistant, mock_orchestrator, mock_db_service):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_with_errors(self, research_agent_mvp, mock_researcher, mock_assistant):
...
class TestMainExecutionLoop:
    ...
    async def test_run_method_single_cycle(self, research_agent_mvp):
        ...
        with patch.object(research_agent_mvp, '_execute_research_cycle', side_effect=mock_execute_cycle):
...
class TestFactoryFunction:
    ...
    async def test_create_research_agent_mvp(self, mvp_config):
        ...
        with patch('research_agents.agents.research_agent_mvp.ResearchAgentMVP.initialize') as mock_init:
            from research_agents.agents.research_agent_mvp import create_research_agent_mvp
...
class TestIntegrationScenarios:
    ...
    @pytest.mark.asyncio
    async def test_multiple_research_cycles(self, research_agent_mvp, mock_db_service, mock_orchestrator, mock_researcher, mock_assistant):
        ...
        mock_assistant.analyze_experiment_results.side_effect = [
            {"insights": ["Insight 1"], "fitness_score": score} for score in fitness_scores
        ]
            ...
            with patch.object(research_agent_mvp, '_get_knowledge_base_size', return_value=10):
                with patch.object(research_agent_mvp, '_should_explore', return_value=True):
                    with patch.object(research_agent_mvp, '_refresh_knowledge_cache'):
                        ...
                        assert research_agent_mvp.research_progress.completed_cycles == 3
                        assert research_agent_mvp.research_progress.best_fitness_score == 2.0
                        
                        # Verify strategy performance tracking
                        assert ResearchStrategy.EXPLORATORY in research_agent_mvp.strategy_performance
                        assert len(research_agent_mvp.strategy_performance[ResearchStrategy.EXPLORATORY]) == 3
