...
from research_agents.tests.mocks import MockDatabaseService, MockLLMService, MockKTRDRService
...
@pytest.fixture
def clean_mock_database(mock_database_service):
    """Provide a clean mock database for each test."""
    # Clear any existing test data
    mock_database_service.clear_data()
...
def mock_database_service():
    """Clean database mock for testing."""
    return MockDatabaseService()


@pytest.fixture
def mock_llm_service():
    """Clean LLM service mock for testing."""
    return MockLLMService()


@pytest.fixture
def mock_ktrdr_service():
    """Clean KTRDR service mock for testing."""
    return MockKTRDRService()


@pytest.fixture
def test_app(mock_database_service):
    """Create a test FastAPI application with clean mocks."""
    import os
    os.environ["TESTING"] = "1"  # Signal to skip database initialization
    
    app = create_app()
    
    # Use the clean mock database service
...
@pytest.fixture
def researcher_agent_with_mock_services(mock_llm_service):
    """Create a researcher agent with injected mock services."""
    from research_agents.agents.researcher import ResearcherAgent
    
    agent = ResearcherAgent(
        agent_id="test-researcher-001",
        llm_service=mock_llm_service,
        database_url=None  # Will use null database service
    )
    return agent


@pytest.fixture 
def assistant_agent_with_mock_services(mock_ktrdr_service):
    """Create an assistant agent with injected mock services."""
    from research_agents.agents.assistant import AssistantAgent
    
    agent = AssistantAgent(
        agent_id="test-assistant-001",
