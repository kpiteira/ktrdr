...
class AssistantAgent(BaseResearchAgent):
    ...
    async def _execute_experiment(self, experiment_id: UUID) -> None:
        ...
        try:
            ...
            await self.log_activity(f"Executing experiment: {experiment['experiment_name']}")
    ...
    async def _prepare_experiment_config(self, experiment: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _monitor_training_progress(self, experiment_id: UUID) -> None:
        ...
        while True:
            try:
                ...
                # Check timeout
                if (datetime.utcnow() - start_time).total_seconds() > self.max_training_time:
                ...
                if status["status"] == "completed":
                    ...
                    raise Exception(f"KTRDR training failed: {status.get('error', 'Unknown error')}")
    ...
    async def _should_stop_training(self, experiment_id: UUID, metrics: Dict[str, Any]) -> bool:
        ...
        if len(training_metrics) >= 20:
            recent_losses = [m["metrics"].get("loss", float("inf")) for m in training_metrics[-20:]]
        ...
        if train_acc > 0.95 and val_acc < 0.4:  # Extreme overfitting
            await self.log_activity(f"Overfitting detected: train={train_acc:.3f}, val={val_acc:.3f}")
            return True
    ...
    async def _generate_experiment_insights(self, experiment_id: UUID, results: Dict[str, Any]) -> None:
    ...
    async def _monitor_active_experiments(self) -> None:
        ...
        for experiment_id in list(self.active_experiments.keys()):
            ...
            if experiment_data["status"] == "running":
                elapsed = (datetime.utcnow() - experiment_data["started_at"]).total_seconds()
    ...
    async def _process_completed_experiments(self) -> None:
        ...
        completed_experiments = [
            exp_id for exp_id, data in self.active_experiments.items()
