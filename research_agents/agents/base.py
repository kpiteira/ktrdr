...
class BaseResearchAgent(ABC):
    ...
    def __init__(
        ...
        **config,
        ...
        self.memory_context = {}
        self._error_count = 0
        self._max_errors = config.get(
            "max_errors", 3
        )  # Allow max 3 errors before stopping
        ...
        try:
            ...
            while self.is_running and not self._stop_requested:
                try:
                    ...
                    self._error_count += 1
                    self.logger.error(
                        f"Error in agent execution cycle: {e} (error {self._error_count}/{self._max_errors})"
                    )
                    ...
                    if self._error_count >= self._max_errors:
                        self.logger.error(
                            f"Agent stopping due to too many errors ({self._max_errors})"
                        )
                        self.is_running = False
                        raise AgentExecutionError(
                            f"Agent failed after {self._max_errors} errors: {e}"
                        ) from e
