...
class ResearchOrchestrator:
    ...
    def __init__(
        ...
        logger.info(
            f"Research orchestrator initialized with max_concurrent={max_concurrent_experiments}"
        )
    ...
    async def shutdown(self) -> None:
        ...
        if self._running_experiments:
            logger.info(
                f"Cancelling {len(self._running_experiments)} running experiments"
            )
    ...
    async def create_experiment(
        self, session_id: UUID, config: ExperimentConfig
    ) -> UUID:
        ...
        try:
            ...
            logger.info(
                f"Experiment created: {experiment_id} '{config.experiment_name}' type={config.experiment_type.value}"
            )
    ...
    async def _execute_experiment(
        ...
        try:
            async with self._experiment_locks[experiment_id]:
                ...
                logger.info(
                    f"Experiment execution started: {experiment_id} '{experiment_config['experiment_name']}'"
                )
                ...
                logger.info(
                    f"Experiment completed successfully: {experiment_id} (time={execution_time:.1f}min, fitness={results.fitness_score})"
                )
            ...
            logger.error(
                f"Experiment failed: {experiment_id} - {e} (time={execution_time:.1f}min)"
            )
    ...
    async def _complete_experiment(
        self, experiment_id: UUID, results: ExperimentResults
    ) -> None:
    ...
    async def _recover_interrupted_experiments(self) -> None:
        ...
        if interrupted:
            ...
            for exp in interrupted:
                ...
                logger.info(
                    f"Marked interrupted experiment as failed: {exp['id']} '{exp['experiment_name']}'"
                )


# Factory function for creating research orchestrator
async def create_research_orchestrator(
    db_service: ResearchDatabaseService, max_concurrent_experiments: int = 3
) -> ResearchOrchestrator:
    """Create and initialize a research orchestrator instance"""
    orchestrator = ResearchOrchestrator(
        db_service=db_service, max_concurrent_experiments=max_concurrent_experiments
    )
