...
class KTRDRIntegrationService:
    ...
    def __init__(
        ...
        logger.info(
            "KTRDR integration service initialized", api_base_url=ktrdr_api_base_url
        )
    ...
    async def submit_training(self, config: TrainingConfig) -> str:
        ...
        try:
            logger.info(
                f"Submitting training job: {config.strategy_name} {config.symbol} {config.timeframe}"
            )
            ...
            async with self._session.post(
                ...
                if response.status == 201:
                    ...
                    logger.info(
                        f"Training job submitted successfully: {training_id} for {config.strategy_name}"
                    )
                    ...
                    raise TrainingError(
                        f"Invalid training configuration: {error_data.get('detail', 'Unknown error')}"
                    )
                    ...
                    raise TrainingError(
                        f"Training submission failed with status {response.status}: {error_text}"
                    )
    ...
    async def get_training_status(self, training_id: str) -> TrainingResults:
        ...
        try:
            ...
            raise TrainingError(
                f"Invalid JSON response from training status API: {e}"
            ) from e
    ...
    async def wait_for_training_completion(
        ...
        while True:
            ...
            if elapsed > max_wait_seconds:
                raise TrainingError(
                    f"Training {training_id} exceeded maximum wait time of {max_wait_minutes} minutes"
                )
            ...
            if results.status == TrainingStatus.COMPLETED:
                ...
                error_msg = (
                    results.error_info.get("message", "Unknown error")
        ...
        try:
            ...
            async with self._session.post(
                ...
                if response.status == 201:
                    ...
                    error_data = await response.json()
                    raise BacktestError(
                        f"Invalid backtest configuration: {error_data.get('detail', 'Unknown error')}"
                    )

                else:
                    error_text = await response.text()
                    raise BacktestError(
                        f"Backtest submission failed with status {response.status}: {error_text}"
                    )

        except aiohttp.ClientError as e:
            raise ConnectionError(
                f"Network error during backtest submission: {e}"
            ) from e
    ...
    async def wait_for_backtest_completion(
        ...
        while True:
            ...
            if elapsed > max_wait_seconds:
                raise BacktestError(
                    f"Backtest {backtest_id} exceeded maximum wait time of {max_wait_minutes} minutes"
                )
    ...
    async def _verify_api_connection(self) -> None:
        ...
        try:
            async with self._session.get(self.health_endpoint) as response:
                if response.status != 200:
                    raise ConnectionError(
                        f"KTRDR API health check failed with status {response.status}"
                    )
