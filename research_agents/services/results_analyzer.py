...
class ResultsAnalyzer:
    ...
    async def analyze_experiment_results(
        ...
        try:
            logger.info(
                "Starting experiment analysis", experiment_id=str(experiment_id)
            )
            ...
            insights = await self._generate_insights(
                performance_metrics, fitness_components
            )
            warnings = await self._generate_warnings(performance_metrics)
            recommendations = await self._generate_recommendations(
                performance_metrics, risk_profile
            )
    ...
    async def _calculate_performance_metrics(
        ...
        annualized_return = (
            total_return * (252 / backtest_days) if backtest_days > 0 else total_return
        )
        ...
        # Trading statistics
        trade_frequency = (
            total_trades / (backtest_days / 252) if backtest_days > 0 else 0.0
        )
    ...
    async def _calculate_fitness_components(
        ...
        robustness_component = await self._calculate_robustness_component(
    ...
    async def _calculate_consistency_component(
    ...
    async def _calculate_efficiency_component(
        self, metrics: PerformanceMetrics
    ) -> float:
        ...
        efficiency_score = (
            freq_score * 0.4 + avg_trade_score * 0.3 + trade_count_score * 0.3
        )
    ...
    async def _calculate_robustness_component(
        ...
        kurtosis_score = 1.0 / (
            1.0 + abs(metrics.kurtosis - 3.0)
        )  # Prefer normal kurtosis
    ...
    async def _classify_risk_profile(self, metrics: PerformanceMetrics) -> RiskProfile:
        ...
        risk_score = (
            vol_score * 0.4 + dd_score * 0.4 + (1.0 / max(sharpe_score, 0.1)) * 0.2
        )
    ...
    async def _generate_warnings(self, metrics: PerformanceMetrics) -> List[str]:
        ...
        if abs(metrics.max_drawdown) > self.max_acceptable_drawdown:
            warnings.append(
                f"Maximum drawdown ({metrics.max_drawdown:.1%}) exceeds acceptable threshold"
            )
        ...
        if metrics.total_trades < self.min_trade_count:
            warnings.append(
                f"Insufficient trade count ({metrics.total_trades}) for statistical significance"
            )

        if metrics.profit_factor < self.min_profit_factor:
            warnings.append(
                f"Profit factor ({metrics.profit_factor:.2f}) below minimum threshold"
            )

        if metrics.volatility > 0.4:
            warnings.append(
                f"High volatility ({metrics.volatility:.1%}) may indicate unstable strategy"
            )
    ...
    async def _generate_recommendations(
        ...
        if metrics.sharpe_ratio < 1.0:
            recommendations.append(
                "Consider improving risk management to enhance Sharpe ratio"
            )

        if abs(metrics.max_drawdown) > 0.15:
            recommendations.append(
                "Implement position sizing or stop-loss mechanisms to reduce drawdown"
            )
        ...
        if metrics.total_trades < 100:
            recommendations.append(
                "Increase sample size by extending backtesting period"
            )

        if risk_profile == RiskProfile.SPECULATIVE:
            recommendations.append(
                "Consider risk reduction measures for better long-term sustainability"
            )
    ...
    async def compare_strategies(self, results: List[AnalysisResult]) -> Dict[str, Any]:
        ...
        comparison = {
            ...
            "statistics": {
                ...
                "success_rate": len([s for s in fitness_scores if s > 1.0])
                / len(fitness_scores),
            },
        ...
        for profile in RiskProfile:
            comparison["risk_profile_distribution"][profile.value] = (
                risk_profiles.count(profile.value)
            )
