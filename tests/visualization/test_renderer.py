...
class TestRenderer:
    ...
    def test_render_chart_light_theme(
        self, renderer, sample_chart_configs, sample_chart_data
    ):
    ...
    def test_save_chart_file_exists(self, renderer, tmp_path):
        ...
        with pytest.raises(DataError) as exc_info:
            renderer.save_chart(
                html_content="New content", output_path=output_path, overwrite=False
            )
        ...
        saved_path = renderer.save_chart(
            html_content="New content", output_path=output_path, overwrite=True
        )
    ...
    def test_update_theme(self, renderer):
        ...
        updated_html = renderer.update_theme(html_content=html_content, theme="light")
    ...
    def test_update_theme_invalid_theme(self, renderer):
        ...
        with pytest.raises(ConfigurationError) as exc_info:
            renderer.update_theme(html_content=html_content, theme="invalid")

        # Check error
        assert "Invalid theme: invalid" in str(exc_info.value)

    def test_generate_standalone_html(
        self, renderer, sample_chart_configs, sample_chart_data, monkeypatch
    ):
