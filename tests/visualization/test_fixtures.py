...
def sample_price_data():
    ...
    base_price = 100.0
    volatility = 2.0
    trend_up = np.array([base_price + i * 1.5 + np.random.normal(0, volatility) for i in range(10)])
    consolidation = np.array([base_price + 15 + np.random.normal(0, volatility) for i in range(10)])
    trend_down = np.array([base_price + 15 - i * 1.0 + np.random.normal(0, volatility) for i in range(10)])
    ...
    volume = np.concatenate([
        ...
        np.random.uniform(1000, 2500, size=10) * 1.7   # Highest volume in trend down
    ])
...
def sample_indicators(sample_price_data):
    ...
    df['macd_line'] = df['ema_12'] = df['close'].ewm(span=12).mean() - df['close'].ewm(span=26).mean()
...
def multiple_series_data():
    ...
    line3 = [65 + 10 * np.sin(i/3) + np.random.normal(0, 1) for i in range(30)]  # Cyclic
