...
class TestMultiTimeframeMLP:
    ...
    def basic_config(self):
        ...
        return {
            "timeframe_configs": {
                "1h": {
                    "expected_features": ["rsi_low", "rsi_high", "macd_negative", "macd_positive"],
                    "weight": 1.0,
                    "enabled": True
                },
                "4h": {
                    "expected_features": ["rsi_low", "rsi_high", "trend_up", "trend_down"],
                    "weight": 1.2,
                    "enabled": True
                },
                "1d": {
    ...
    def test_initialization(self, basic_config):
        ...
        assert all("_1h" in name or "_4h" in name or "_1d" in name for name in model.feature_order)
...
class TestMultiTimeframeFeatureEngineer:
    ...
    def test_prepare_multi_timeframe_features(self, feature_config):
        ...
        assert result.features_tensor.shape[1] == 7  # 3 + 2 + 2 fuzzy + indicator features
    ...
    def test_feature_validation(self, feature_config):
        ...
        # Valid data
        fuzzy_data = {
            "1h": pd.DataFrame({"rsi_low": [0.8], "rsi_high": [0.1], "macd_negative": [0.6]}),
...
class TestMultiTimeframeIntegration:
    ...
    def test_end_to_end_workflow(self, integration_config):
        ...
        fuzzy_data = {
            "1h": pd.DataFrame({
                "rsi_low": [0.8, 0.6, 0.9],
                "rsi_high": [0.1, 0.3, 0.05]
            }),
            "4h": pd.DataFrame({
                "trend_up": [0.7, 0.8, 0.6],
                "trend_down": [0.2, 0.1, 0.3]
            })
        }
