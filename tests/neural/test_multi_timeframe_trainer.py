...
class TestMultiTimeframeTrainer:
    ...
    @patch(
        "ktrdr.neural.training.multi_timeframe_trainer.MultiTimeframeFeatureEngineer"
    )
    ...
    def test_prepare_timeframe_features(
        self, basic_training_config, sample_multi_timeframe_data
    ):
        """Test timeframe feature preparation."""
        trainer = MultiTimeframeTrainer(basic_training_config)

        features_by_timeframe = trainer._prepare_timeframe_features(
            sample_multi_timeframe_data
        )
        ...
        assert (
            features_by_timeframe["1h"].shape[1] == 8 + 5
        )  # fuzzy + indicator features
    ...
    def test_cross_timeframe_consistency_calculation(self, basic_training_config):
        ...
        features_by_timeframe = {
            "1h": np.column_stack(
                [base_signal + 0.1 * np.random.randn(100) for _ in range(5)]
            ),
            "4h": np.column_stack(
                [base_signal + 0.2 * np.random.randn(100) for _ in range(3)]
            ),
            "1d": np.column_stack(
                [base_signal + 0.3 * np.random.randn(100) for _ in range(2)]
            ),
        }

        consistency = trainer._calculate_cross_timeframe_consistency(
            features_by_timeframe
        )
    ...
    def test_single_timeframe_consistency(self, basic_training_config):
        ...
        features_by_timeframe = {"1h": np.random.randn(100, 5)}
