...
def mock_fuzzy_engine():
    ...
    with patch('ktrdr.api.services.fuzzy_service.FuzzyEngine') as mock_engine:
        ...
        mock_instance.get_output_names.side_effect = lambda indicator: [f"{indicator}_{s}" for s in 
            {
                "rsi": ["low", "medium", "high"],
                "macd": ["negative", "zero", "positive"],
                "bb": ["lower", "middle", "upper"]
...
def mock_config_loader():
    ...
    with patch('ktrdr.api.services.fuzzy_service.FuzzyConfigLoader') as mock_loader_class:
...
@pytest.mark.api
@pytest.mark.asyncio
async def test_fuzzify_indicator_unknown_indicator(mock_fuzzy_engine, mock_config_loader):
...
@pytest.mark.api
@pytest.mark.asyncio
async def test_fuzzify_data_with_dates(mock_fuzzy_engine, mock_data_manager, mock_config_loader):
...
@pytest.mark.api
@pytest.mark.asyncio
async def test_fuzzify_data_load_error(mock_fuzzy_engine, mock_data_manager, mock_config_loader):
...
@pytest.mark.api
@pytest.mark.asyncio
async def test_fuzzify_data_empty_data(mock_fuzzy_engine, mock_data_manager, mock_config_loader):
