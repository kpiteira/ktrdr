...
def sample_rsi_data():
    ...
    rsi_values = 50 + 30 * np.sin(np.linspace(0, 4*np.pi, 100)) + np.random.randn(100) * 5
...
class TestFuzzyServiceEnhanced:
    ...
    def test_initialization_with_batch_calculator(self, mock_indicator_engine, mock_data_manager, mock_config_loader, sample_fuzzy_config):
        """Test that FuzzyService initializes with BatchFuzzyCalculator."""
        # Configure mocks
        mock_config_loader.return_value.load_from_yaml.return_value = sample_fuzzy_config
    ...
    @pytest.mark.asyncio
    @patch('ktrdr.api.services.fuzzy_service.FuzzyConfigLoader')
    async def test_get_fuzzy_overlays_basic(self, mock_config_loader, sample_fuzzy_config, sample_ohlcv_data, sample_rsi_data):
    ...
    async def test_get_fuzzy_overlays_uninitialized_engine(self, mock_config_loader):
        ...
        mock_config_loader.return_value.load_from_yaml.side_effect = Exception("Config load failed")
