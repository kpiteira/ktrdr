...
class TestTrainingService:
    ...
    async def test_get_training_status_success(
        ...
        status = await training_service.get_training_status("test_training_id")

        assert status["success"] is True
        assert status["task_id"] == "test_training_id"
        assert status["status"] == "running"
        assert status["progress"] == 65
        assert status["current_epoch"] == 65
        assert status["total_epochs"] == 100
        assert status["symbol"] == "AAPL"
        assert status["timeframe"] == "1h"
        assert status["current_metrics"]["train_loss"] == 0.045
    ...
    async def test_get_training_status_not_found(
        ...
        with pytest.raises(ValidationError, match="not found"):
            await training_service.get_training_status("nonexistent_id")

    @pytest.mark.asyncio
    async def test_get_training_status_with_completion_estimate(
        self, training_service, mock_operations_service
    ):
        """Test getting training status with estimated completion time."""
        # Mock operation with progress for time estimation
        mock_operation = MagicMock()
        mock_operation.operation_id = "test_training_id"
        mock_operation.metadata.symbol = "AAPL"
        mock_operation.metadata.timeframe = "1h"
        mock_operation.status.value = "running"
        mock_operation.progress.percentage = 50.0
        mock_operation.progress.items_processed = 50
        mock_operation.progress.items_total = 100

        # Set started time 10 minutes ago
        start_time = datetime.now(timezone.utc).replace(second=0, microsecond=0)
        mock_operation.started_at = start_time
        mock_operation.error_message = None
        mock_operation.progress.details = {}

        mock_operations_service.get_operation.return_value = mock_operation

        with patch("ktrdr.api.services.training_service.datetime") as mock_datetime:
            # Mock current time to be 10 minutes later
            current_time = start_time.replace(minute=start_time.minute + 10)
