...
@pytest.mark.asyncio
async def test_calculate_indicators_with_valid_request(
    indicator_service, mock_data_frame
):
    """Test indicator calculation with a valid request."""
    # Mock the data_manager to return our test DataFrame
    with patch.object(
        indicator_service.data_manager, "load", return_value=mock_data_frame
    ):
        ...
        request = IndicatorCalculateRequest(
            ...
            indicators=[
                IndicatorConfig(
                    id="RSIIndicator", parameters={"period": 5, "source": "close"}
                )
            ],
        )
...
async def test_calculate_indicators_with_data_error(indicator_service):
    ...
    with patch.object(
        ...
        request = IndicatorCalculateRequest(
            symbol="UNKNOWN",
            timeframe="1d",
            indicators=[IndicatorConfig(id="RSIIndicator", parameters={"period": 14})],
        )
...
@pytest.mark.asyncio
async def test_calculate_indicators_with_unknown_indicator(
    indicator_service, mock_data_frame
):
    ...
    with patch.object(
        ...
        request = IndicatorCalculateRequest(
            symbol="AAPL",
            timeframe="1d",
            indicators=[IndicatorConfig(id="UnknownIndicator", parameters={})],
        )
...
@pytest.mark.asyncio
async def test_calculate_indicators_with_invalid_parameters(
    indicator_service, mock_data_frame
):
    ...
    with patch.object(
        ...
        request = IndicatorCalculateRequest(
            symbol="AAPL",
            timeframe="1d",
            indicators=[IndicatorConfig(id="RSIIndicator", parameters={"period": -5})],
        )
...
async def test_health_check_healthy(indicator_service):
    ...
    with patch(
        "ktrdr.api.services.indicator_service.BUILT_IN_INDICATORS",
        {
            "RSIIndicator": MagicMock(),
            "SMAIndicator": MagicMock(),
