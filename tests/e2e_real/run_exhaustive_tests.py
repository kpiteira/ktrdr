...
class ExhaustiveTestRunner:
    ...
    def __init__(self, ib_host: str = "127.0.0.1", ib_port: int = 4003, 
                 api_base_url: str = "http://localhost:8000", verbose: bool = False):
    ...
    def check_prerequisites(self) -> Dict[str, Any]:
        ...
        try:
            ...
            client.close()
            print(f"  ✅ API server: {'running' if results['api_server'] else 'not available'}")
        ...
        try:
            ...
            client.close()
            print(f"  {'✅' if results['ib_gateway'] else '⚠️ '} IB Gateway: {'connected' if results['ib_gateway'] else 'not connected'}")
        except Exception as e:
        ...
        try:
            result = subprocess.run(
                ["docker", "ps", "--filter", "name=ktrdr-backend", "--format", "{{.Names}}"],
                capture_output=True, text=True, timeout=5
            )
            results["backend_container"] = "ktrdr-backend" in result.stdout
            print(f"  {'✅' if results['backend_container'] else '❌'} Backend container: {'running' if results['backend_container'] else 'not running'}")
        except Exception as e:
        ...
        print(f"  {'✅' if results['test_environment'] else '❌'} Test environment: {'ready' if results['test_environment'] else f'missing {missing_files}'}")
    ...
    def _parse_pytest_output(self, stdout: str, stderr: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if any(term in line.lower() for term in ["runtimewarning", "coroutine", "was never awaited"]):
    ...
    def generate_report(self, test_results: Dict[str, Any], output_file: str = None) -> str:
...
"""
        ...
        if "test_results" in test_results and isinstance(test_results["test_results"], dict):
...
"""
    ...
    def run_comprehensive_test(self, test_level: str = "standard", generate_report: bool = False) -> bool:
...
def main():
    ...
    parser.add_argument("--test-level", choices=["basic", "standard", "exhaustive"], 
