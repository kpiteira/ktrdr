...
class TestRealIBErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_real_pace_limiting_handling(self, clean_test_symbols, real_ib_connection_test):
    ...
    @pytest.mark.asyncio
    async def test_real_connection_pool_exhaustion_recovery(self, real_ib_connection_test):
        ...
        for symbol in invalid_symbols:
            try:
                ...
                assert any(word in error_str for word in [
                    "invalid", "symbol", "not found", "error", "failed"
                ])

    @pytest.mark.asyncio
    async def test_real_data_fetching_error_scenarios(self, clean_test_symbols, real_ib_connection_test):
        ...
        try:
            ...
            assert any(word in error_str for word in [
                "future", "date", "invalid", "range", "data"
            ])
    ...
    async def test_real_connection_timeout_recovery(self, real_ib_connection_test):
        ...
        try:
            ...
            assert any(word in error_str for word in [
                "timeout", "connection", "error", "failed"
            ])

    @pytest.mark.asyncio
    async def test_real_head_timestamp_error_scenarios(self, clean_test_symbols, real_ib_connection_test):
...
class TestRealSystemErrorRecovery:
    ...
    @pytest.mark.asyncio
    async def test_datamanager_ib_fallback_error_recovery(self, clean_test_symbols, real_ib_connection_test):
        ...
        for scenario in error_scenarios:
            try:
                ...
                assert "runtimewarning" not in error_str, f"Async error in {scenario['name']}: {e}"
                assert "coroutine" not in error_str, f"Coroutine error in {scenario['name']}: {e}"
                assert "was never awaited" not in error_str, f"Await error in {scenario['name']}: {e}"
                
                # Should be meaningful error message
                assert len(error_str) > 0, f"Empty error message in {scenario['name']}"

    @pytest.mark.asyncio
    async def test_api_endpoint_error_propagation(self, api_client, real_ib_connection_test):
        """Test that API endpoints properly handle and propagate IB errors."""
        
        # Test symbol discovery with potentially problematic symbol
        request_data = {
            "symbol": "VERY_INVALID_SYMBOL_XYZ123",
            "force_refresh": True
        }
    ...
    @pytest.mark.asyncio
    async def test_concurrent_error_scenarios(self, clean_test_symbols, real_ib_connection_test):
        ...
        async def operation_with_errors(symbol, operation_type):
            ...
            try:
                if operation_type == "validate":
                    validator = IbSymbolValidatorUnified(component_name=f"concurrent_error_{symbol}")
                    return await validator.validate_symbol_async(symbol)
                    
                elif operation_type == "fetch":
                    fetcher = IbDataFetcherUnified(component_name=f"concurrent_error_{symbol}")
        ...
        # Run concurrently
        tasks = [operation_with_errors(symbol, op_type) for symbol, op_type in operations]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                ...
                assert "runtimewarning" not in error_str, f"Async error in {operation_desc}"
                assert "coroutine" not in error_str, f"Coroutine error in {operation_desc}"
                assert "was never awaited" not in error_str, f"Await error in {operation_desc}"
...
class TestRealPerformanceUnderStress:
    ...
    @pytest.mark.asyncio
    async def test_rapid_sequential_operations(self, clean_test_symbols, real_ib_connection_test):
    ...
    @pytest.mark.asyncio
    async def test_memory_stability_under_load(self, clean_test_symbols, real_ib_connection_test):
