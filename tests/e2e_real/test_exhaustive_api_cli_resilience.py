...
class TestAPIResilienceUnderRealLoad:
    ...
    def test_api_endpoints_under_connection_stress(self, api_client, clean_test_symbols, real_ib_connection_test):
        ...
        def test_endpoint(endpoint_info):
            ...
            try:
                ...
                return {
                    ...
                    "elapsed": elapsed,
                    "success": response.status_code in [200, 503],  # 503 is acceptable for unavailable services
        ...
        for round_num in range(3):
            ...
            print(f"  Round {round_num + 1}: {len(successful_requests)}/{len(round_results)} successful")
            
            if failed_requests:
                for failed in failed_requests[:3]:  # Show first 3 failures
                    print(f"  Failed: {failed['url']} - {failed.get('error', 'HTTP error')}")
    ...
    def test_symbol_discovery_resilience_under_load(self, api_client, clean_test_symbols, real_ib_connection_test):
        ...
        def test_symbol_discovery(symbol, test_id):
            ...
            try:
                ...
                return {
                    ...
                    "has_symbol_info": data.get("data", {}) is not None if data.get("success") else False
                }
        ...
        for symbol, symbol_results in by_symbol.items():
            ...
            print(f"✅ {symbol}: {len(successful)}/{len(symbol_results)} successful, avg {avg_time:.2f}s")
            
            # Each symbol should have some successes
            assert len(successful) > 0, f"All requests failed for {symbol}"

    def test_data_loading_api_resilience(self, api_client, clean_test_symbols, real_ib_connection_test):
        ...
        for scenario in test_scenarios:
            ...
            try:
                ...
                assert response.status_code in [200, 400], f"Unexpected status for {scenario_name}"
...
class TestCLIResilienceUnderRealLoad:
    ...
    def test_cli_commands_no_async_errors_under_load(self, clean_test_symbols):
        ...
        for result in all_cli_results:
            if not result["success"]:
                ...
                print(f"❌ ASYNC ERROR in '{result['command']}': {result['async_errors']}")
            else:
                status = "timeout" if result.get("timeout") else f"rc:{result.get('returncode', 'unknown')}"
                print(f"✅ '{result['command']}': {status} in {result['elapsed']:.2f}s")
        
        # CRITICAL: No commands should have async errors
        assert len(commands_with_async_errors) == 0, f"Commands with async errors: {commands_with_async_errors}"
        
        print(f"✅ CLI resilience: {len(all_cli_results)} commands completed without async errors")
    ...
    def test_cli_concurrent_ib_operations(self, clean_test_symbols):
        ...
        def run_ib_test_command(symbol, worker_id):
            ...
            try:
                ...
                has_async_error = any(term in combined_output for term in [
        ...
        assert async_error_count == 0, "Async errors detected in concurrent CLI operations"
    ...
    def test_cli_memory_stability_over_time(self, clean_test_symbols):
        ...
        if len(times) > 1:
            ...
            assert time_variance < 10.0, f"High time variance suggests memory issues: {time_variance:.2f}s"
...
class TestFullSystemIntegrationResilience:
    ...
    def test_end_to_end_data_pipeline_resilience(self, api_client, clean_test_symbols, real_ib_connection_test):
        ...
        discovery_response = api_client.post("/api/v1/ib/symbols/discover", json=discovery_request)
        ...
        if not discovery_data.get("success"):
            print(f"  Symbol discovery failed (IB unavailable): {discovery_data.get('error', {}).get('message')}")
        else:
        ...
        assert data_response.status_code in [200, 400]  # 400 acceptable for invalid params
        ...
        print(f"  System health: {'healthy' if health_data.get('healthy') else 'unhealthy'}")
        ...
        phases = ["phase_1_systematic_validation", "phase_2_garbage_collection", "phase_3_client_id_preference"]
        for phase in phases:
    ...
    def test_system_recovery_after_simulated_stress(self, api_client, clean_test_symbols, real_ib_connection_test):
        ...
        stress_success_count = sum(1 for r in stress_requests if isinstance(r, int) and r in [200, 503])
        ...
        assert score_degradation <= 10, f"Resilience score degraded too much: {score_degradation}"
