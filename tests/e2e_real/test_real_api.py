...
class TestRealAPIDataFlow:
    ...
    def test_real_ib_symbol_discovery(self, api_client, clean_test_symbols, ib_availability_check):
        ...
        if ib_availability_check["available"]:
            ...
            if not data["success"]:
                ...
                assert any(word in error_msg for word in ["ib", "connection", "gateway", "unavailable", "timeout", "discover"])
    ...
    @pytest.mark.asyncio
    async def test_real_data_load_api_with_ib_fallback(self, api_client, clean_test_symbols, e2e_helper, real_ib_connection_test):
        ...
        # Make async request
        response = api_client.post("/api/v1/data/load?async_mode=true", json=request_data)
        ...
        try:
            result = await e2e_helper.wait_for_operation_completion(api_client, operation_id, timeout=60)
            ...
            if status_response.status_code == 200:
                ...
                pytest.fail(f"Operation timed out and status check failed: {status_response.status_code}")

    @pytest.mark.asyncio 
    async def test_real_ib_load_api_endpoint(self, api_client, clean_test_symbols, e2e_helper, real_ib_connection_test):
        ...
        result = await e2e_helper.wait_for_operation_completion(api_client, operation_id, timeout=45)
        
        # Verify IB operation results
        assert result["status"] in ["completed", "failed"]  # Could fail if IB unavailable
    ...
    @pytest.mark.asyncio
    async def test_real_ib_cleanup_api_endpoint(self, api_client, real_ib_connection_test):
    ...
    @pytest.mark.asyncio
    async def test_real_data_endpoint_with_ib_fallback(self, api_client, clean_test_symbols, real_ib_connection_test):
    ...
    @pytest.mark.asyncio
    async def test_real_ib_resilience_endpoint_with_ib_running(self, api_client, real_ib_connection_test):
...
class TestRealAPIErrorScenarios:
    ...
    @pytest.mark.asyncio
    async def test_api_handles_invalid_symbol_discovery(self, api_client, real_ib_connection_test):
    ...
    @pytest.mark.asyncio
    async def test_api_handles_future_date_range(self, api_client, clean_test_symbols, real_ib_connection_test):
    ...
    @pytest.mark.asyncio
    async def test_api_concurrent_ib_operations(self, api_client, clean_test_symbols, real_ib_connection_test):
        ...
        async def test_ib_status():
            ...
            response = await loop.run_in_executor(None, lambda: api_client.get("/api/v1/ib/status"))
        ...
        async def test_symbol_discovery(symbol):
            ...
            response = await loop.run_in_executor(
                None, 
                lambda: api_client.post("/api/v1/ib/symbols/discover", json=request_data)
            )
        ...
        for result in results:
            ...
            assert status_code == 200, f"Operation {operation_type} failed with status {status_code}"
...
class TestRealAPIWhenIBUnavailable:
    ...
    def test_ib_status_graceful_failure_without_ib(self, api_client, ib_availability_check):
    ...
    def test_ib_health_reports_unavailable_without_ib(self, api_client, ib_availability_check):
    ...
    def test_data_endpoints_graceful_fallback_without_ib(self, api_client, clean_test_symbols, ib_availability_check):
...
class TestRealAPIDataQuality:
    ...
    @pytest.mark.asyncio
    async def test_real_data_consistency_across_endpoints(self, api_client, clean_test_symbols, real_ib_connection_test):
