...
class TestRealDataPipeline:
    ...
    async def test_complete_symbol_validation_to_data_loading_pipeline(
        ...
        if not df.empty:  # Data might be empty for weekends/holidays
            ...
            assert len(loaded_df) == len(df), "Loaded data should match fetched data length"
    ...
    async def test_data_manager_full_integration_with_ib(
        ...
        try:
            ...
            if "data not found" not in error_str and "no data available" not in error_str:
                raise
    ...
    async def test_forex_symbol_complete_pipeline(
        ...
        assert validator.is_forex_symbol(forex_symbol), f"{forex_symbol} should be detected as forex"
    ...
    async def test_head_timestamp_integration_pipeline(
        ...
        if head_timestamp:  # Might be None if IB doesn't support it for this symbol
            ...
            is_valid, warning_msg, suggested_date = validator.validate_date_range_against_head_timestamp(
                symbol=symbol,
            ...
            if suggested_date:
                assert suggested_date >= head_dt, "Suggested date should be after head timestamp"
    ...
    async def test_concurrent_pipeline_operations(
        ...
        async def run_pipeline(symbol):
            ...
            try:
                # Validation
                validator = IbSymbolValidatorUnified(component_name=f"concurrent_{symbol}")
                is_valid = await validator.validate_symbol_async(symbol)
                ...
                if is_valid:
                    ...
                    return {
                        'symbol': symbol,
                        'valid': True,
                        'data_loaded': df is not None and not df.empty if df is not None else False
                    }
                else:
    ...
    async def test_pipeline_handles_ib_disconnection_gracefully(
        ...
        try:
            ...
            assert any(phrase in error_str for phrase in [
                "connection", "timeout", "data not found", "ib error"
