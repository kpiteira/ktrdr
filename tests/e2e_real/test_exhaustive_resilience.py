...
class TestPhase1SystematicValidationExhaustive:
    ...
    @pytest.mark.asyncio
    async def test_validation_prevents_silent_connections(self, real_ib_connection_test):
            ...
            try:
                async with pool.acquire_connection(
                    ...
                    accounts = await asyncio.wait_for(
                        connection.ib.reqManagedAcctsAsync(), 
                        timeout=5.0
                    )
        ...
        assert successful_validations > 0, f"No successful validations out of 10 attempts"
        print(f"✅ Systematic validation: {successful_validations}/10 succeeded, {failed_validations}/10 failed gracefully")
    ...
    async def test_validation_timing_requirements(self, real_ib_connection_test):
        ...
        for i in range(5):
            ...
            try:
                ...
                assert elapsed < 15.0, f"Failed validation took too long: {elapsed:.2f}s"
        
        avg_time = sum(validation_times) / len(validation_times)
        print(f"✅ Average validation time: {avg_time:.2f}s (target: <3s per validation)")
        
        # Most validations should be reasonably fast
        fast_validations = sum(1 for t in validation_times if t < 5.0)
        assert fast_validations >= len(validation_times) // 2, "Too many slow validations"
    ...
    async def test_concurrent_validation_integrity(self, real_ib_connection_test):
        ...
        async def validate_connection(connection_id: str):
            ...
            try:
                async with pool.acquire_connection(
                    ...
                    return {"id": connection_id, "success": True, "client_id": connection.client_id}
        ...
        for result in results:
            assert not isinstance(result, Exception), f"Concurrent validation failed: {result}"
...
class TestPhase2GarbageCollectionExhaustive:
    ...
    async def test_garbage_collection_timing_accuracy(self, real_ib_connection_test):
        ...
        print(f"✅ Garbage collection configured: {phase2['max_idle_time_seconds']}s idle timeout")
    ...
    async def test_connection_lifecycle_tracking(self, real_ib_connection_test):
        ...
        for i in range(3):
            async with pool.acquire_connection(
                purpose=ClientIdPurpose.API_POOL,
                requested_by=f"lifecycle_test_{i}"
            ) as connection:
...
class TestPhase3ClientIdPreferenceExhaustive:
        ...
        try:
            ...
            for i in range(5):
                try:
                    ...
                    print(f"✅ Connection {i}: allocated Client ID {connection.client_id}")
            ...
            if allocated_ids:
                ...
                assert first_id <= 3, f"First connection should get low ID, got {first_id}"
                
                # IDs should be sequential or close to sequential
                if len(allocated_ids) > 1:
                    id_gaps = [allocated_ids[i+1] - allocated_ids[i] for i in range(len(allocated_ids)-1)]
    ...
    async def test_client_id_reuse_after_cleanup(self, real_ib_connection_test):
        ...
        for i in range(3):
            async with pool.acquire_connection(
                purpose=ClientIdPurpose.API_POOL,
                requested_by=f"reuse_test_first_{i}"
            ) as connection:
...
class TestConnectionPoolStressExhaustive:
    ...
    async def test_rapid_connection_creation_destruction(self, real_ib_connection_test):
        ...
        print(f"✅ Rapid operations: {successful_operations}/20 succeeded in {elapsed:.2f}s")
        print(f"✅ Failed operations: {failed_operations}/20 (acceptable for stress test)")
    ...
    async def test_concurrent_heavy_load(self, real_ib_connection_test):
        ...
        async def heavy_connection_work(worker_id: int):
            ...
            try:
                async with pool.acquire_connection(
                    ...
                    return {"worker": worker_id, "success": True, "results": len(results)}
        ...
        for result in results:
            assert not isinstance(result, Exception), f"Worker failed with exception: {result}"
    ...
    async def test_connection_pool_recovery_after_stress(self, real_ib_connection_test):
        ...
        for i in range(5):
            try:
                async with pool.acquire_connection(
                    purpose=ClientIdPurpose.API_POOL,
                    requested_by=f"recovery_test_{i}"
                ) as connection:
                    await connection.ib.reqCurrentTimeAsync()
        ...
        for test in timeout_tests:
            ...
            try:
                ...
                async with pool.acquire_connection(
                    ...
                    assert elapsed < timeout * 0.8, f"Operation too slow for {description}"
                    
            except asyncio.TimeoutError:
                elapsed = time.time() - start_time
                print(f"⚠️  {description}: timed out after {elapsed:.2f}s (this is acceptable)")
...
class TestResilienceScoreValidationExhaustive:
    ...
    def test_resilience_score_with_active_connections(self, real_ib_connection_test):
        ...
        try:
            ...
            print(f"✅ Pool health: {pool_health['healthy_connections']}/{pool_health['total_connections']} healthy")
    ...
    async def test_resilience_score_under_load(self, real_ib_connection_test):
        ...
        try:
            ...
            assert score_under_load >= 65, f"Score dropped too much under load: {score_under_load}"
            
            # Pool should show active connections
            pool_health = data["connection_pool_health"]
            assert pool_health["active_connections"] > 0, "Should show active connections under load"
    ...
    def test_resilience_timestamp_accuracy(self, real_ib_connection_test):
        ...
        try:
            ...
            assert before_time <= timestamp <= after_time, "Timestamp not in expected range"
