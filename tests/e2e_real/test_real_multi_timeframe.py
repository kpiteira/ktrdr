...
class TestRealMultiTimeframeDataLoading:
    ...
    def test_real_aapl_multi_timeframe_loading(self, manager, basic_config):
        ...
        assert all(col in primary_data.columns for col in ["open", "high", "low", "close", "volume"])
        ...
        for timeframe, data in result.data.items():
            ...
            assert isinstance(data.index, pd.DatetimeIndex), f"Invalid index for {timeframe}"
    ...
    def test_real_multi_timeframe_with_limited_symbol(self, manager):
        ...
        requested_timeframes = {config.primary_timeframe} | set(config.auxiliary_timeframes)
    ...
    def test_real_graceful_degradation(self, manager):
        ...
        for symbol, result in results.items():
            success_rate = len(result.available_timeframes) / (1 + len(config.auxiliary_timeframes))
            print(f"   {symbol}: {len(result.available_timeframes)}/3 timeframes "
    ...
    def test_real_cache_behavior(self, manager, basic_config):
        ...
        # Verify cache hit
        assert second_load_time < first_load_time, "Cache should make second load faster"
    ...
    def test_real_market_gaps_handling(self, synchronizer, manager):
        ...
        if "1h" in result.data and "1d" in result.data:
            ...
            for tf, is_consistent in consistency_results.items():
                print(f"      {tf}: {'✅ consistent' if is_consistent else '⚠️ gaps detected'}")
    ...
    def test_real_memory_usage_large_dataset(self):
        ...
        for symbol in symbols:
            try:
                ...
                print(f"   {symbol}: {len(result.available_timeframes)} timeframes, "
...
class TestRealErrorScenarios:
    ...
    def test_real_ib_unavailable_handling(self):
        ...
        try:
            ...
            if any(keyword in str(e).lower() for keyword in ["connection", "ib", "gateway"]):
...
def run_comprehensive_real_multi_timeframe_test():
    ...
    try:
        manager = MultiTimeframeDataManager(enable_ib=True, enable_synthetic_generation=True)
