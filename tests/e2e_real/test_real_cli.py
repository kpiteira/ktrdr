...
class TestRealCLICommands:
    ...
    def test_real_ib_test_command(self, clean_test_symbols):
        ...
        result = subprocess.run([
            "uv", "run", "ktrdr", "ib", "test", 
            "--symbol", symbol,
            "--verbose"
        ], capture_output=True, text=True)
        
        # Verify no runtime warnings (the bug we just fixed)
        assert "RuntimeWarning" not in result.stderr, f"Runtime warning detected: {result.stderr}"
        assert "coroutine" not in result.stderr, f"Coroutine error detected: {result.stderr}"
    ...
    def test_real_data_load_command_tail_mode(self, clean_test_symbols, test_date_ranges):
        """Test 'ktrdr data load' command with real IB in tail mode."""
        symbol = clean_test_symbols[0]  # AAPL
        date_range = test_date_ranges["recent_days"]
        
        result = subprocess.run([
            "uv", "run", "ktrdr", "data", "load", symbol,
            "-t", "1h",
        ...
        assert "RuntimeWarning" not in result.stderr, f"Runtime warning detected: {result.stderr}"
        assert "coroutine 'acquire_ib_connection' was never awaited" not in result.stderr
    ...
    def test_real_data_load_command_full_mode(self, clean_test_symbols, test_date_ranges):
        ...
        if result.returncode == 0:
            assert any(word in result.stdout.lower() for word in ["loaded", "bars", "completed"])
    ...
    def test_real_ib_load_command(self, clean_test_symbols, test_date_ranges):
        ...
        assert result.returncode in [0, 1], f"Command crashed unexpectedly: {result.stderr}"

    def test_real_ib_cleanup_command(self):
        """Test 'ktrdr ib-cleanup' command with real IB connections."""
        result = subprocess.run([
            "uv", "run", "ktrdr", "ib", "cleanup",
            "--verbose"
        ], capture_output=True, text=True)
        ...
        stdout_lower = result.stdout.lower()
        assert any(word in stdout_lower for word in ["cleanup", "connections", "closed", "completed"])
    ...
    def test_real_cli_concurrent_operations(self, clean_test_symbols):
        ...
        threads = [threading.Thread(target=run_test_ib, args=(symbol,)) for symbol in symbols]
