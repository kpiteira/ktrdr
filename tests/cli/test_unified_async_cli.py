"""Tests for UnifiedAsyncCLI base class."""
...
class TestUnifiedAsyncCLI:
    ...
    async def cli_instance(self, cli_config):
        ...
        cli = UnifiedAsyncCLI(**cli_config)
        yield cli
        # Cleanup
    ...
    async def test_context_manager_lifecycle(self, cli_config):
        ...
        async with UnifiedAsyncCLI(**cli_config) as cli:
    ...
    async def test_proper_error_handling(self, cli_config):
        ...
        async with UnifiedAsyncCLI(**cli_config) as cli:
            with patch.object(
                ...
                with pytest.raises(UnifiedAsyncCLIError) as exc_info:
    ...
    async def test_custom_parameters_per_request(self, cli_config):
        ...
        async with UnifiedAsyncCLI(**cli_config) as cli:
            ...
            with patch.object(
                ...
                with pytest.raises(UnifiedAsyncCLIError):
    ...
    async def test_resource_cleanup_on_exception(self, cli_config):
        ...
        cli = None
        try:
            async with UnifiedAsyncCLI(**cli_config) as instance:
                cli = instance
                assert cli._http_client is not None
                assert not cli._http_client.is_closed
