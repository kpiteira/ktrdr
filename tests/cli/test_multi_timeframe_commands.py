...
class TestMultiTimeframeCLICommands:
    ...
    def test_decide_command_help(self, runner):
        ...
        assert result.exit_code == 0
        assert "Generate a trading decision using multi-timeframe analysis" in result.stdout
    ...
    def test_decide_command_invalid_symbol(self, runner, temp_strategy_file):
        ...
        result = runner.invoke(multi_timeframe_app, [
            "decide", 
            "INVALID@SYMBOL", 
            temp_strategy_file
        ])
    ...
    @patch('ktrdr.cli.multi_timeframe_commands.create_multi_timeframe_decision_orchestrator')
    ...
    @patch('ktrdr.cli.multi_timeframe_commands.DataManager')
    def test_status_command_direct_success(
        self, 
        mock_dm,
        runner
    ):
    ...
    def test_decide_command_with_all_options(self, runner, temp_strategy_file):
        ...
        assert "Making multi-timeframe decision" in result.stdout or result.exit_code == 1
    ...
    def test_analyze_command_with_all_options(self, runner, temp_strategy_file):
        ...
        assert "Analyzing timeframe performance" in result.stdout or result.exit_code == 1
