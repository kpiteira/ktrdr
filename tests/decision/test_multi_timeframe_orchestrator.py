...
class TestMultiTimeframeDecisionOrchestrator:
    ...
    def mock_orchestrator(self, temp_strategy_config_file):
        ...
        with patch('ktrdr.decision.multi_timeframe_orchestrator.DecisionOrchestrator') as mock_single_tf:
    ...
    def test_freshness_score_calculation(self, mock_orchestrator, sample_timeframe_data):
    ...
    def test_timeframe_context_preparation(self, mock_orchestrator, sample_timeframe_data):
    ...
    def test_weighted_majority_consensus(self, mock_orchestrator):
        ...
        consensus = mock_orchestrator._weighted_majority_consensus(timeframe_decisions, context)
        
        assert isinstance(consensus, MultiTimeframeConsensus)
        assert consensus.final_signal == Signal.BUY  # Should win with higher weighted votes
    ...
    def test_hierarchical_consensus(self, mock_orchestrator):
        ...
        consensus = mock_orchestrator._hierarchical_consensus(timeframe_decisions, context)
    ...
    def test_state_reset(self, mock_orchestrator):
        ...
        mock_orchestrator.decision_history.append(TradingDecision(
            signal=Signal.BUY,
            confidence=0.8,
            timestamp=pd.Timestamp.now(tz="UTC"),
