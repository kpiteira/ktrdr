...
class Phase4Validator:
    ...
    async def _test_service_management(self) -> List[ValidationResult]:
        ...
        try:
            ...
            if success:
                ...
                success = (ib_status.name == "RUNNING" and training_status.name == "RUNNING")
            else:
    ...
    async def _test_health_monitoring(self) -> List[ValidationResult]:
        ...
        try:
            ...
            has_gpu_metrics = (
                training_health.gpu_metrics is not None and
                hasattr(training_health.gpu_metrics, 'available')
            )
    ...
    async def _test_system_integration(self) -> List[ValidationResult]:
        ...
        try:
            ...
            success = (
                ib_status.name == "RUNNING" and
                training_status.name == "RUNNING" and
                health_report.get("overall_status") in ["healthy", "warning"]
            )
    ...
    async def _test_rollback_procedures(self) -> List[ValidationResult]:
        ...
        try:
            ...
            final_training_status = self.service_manager.get_service_status("training-host")
            ...
            success = (
                stop_success and
                restart_success and
                final_ib_status.name == "RUNNING" and
                final_training_status.name == "RUNNING"
            )
    ...
    def print_validation_summary(self, report: Dict[str, Any]):
        ...
        for category, stats in report["categories"].items():
            ...
            print(f"  {status} {category.title()}: {stats['passed']}/{stats['total']} ({success_rate:.1f}%)")
        ...
        if report["performance_benchmarks"]:
            ...
            for bench in report["performance_benchmarks"]:
                ...
                print(f"  {status} {bench['test_name']}: {bench['improvement_factor']:.2f}x improvement")
        
        # Overall status
        if summary["phase4_complete"]:
            print(f"\n🎉 Phase 4 Validation: COMPLETE")
            print(f"✅ GPU acceleration implementation validated and ready for production!")
        else:
