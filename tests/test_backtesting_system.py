...
class TestPositionManager:
    ...
    def test_portfolio_value_calculation(self):
        ...
        if pm.current_position:
            unrealized_pnl = (105.0 - pm.current_position.entry_price) * pm.current_position.quantity
            expected_total = pm.current_capital + unrealized_pnl
            assert abs(portfolio_value_higher - expected_total) < 0.01  # Should match calculation
...
def create_sample_price_data(n_periods: int = 100, start_price: float = 100.0) -> pd.DataFrame:
