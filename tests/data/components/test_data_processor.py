...
from ktrdr.data.components.data_processor import (
    DataProcessor,
    ProcessorConfig,
    ValidationResult,
)
...
class TestDataProcessor:
    ...
    def valid_ohlc_data(self):
        ...
        dates = pd.date_range(
            start="2023-01-01 09:30:00", periods=100, freq="1h", tz="UTC"
        )
        return pd.DataFrame(
            {
    ...
    def invalid_ohlc_data(self):
        ...
        dates = pd.date_range(
            start="2023-01-01 09:30:00", periods=5, freq="1h", tz="UTC"
        )
        return pd.DataFrame(
            {
        ...
        return pd.DataFrame(
            {
                "open": [
                    ...
                    108,
                    106,
                ],  # Duplicate values for duplicate timestamp
    ...
    def data_with_gaps(self):
        ...
        dates1 = pd.date_range(
            start="2023-01-01 09:30:00", periods=3, freq="1h", tz="UTC"
        )
        dates2 = pd.date_range(
            start="2023-01-01 14:30:00", periods=3, freq="1h", tz="UTC"
        )
    ...
    def test_clean_data_preserves_data_when_no_duplicates(
        self, processor, valid_ohlc_data
    ):
    ...
    def test_apply_transformations_timezone_conversion(
        self, processor, valid_ohlc_data
    ):
    ...
    def test_configuration_controls_behavior(
        self, valid_ohlc_data, data_with_duplicates
    ):
