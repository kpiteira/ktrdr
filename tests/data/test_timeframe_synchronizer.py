...
class TestTimeframeSynchronizer:
    ...
    def test_get_timeframe_relation(self):
        ...
        assert TimeframeSynchronizer.get_timeframe_relation("4h", "1h") == TimeframeRelation.HIGHER
        assert TimeframeSynchronizer.get_timeframe_relation("1d", "4h") == TimeframeRelation.HIGHER
        assert TimeframeSynchronizer.get_timeframe_relation("1w", "1d") == TimeframeRelation.HIGHER
        
        # Lower timeframe relations
        assert TimeframeSynchronizer.get_timeframe_relation("1h", "4h") == TimeframeRelation.LOWER
        assert TimeframeSynchronizer.get_timeframe_relation("4h", "1d") == TimeframeRelation.LOWER
        
        # Equal timeframes
        assert TimeframeSynchronizer.get_timeframe_relation("1h", "1h") == TimeframeRelation.EQUAL
        assert TimeframeSynchronizer.get_timeframe_relation("4h", "4h") == TimeframeRelation.EQUAL
        
        # Incomparable timeframes
        assert TimeframeSynchronizer.get_timeframe_relation("invalid", "1h") == TimeframeRelation.INCOMPARABLE
        assert TimeframeSynchronizer.get_timeframe_relation("1h", "invalid") == TimeframeRelation.INCOMPARABLE
    
    def test_forward_fill_alignment_basic(self, synchronizer, sample_1h_data, sample_4h_data):
    ...
    def test_synchronize_multiple_timeframes(self, synchronizer, sample_1h_data, sample_4h_data, sample_1d_data):
    ...
    def test_synchronize_multiple_timeframes_invalid_reference(self, synchronizer, sample_1h_data):
        """Test error when reference timeframe is not in data."""
        data_dict = {"1h": sample_1h_data}
        
        with pytest.raises(DataValidationError, match="Reference timeframe 4h not found"):
    ...
    def test_interpolate_missing_data(self, synchronizer):
        ...
        data = pd.DataFrame({
            'close': [1, 2, np.nan, np.nan, 5, 6, np.nan, 8, 9, 10]
        }, index=dates)
    ...
    def test_interpolate_missing_data_with_limit(self, synchronizer):
        ...
        dates = pd.date_range('2023-01-01', periods=10, freq="1h", tz='UTC')
        data = pd.DataFrame({
            'close': [1, 2, np.nan, np.nan, np.nan, np.nan, 7, 8, 9, 10]
        }, index=dates)
        
        # Limit to 2 consecutive interpolations
        interpolated = synchronizer.interpolate_missing_data(data, method="linear", limit=2)
    ...
    def test_validate_temporal_consistency_empty_data(self, synchronizer):
        ...
        small_data = pd.DataFrame({'close': [1]}, 
    ...
    def test_get_optimal_reference_timeframe_errors(self):
        ...
        with pytest.raises(ValueError, match="No supported timeframes found"):
            TimeframeSynchronizer.get_optimal_reference_timeframe(["invalid1", "invalid2"])
