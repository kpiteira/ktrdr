...
class TestIbClientIdRegistry:
    ...
    def registry(self, temp_registry_file):
        ...
        with patch.object(
            IbClientIdRegistry, "_get_persistence_file", return_value=temp_registry_file
        ):
    ...
    def test_basic_allocation_and_deallocation(self, registry):
        ...
        client_id = registry.allocate_client_id(
            purpose=ClientIdPurpose.API_POOL, allocated_by="test_component"
        )
    ...
    def test_purpose_based_allocation(self, registry):
        ...
        api_id = registry.allocate_client_id(
            purpose=ClientIdPurpose.API_POOL, allocated_by="api_component"
        )

        gap_filler_id = registry.allocate_client_id(
            purpose=ClientIdPurpose.GAP_FILLER, allocated_by="gap_component"
        )

        cli_id = registry.allocate_client_id(
            purpose=ClientIdPurpose.CLI_TEMPORARY, allocated_by="cli_component"
        )
    ...
    def test_allocations_by_purpose(self, registry):
        ...
        gap_allocations = registry.get_allocations_by_purpose(
    ...
    def test_invalid_purpose_handling(self, registry):
        ...
        with patch(
            "ktrdr.data.ib_client_id_registry.IbLimitsRegistry.CLIENT_ID_RANGES",
            {"api_pool": []},
        ):  # Empty range
...
class TestErrorHandling:
    ...
    def temp_registry_file(self, tmp_path):
        """Create a temporary file for registry persistence testing."""
