...
class TestLocalDataLoader:
    ...
    def test_init_raises_error_if_path_exists_but_not_directory(self, tmp_path):
        ...
        assert (
            "not a directory" in str(excinfo.value).lower()
            or "invalid path" in str(excinfo.value).lower()
        )
    ...
    def test_get_available_data_files(self, tmp_path):
        ...
        symbols_timeframes = [
            (symbol, timeframe) for symbol, timeframe in available_files
        ]
    ...
    def test_save_data_success(self, tmp_path, sample_ohlcv_data):
        ...
        try:
            ...
            assert (
                loaded_data.shape[0] == sample_ohlcv_data.shape[0]
            )  # Same number of rows
            assert set(loaded_data.columns) == set(
                sample_ohlcv_data.columns
            )  # Same columns
