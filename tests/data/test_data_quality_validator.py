...
class TestDataQualityValidator:
    ...
    def test_validate_good_data(self, validator, good_ohlcv_data):
        ...
        df_validated, report = validator.validate_data(good_ohlcv_data, 'MSFT', '1h', 'ib')
    ...
    def test_ohlc_relationship_validation_and_fixing(self, validator):
        ...
        assert df_validated.loc[dates[0], 'high'] == 104  # Corrected to max(open, close)
    ...
    def test_price_outlier_detection(self, validator):
        ...
        dates = pd.date_range('2023-01-01', periods=10, freq='1H')
        data = {
            'open': [100, 101, 102, 500, 104, 105, 106, 107, 108, 109],  # 500 is outlier
            'high': [105, 106, 107, 108, 109, 110, 111, 112, 113, 114],
            'low': [99, 100, 101, 102, 103, 104, 105, 106, 107, 108],
            'close': [104, 105, 106, 107, 108, 109, 110, 111, 112, 113],
            'volume': [1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800, 1900]
        ...
        open_outlier = [issue for issue in outlier_issues if issue.metadata.get('column') == 'open']
    ...
    def test_extreme_price_movement_detection(self, validator):
        ...
        data = {
            ...
            'high': [105, 106, 107, 108, 109],
            'low': [99, 100, 101, 102, 103],
            'close': [104, 105, 150, 107, 108],  # 43% jump from 105 to 150, then 29% drop to 107
            'volume': [1000, 1100, 1200, 1300, 1400]
        }
        df = pd.DataFrame(data, index=dates)
        ...
        assert extreme_issue.metadata['count'] == 2  # Both up and down moves are extreme
    ...
    def test_comprehensive_validation_workflow(self, validator):
        ...
        expected_types = {
            'duplicate_timestamps', 'non_positive_price', 'high_too_low', 
            'low_too_high', 'negative_volume', 'timestamp_gaps',
            'extreme_price_movement', 'zero_volume', 'extreme_volume_spike'
        }
