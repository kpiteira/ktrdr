...
class TestGapAnalyzerIntegration:
    ...
    def test_detect_gaps_behavior_preservation(self, sample_ohlcv_data):
        ...
        gaps_to_create = [2, 3, 5]  # Create gaps at these indices (0-based, max index is 6)
        ...
        assert len(detected_gaps) >= 0  # Could be 0 if gaps are classified as non-significant
    ...
    def test_detect_gaps_uses_gap_analyzer_component(self, sample_ohlcv_data):
        ...
        with patch.object(data_manager.gap_analyzer, 'detect_internal_gaps', 
                         return_value=[(datetime(2024, 1, 1, 12, 0, 0, tzinfo=timezone.utc),
                                      datetime(2024, 1, 1, 13, 0, 0, tzinfo=timezone.utc))]) as mock_gap_finder:
        ...
        expected_gaps = [(datetime(2024, 1, 1, 12, 0, 0, tzinfo=timezone.utc),
