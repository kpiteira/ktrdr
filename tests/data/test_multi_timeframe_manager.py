...
class TestTimeframeConfig:
    ...
    def test_config_validation_primary_in_auxiliary(self):
        ...
        with pytest.raises(ValueError, match="Primary timeframe cannot be in auxiliary"):
            TimeframeConfig(
                primary_timeframe="1h",
                auxiliary_timeframes=["1h", "4h"],
                periods=200
            )
...
class TestMultiTimeframeDataManager:
    ...
    def test_validate_multi_timeframe_config(self, manager):
        ...
        # Invalid symbol
        with pytest.raises(DataValidationError, match="Symbol must be a non-empty string"):
    ...
    @patch.object(MultiTimeframeDataManager, '_load_single_timeframe')
    def test_load_multi_timeframe_data_primary_failure(self, mock_load_single, manager, config):
    ...
    def test_synthetic_data_generation(
        ...
        if "1d" in result.data:
            ...
            assert all(col in synthetic_1d.columns for col in ['open', 'high', 'low', 'close', 'volume'])
    ...
    def test_minimum_timeframes_requirement(self, manager):
        ...
        with patch.object(manager, '_load_single_timeframe') as mock_load:
            with patch.object(manager, '_synchronize_available_timeframes') as mock_sync:
                # Mock only primary timeframe success
                def mock_load_side_effect(s, tf, p):
                    if tf == "1h":
                        return pd.DataFrame({'close': [100]}, index=pd.date_range('2023-01-01', periods=1, freq='1h', tz='UTC'))
                    else:
