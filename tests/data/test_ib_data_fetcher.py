...
class TestIbDataFetcher:
    ...
    async def test_fetch_chunk_success(self, fetcher, mock_connection):
        ...
        mock_bars = [
            Mock(spec=BarData, date=datetime(2024, 1, 1, 9, 30), 
                 open=100.0, high=101.0, low=99.0, close=100.5, volume=1000),
            Mock(spec=BarData, date=datetime(2024, 1, 1, 9, 31),
                 open=100.5, high=101.5, low=100.0, close=101.0, volume=1500),
        ]
    ...
    async def test_fetch_historical_data_integration(self, fetcher, mock_connection):
        ...
        def create_mock_bars(date):
            return [
                Mock(spec=BarData, date=date + timedelta(hours=i),
        ...
        with patch('ktrdr.data.ib_data_fetcher.util.df') as mock_df:
            ...
            start = datetime(2024, 1, 1, tzinfo=timezone.utc)
            end = datetime(2024, 3, 1, tzinfo=timezone.utc)  # 2 months = ~60 days, requires 2 chunks
