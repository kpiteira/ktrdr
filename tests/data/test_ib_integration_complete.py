...
def connection_config():
    ...
    return ConnectionConfig(host="127.0.0.1", port=7497, client_id=1, timeout=30)
...
class TestConnectionManagement:
    ...
    def test_connection_auto_reconnect(self, connection_config, mock_ib_client):
        ...
        with patch("ktrdr.data.ib_connection_sync.IB", return_value=mock_ib_client):
            ...
            mock_ib_client.isConnected.side_effect = [
                False,
                True,
...
class TestDataFetchingIntegration:
    ...
    def test_full_data_fetch_workflow(
        self, connection_config, mock_ib_client, sample_ohlcv_data_integration
    ):
        ...
        with patch("ktrdr.data.ib_connection_sync.IB", return_value=mock_ib_client):
            ...
            for i, (timestamp, row) in enumerate(
                sample_ohlcv_data_integration.head(10).iterrows()
            ):
                mock_bar = Mock()
...
class TestRangeDiscoveryIntegration:
    ...
    def test_range_discovery_with_head_timestamp(
        self, connection_config, mock_ib_client
    ):
        ...
        with patch("ktrdr.data.ib_connection_sync.IB", return_value=mock_ib_client):
            ...
            mock_ib_client.reqHeadTimeStamp.return_value = datetime(
...
class TestErrorRecoveryIntegration:
    ...
    def test_download_session_management(
        self, tmp_path, connection_config, mock_ib_client
    ):
    ...
    def test_resumable_download_workflow(
        self, tmp_path, connection_config, mock_ib_client
    ):
...
class TestDataQualityIntegration:
    ...
    def test_data_quality_with_outlier_detection(self, sample_ohlcv_data_integration):
        ...
        data.iloc[50, data.columns.get_loc("close")] = (
            data.iloc[50, data.columns.get_loc("close")] * 3
        )  # 300% spike
        ...
        outlier_issues = [
            issue for issue in quality_report.issues if "outlier" in issue.issue_type
        ]
...
class TestEndToEndWorkflows:
    ...
    def test_complete_data_manager_ib_workflow(
        self, ib_config, mock_ib_client, tmp_path
    ):
        """Test complete DataManager workflow with IB integration."""
        with (
            patch("ktrdr.data.data_manager.get_ib_config", return_value=ib_config),
            ...
            manager.data_loader.load = Mock(
                side_effect=DataNotFoundError("No local data")
            )
    ...
    def test_symbol_discovery_and_validation_workflow(
        self, connection_config, mock_ib_client
    ):
    ...
    def test_error_scenarios_and_recovery(
        self, connection_config, mock_ib_client, tmp_path
    ):
        ...
        with patch("ktrdr.data.ib_connection_sync.IB", return_value=mock_ib_client):
            ...
            mock_ib_client.isConnected.side_effect = [
                ...
                False,
                True,
            ]  # Connection drops then recovers
    ...
    def test_large_dataset_handling(self, connection_config, mock_ib_client):
        ...
        with patch("ktrdr.data.ib_connection_sync.IB", return_value=mock_ib_client):
            ...
            large_data["high"] = np.maximum(
                large_data["high"], np.maximum(large_data["open"], large_data["close"])
            )
            large_data["low"] = np.minimum(
                large_data["low"], np.minimum(large_data["open"], large_data["close"])
            )
    ...
    def test_concurrent_symbol_processing(self, connection_config, mock_ib_client):
        ...
        with patch("ktrdr.data.ib_connection_sync.IB", return_value=mock_ib_client):
            ...
            def mock_head_timestamp(contract, **kwargs):
                ...
                if hasattr(contract, "symbol"):
                    return symbol_dates.get(
                        contract.symbol, datetime(2020, 1, 1, tzinfo=timezone.utc)
                    )
                return datetime(2020, 1, 1, tzinfo=timezone.utc)
