...
class TestIbDataRangeDiscovery:
    ...
    def test_cache_validity(self, range_discovery):
        ...
        # Expired cache entry
        range_discovery.cache_timestamps["test_key"] = (
            time.time() - 90000
        )  # Over 24 hours
        assert range_discovery._is_cache_valid("test_key") is False
    ...
    def test_has_data_at_date_with_data(self, range_discovery, sample_dataframe):
        ...
        range_discovery.data_fetcher.fetch_historical_data.return_value = (
            sample_dataframe
        )
    ...
    def test_has_data_at_date_exception(self, range_discovery):
        ...
        range_discovery.data_fetcher.fetch_historical_data.side_effect = Exception(
            "API Error"
        )
    ...
    def test_has_data_at_date_date_tolerance(self, range_discovery):
        ...
        dates = pd.date_range(
            start=actual_data_date, end=actual_data_date + timedelta(days=5), freq="D"
        )
        ...
        dates = pd.date_range(
            start=far_data_date, end=far_data_date + timedelta(days=5), freq="D"
        )
        df = pd.DataFrame({"close": [100.0] * len(dates)}, index=dates)
        range_discovery.data_fetcher.fetch_historical_data.return_value = df
    ...
    def test_get_earliest_data_point_binary_search(self, range_discovery):
        ...
        sample_df = pd.DataFrame({"close": [100.0]}, index=[data_start_date])
        range_discovery.data_fetcher.fetch_historical_data.return_value = sample_df

        with patch.object(
            range_discovery, "_has_data_at_date", side_effect=mock_has_data_at_date
        ):
            result = range_discovery.get_earliest_data_point(
                symbol, timeframe, max_lookback_years=5
            )
    ...
    def test_get_earliest_data_point_refinement_error(self, range_discovery):
        ...
        with patch.object(
            ...
            result = range_discovery.get_earliest_data_point(
                symbol, timeframe, max_lookback_years=2
            )
    ...
    def test_get_data_range_with_discovery(self, range_discovery):
        ...
        with patch.object(
            range_discovery, "get_earliest_data_point", return_value=earliest_date
        ):
    ...
    def test_get_data_range_no_data(self, range_discovery):
        ...
        with patch.object(
            range_discovery, "get_earliest_data_point", return_value=None
        ):
    ...
    def test_get_multiple_ranges(self, range_discovery):
        ...
        with patch.object(
            range_discovery, "get_data_range", side_effect=mock_get_data_range
        ):
    ...
    def test_clear_cache(self, range_discovery):
        ...
        range_discovery._cache_range(
            "AAPL", "1 day", datetime(2020, 1, 1), datetime(2023, 1, 1)
        )
        range_discovery._cache_range(
            "MSFT", "1 hour", datetime(2021, 1, 1), datetime(2023, 1, 1)
        )
    ...
    def test_get_cache_stats(self, range_discovery):
        ...
        range_discovery._cache_range(
            "AAPL", "1 hour", datetime(2020, 1, 1), datetime(2023, 1, 1)
        )
        range_discovery._cache_range(
            "MSFT", "1 day", datetime(2021, 1, 1), datetime(2023, 1, 1)
        )
    ...
    def test_binary_search_iteration_limit(self, range_discovery):
        ...
        sample_df = pd.DataFrame({"close": [100.0]}, index=[datetime(2020, 1, 1)])
        range_discovery.data_fetcher.fetch_historical_data.return_value = sample_df
        ...
        with patch.object(
            ...
            result = range_discovery.get_earliest_data_point(
                symbol, timeframe, max_lookback_years=20
            )
