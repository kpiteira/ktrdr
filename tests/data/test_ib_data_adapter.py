...
class TestIbDataAdapter(unittest.TestCase):
    ...
    def test_adapter_initialization(self):
        ...
        # Should have connection pool and pace manager
        self.assertIsNotNone(adapter.connection_pool)
        self.assertIsNotNone(adapter.pace_manager)
    ...
    def test_convert_timeframe_to_ib(self):
        ...
        self.assertEqual(adapter._convert_timeframe_to_ib("1m"), "1 min")
        self.assertEqual(adapter._convert_timeframe_to_ib("5m"), "5 mins")
        self.assertEqual(adapter._convert_timeframe_to_ib("1h"), "1 hour")
        self.assertEqual(adapter._convert_timeframe_to_ib("1d"), "1 day")
        self.assertEqual(adapter._convert_timeframe_to_ib("1w"), "1 week")
        self.assertEqual(adapter._convert_timeframe_to_ib("1M"), "1 month")
        
        # Unknown timeframe should default to 1 day
        self.assertEqual(adapter._convert_timeframe_to_ib("unknown"), "1 day")
    ...
    def test_calculate_duration(self):
        ...
        self.assertEqual(adapter._calculate_duration(start, end), "3 D")
        
        # 2 weeks
        end = start + timedelta(weeks=2)
        self.assertEqual(adapter._calculate_duration(start, end), "2 W")
        
        # 2 years
        end = start + timedelta(days=730)  # ~2 years
        self.assertEqual(adapter._calculate_duration(start, end), "2 Y")
    ...
    def test_fetch_historical_data_success(self, mock_pace_manager_class, mock_pool_class):
    ...
    def test_validate_symbol_success(self, mock_pace_manager_class, mock_pool_class):
        ...
        async def run_test():
            ...
            mock_pace_manager.wait_if_needed.assert_called_once_with(is_historical=False)
            self.assertEqual(adapter.requests_made, 1)
    ...
    def test_validate_symbol_failure(self, mock_pace_manager_class, mock_pool_class):
        ...
        mock_pool = Mock()
        mock_pool.execute_with_connection = AsyncMock(side_effect=Exception("Symbol not found"))
        mock_pool_class.return_value = mock_pool
    ...
    def test_handle_ib_error_pacing_violation(self):
        ...
        with patch('ktrdr.data.ib_data_adapter.IbErrorClassifier.classify') as mock_classify:
            mock_classify.return_value = (IbErrorType.PACING_VIOLATION, 60.0)
            
            with self.assertRaises(DataProviderRateLimitError) as context:
                adapter._handle_ib_error(Exception("Rate limit exceeded"), "test_operation")
    ...
    def test_handle_ib_error_connection_error(self):
        ...
        with patch('ktrdr.data.ib_data_adapter.IbErrorClassifier.classify') as mock_classify:
            ...
            with self.assertRaises(DataProviderConnectionError) as context:
                adapter._handle_ib_error(Exception("Connection failed"), "test_operation")
    ...
    def test_handle_ib_error_data_unavailable(self):
        ...
        with patch('ktrdr.data.ib_data_adapter.IbErrorClassifier.classify') as mock_classify:
            ...
            with self.assertRaises(DataProviderDataError) as context:
                adapter._handle_ib_error(Exception("Data not available"), "test_operation")
