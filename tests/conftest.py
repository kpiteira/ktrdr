...
# Ensure project root is in path for imports (needed for research_agents package)
...
def disable_retries():
    ...
    with patch(
        "ktrdr.errors.retry.retry_with_backoff", lambda *args, **kwargs: lambda x: x
    ):
        yield
...
def sample_ohlcv_data():
    ...
    highs = [
        max(o, c) * (1 + abs(np.random.normal(0, 0.005))) for o, c in zip(opens, closes)
    ]

    # Low is min of open and close minus a random amount
    lows = [
        min(o, c) * (1 - abs(np.random.normal(0, 0.005))) for o, c in zip(opens, closes)
    ]

    # Volume is random but correlated with price movement
    volumes = [
        int(np.random.normal(1000000, 200000) * (1 + 5 * abs(c / o - 1)))
