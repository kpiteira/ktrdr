...
def create_test_data():
    ...
    close_prices = [
        100, 102, 104, 106, 108,  # 5 days up
        107, 106, 105, 104, 103,  # 5 days down
        102, 101, 100, 99, 98,    # 5 more days down
        99, 100, 101, 102, 103,   # 5 days up
        105, 107, 109, 111, 113,  # 5 more days up
        112, 111, 110, 109, 108   # 5 days down
    ]
...
class TestRSIIndicator:
    ...
    def test_compute_basic(self):
        ...
        df = pd.DataFrame({
            'close': [10, 11, 12, 11, 10, 9, 10, 11, 12, 13, 14, 13, 12, 11, 12, 13, 14, 15, 14, 13]
        })
    ...
    def test_rsi_directional_behavior(self):
        ...
        assert result.iloc[24] > result.iloc[19], "RSI should increase with continued uptrends"
        
        # Check that RSI decreases during downtrends
        # Position 25-29 is a downtrend, RSI should decrease
        assert result.iloc[29] < result.iloc[24], "RSI should decrease during downtrends"
        
        # Check that RSI values are in expected ranges
        # After strong uptrend, RSI should be overbought (>70)
        assert result.iloc[24] > 70, "RSI should be overbought (>70) after strong uptrend"
        
        # After downtrend, RSI should be lower than peak value
        assert result.iloc[29] < result.iloc[24] * 0.9, "RSI should decrease significantly after downtrend"
    ...
    def test_edge_cases(self):
        ...
        df_constant = pd.DataFrame({
            'close': [100] * 20  # 20 days of constant price
        })
