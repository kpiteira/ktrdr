...
class TestRVIIndicator:
        ...
        data = pd.DataFrame(
            {
                ...
                "close": [
                    100.8 + i for i in range(20)
                ],  # Consistently close higher than open
            }
        )
        ...
        data = pd.DataFrame(
            {
                ...
                "close": [
                    119.2 - i for i in range(20)
                ],  # Consistently close lower than open
            }
        )
    ...
    def test_rvi_oscillator_range(self):
        ...
        for i in range(20):
            if i % 2 == 0:  # Bullish candles
                ...
                data.append(
                    {"open": 100 + i, "high": 101 + i, "low": 98 + i, "close": 99.5 + i}
                )
    ...
    def test_weighted_calculation_logic(self):
        ...
        indicator = RVIIndicator(
            period=4, signal_period=2
        )  # Shorter periods for easier verification
        result = indicator.compute(data)
        ...
        if len(rvi_values) > 0:
            ...
            )  # All should be positive for consistent bullish pattern
    ...
    def test_signal_line_lag(self):
        ...
        for i in range(20):
            if i < 10:  # First half: bearish
                data.append(
                    {"open": 100 - i, "high": 101 - i, "low": 98 - i, "close": 98.5 - i}
                )
