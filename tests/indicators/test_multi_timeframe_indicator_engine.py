...
class TestMultiTimeframeIndicatorEngine:
        ...
        data_1h = pd.DataFrame(
            {
                ...
                "high": prices
                * (1 + np.abs(np.random.normal(0, 0.005, len(dates_1h)))),
    ...
    def test_compute_specific_indicator(
        self, sample_ohlcv_data, sample_timeframe_configs
    ):
    ...
    def test_create_cross_timeframe_features(
        self, sample_ohlcv_data, sample_timeframe_configs
    ):
...
class TestColumnStandardizer:
    ...
    def test_standardize_indicator_name(self, standardizer):
        ...
        result = standardizer.standardize_indicator_name("RSI", "1h", {"period": 14})
    ...
    def test_filter_columns_by_type(self, standardizer):
        ...
        standardizer.column_mapping = {
            ...
            "rsi_oversold_1h": ColumnInfo(
                "RSI_oversold", "rsi_oversold_1h", ColumnType.FUZZY, "1h"
            ),
            "buy_signal_1h": ColumnInfo(
                "BUY_signal", "buy_signal_1h", ColumnType.SIGNAL, "1h"
            ),
        }

        columns = ["rsi_14_1h", "rsi_oversold_1h", "buy_signal_1h"]

        indicator_cols = standardizer.filter_columns_by_type(
            columns, ColumnType.INDICATOR
        )
