...
from .validation_utils import validate_indicator_against_reference, generate_indicator_report
...
class TestAutomatedIndicatorValidation:
    ...
    def test_indicator_against_references(self, indicator_name):
        ...
        for dataset_name in registry_data['reference_datasets']:
            ...
            if dataset_name not in registry_data['reference_values']:
                logger.info(f"No reference values for {indicator_name} on {dataset_name}, skipping")
                continue
                
            # Create dataset and get reference values
            dataset_creator = DATASET_CREATORS.get(dataset_name)
            if not dataset_creator:
                logger.warning(f"Dataset creator for {dataset_name} not found, skipping")
                continue
            ...
            result = validate_indicator_against_reference(
                indicator, data, reference_values, 
                tolerance=tolerance, relative=True
            )
            
            # Log results
            logger.info(f"Validation {result.passed} for {indicator_name} on {dataset_name}: "
    ...
    def test_indicator_edge_cases(self, indicator_name):
        ...
        for case in edge_cases:
            if case.get('should_raise', False):
                ...
                logger.info(f"{indicator_name} correctly raised {case['error_type'].__name__} "
                ...
                try:
                    ...
                    if 'expected_values' in case:
                        for idx, expected in case['expected_values'].items():
                            if isinstance(expected, (int, float)) and not pd.isna(expected):
                                assert abs(result.iloc[idx] - expected) <= case.get('tolerance', 0.01), \
                                    f"Expected {expected} at position {idx}, got {result.iloc[idx]}"
                except Exception as e:
                    pytest.fail(f"{indicator_name} should not raise error for {case['name']}: {e}")
    ...
    def test_indicator_basic_properties(self, indicator_name):
        ...
        assert isinstance(result, pd.Series), f"{indicator_name} should return a pd.Series"
        assert len(result) == len(data), f"{indicator_name} output should have same length as input"
