...
class TestParabolicSARIndicator:
    ...
    def test_uptrend_behavior(self):
        ...
        for i in range(2, len(data)):
            if not pd.isna(result.iloc[i]):
                assert result.iloc[i] < data['low'].iloc[i], f"SAR should be below low at position {i}"
    ...
    def test_downtrend_behavior(self):
        ...
        for i in range(4, len(data)):
            if not pd.isna(result.iloc[i]):
                assert result.iloc[i] > data['high'].iloc[i], f"SAR should be above high at position {i}"
    ...
    def test_gap_up_gap_down(self):
        ...
        data = pd.DataFrame({
            'high': [101, 103, 105, 110, 112, 108, 105, 103],  # Gap up then gap down
