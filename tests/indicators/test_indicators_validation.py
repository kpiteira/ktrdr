...
class TestSMAValidation:
    ...
    def test_sma5_against_reference(self):
        ...
        assert result.passed, f"SMA(5) failed validation with {result.failed_points} points"
    ...
    def test_sma10_against_reference(self):
        ...
        # Assert all points pass validation
        assert result.passed, f"SMA(10) failed validation with {result.failed_points} points"
    ...
    def test_sma20_against_reference(self):
        ...
        # Assert all points pass validation
        assert result.passed, f"SMA(20) failed validation with {result.failed_points} points"
    ...
    def test_sma_various_parameters(self):
        ...
        for period in [5, 10, 20, 50]:
            ...
            if period > 1:
                ...
                assert result_std <= original_std * 1.05, f"SMA({period}) should reduce volatility"
                
            # Log report
            report = generate_indicator_report(sma, data, key_points=[period, period*2, -1])
...
class TestEMAValidation:
    ...
    def test_ema5_against_reference(self):
        ...
        # Assert all points pass validation
        assert result.passed, f"EMA(5) failed validation with {result.failed_points} points"
    ...
    def test_ema10_against_reference(self):
        ...
        # Assert all points pass validation
        assert result.passed, f"EMA(10) failed validation with {result.failed_points} points"
    ...
    def test_ema_adjusted_vs_non_adjusted(self):
        ...
        assert last_diff < first_diff, "Adjusted and non-adjusted EMA should converge over time"
...
class TestRSIValidation:
    ...
    def test_rsi14_against_reference(self):
        ...
        # Assert all points pass validation
        assert result.passed, f"RSI(14) failed validation with {result.failed_points} points"
    ...
    def test_rsi7_against_reference(self):
        ...
        # Assert all points pass validation
        assert result.passed, f"RSI(7) failed validation with {result.failed_points} points"
    
    def test_rsi_bounded_values(self):
        """Test that RSI values are always bounded between 0 and 100."""
        # Test with all datasets
        for dataset_func in [create_reference_dataset_1, create_reference_dataset_2, 
                           create_reference_dataset_3, create_reference_dataset_4]:
            data = dataset_func()
            ...
            for period in [7, 14]:
                ...
                report = generate_indicator_report(
                    rsi, data, 
                    key_points=[period, period*2, len(data)-1]
                )
...
class TestAllIndicatorsValidation:
    ...
    def test_all_indicators_base_behavior(self):
        ...
        for indicator in indicators:
            ...
            assert isinstance(result, pd.Series), f"{indicator.name} should return a pd.Series"
            
            # 2. Output has same length as input
            assert len(result) == len(data), f"{indicator.name} output should have same length as input"
        ...
        for test in tests:
            for indicator in indicators:
                if test['should_raise']:
                    ...
                    logger.info(f"{indicator.name} correctly raised {test['error_type'].__name__} "
                               f"for {test['name']}: {str(excinfo.value)}")
                else:
                    try:
                        result = indicator.compute(test['data'])
                        logger.info(f"{indicator.name} handled {test['name']} correctly")
                    except Exception as e:
                        pytest.fail(f"{indicator.name} should not raise error for {test['name']}: {e}")
