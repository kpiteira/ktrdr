...
class TestWilliamsRIndicator:
    ...
    def test_williams_r_basic_computation(self):
        ...
        data = pd.DataFrame({
            ...
            'low': [100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114],
    ...
    def test_williams_r_with_falling_data(self):
        ...
        data = pd.DataFrame({
            'high': [116, 115, 114, 113, 112, 111, 110, 109, 108, 107, 106, 105, 104, 103, 102],
            'low': [114, 113, 112, 111, 110, 109, 108, 107, 106, 105, 104, 103, 102, 101, 100],
    ...
    def test_williams_r_overbought_oversold_levels(self):
        ...
        data = pd.DataFrame({
            'high': [100, 105, 110, 115, 120, 125, 130, 135, 140, 145, 150, 155, 160, 165, 170],
            'low': [95, 100, 105, 110, 115, 120, 125, 130, 135, 140, 145, 150, 155, 160, 165],
            'close': [99, 104, 109, 114, 119, 124, 129, 134, 139, 144, 149, 154, 159, 164, 169],  # Close near high
        })
    ...
    def test_williams_r_insufficient_data(self):
        ...
        wr = WilliamsRIndicator()
        with pytest.raises(DataError, match="Williams %R requires at least 14 data points"):
            wr.compute(data)

        # Test with custom period
        wr = WilliamsRIndicator(period=5)
        with pytest.raises(DataError, match="Williams %R requires at least 5 data points"):
            wr.compute(data)
