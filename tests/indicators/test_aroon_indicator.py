...
class TestAroonIndicator:
    ...
    def test_basic_calculation(self):
        ...
        data = pd.DataFrame({
            'open': [100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116],
            'high': [101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117],
            'low': [99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115],
            'close': [100.5, 101.5, 102.5, 103.5, 104.5, 105.5, 106.5, 107.5, 108.5, 109.5, 110.5, 111.5, 112.5, 113.5, 114.5, 115.5, 116.5]
        })
    ...
    def test_downtrend_aroon_behavior(self):
        ...
        if len(valid_up) > 0 and len(valid_down) > 0:
            ...
            assert (recent_down > 70).any(), "Aroon Down should be high during downtrend"
    ...
    def test_consolidation_aroon_behavior(self):
        ...
        if len(valid_up) > 0 and len(valid_down) > 0:
            ...
            assert 20 <= avg_up <= 80, "Aroon Up should be moderate during consolidation"
            assert 20 <= avg_down <= 80, "Aroon Down should be moderate during consolidation"

    def test_aroon_oscillator_calculation(self):
        """Test Aroon Oscillator calculation (Up - Down)."""
        data = pd.DataFrame({
            'high': [100, 102, 101, 103, 102, 104, 103, 105, 104, 106, 105, 107, 106, 108, 107],
            'low': [98, 100, 99, 101, 100, 102, 101, 103, 102, 104, 103, 105, 104, 106, 105]
        })
        ...
        valid_indices = ~(pd.isna(result["Aroon_10_Up"]) | pd.isna(result["Aroon_10_Down"]))
        
        for idx in result.index[valid_indices]:
            expected_oscillator = result["Aroon_10_Up"][idx] - result["Aroon_10_Down"][idx]
    ...
    def test_extreme_values(self):
        ...
        data = pd.DataFrame({
            'high': [100, 1000, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114],
            'low': [99, 1, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113]
        })
    ...
    def test_nan_values_in_data(self):
        ...
        data = pd.DataFrame({
            'high': [101, np.nan, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115],
            'low': [99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113]
        })
    ...
    def test_aroon_boundary_values(self):
        ...
        data = pd.DataFrame({
            'high': [100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 120],  # New high at end
            'low': [99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 119]
        })
    ...
    def test_aroon_time_calculation(self):
        ...
        data = pd.DataFrame({
            'high': [100, 110, 102, 103, 104, 105, 106, 107, 108, 109],  # High at position 9 (last)
            'low': [99, 109, 95, 102, 103, 104, 105, 106, 107, 108]      # Low at position 2
        })
    ...
    def test_aroon_equal_highs_lows(self):
        ...
        data = pd.DataFrame({
            'high': [100, 100, 100, 101, 101, 101, 102, 102, 102, 103, 103, 103, 104, 104, 104],
            'low': [99, 99, 99, 100, 100, 100, 101, 101, 101, 102, 102, 102, 103, 103, 103]
        })
