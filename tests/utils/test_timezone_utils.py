...
from ktrdr.utils.timezone_utils import TimestampManager, ensure_utc_timestamp, format_exchange_time
...
class TestTimestampManager:
    ...
    def test_convert_dataframe_index_with_aware_index(self):
        ...
        dates = pd.date_range('2025-01-07 15:00:00', periods=3, freq='1h', tz='America/New_York')
        df = pd.DataFrame({'value': [1, 2, 3]}, index=dates)
    ...
    def test_to_utc_series(self):
        ...
        dates_est = pd.date_range('2025-01-07 15:00:00', periods=3, freq='1h', tz='America/New_York')
    ...
    def test_validate_timezone_consistency_no_datetime_index(self):
        ...
        df = pd.DataFrame({'value': [1, 2, 3]}, index=[0, 1, 2])
        with pytest.raises(ValueError, match="test_operation: DataFrame must have a DatetimeIndex"):
            TimestampManager.validate_timezone_consistency(df, "test_operation")
    ...
    def test_validate_timezone_consistency_naive_index(self):
        ...
        dates = pd.date_range('2025-06-07 15:00:00', periods=3, freq='1h')
        df = pd.DataFrame({'value': [1, 2, 3]}, index=dates)
        with pytest.raises(ValueError, match="test_operation: DataFrame has timezone-naive index"):
            TimestampManager.validate_timezone_consistency(df, "test_operation")

    def test_validate_timezone_consistency_non_utc(self):
        """Test timezone validation with non-UTC timezone."""
        dates = pd.date_range('2025-06-07 15:00:00', periods=3, freq='1h', tz='America/New_York')
        df = pd.DataFrame({'value': [1, 2, 3]}, index=dates)
        with pytest.raises(ValueError, match="test_operation: DataFrame has non-UTC timezone"):
            TimestampManager.validate_timezone_consistency(df, "test_operation")
