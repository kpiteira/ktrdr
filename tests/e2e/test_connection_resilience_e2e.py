...
class TestConnectionResilienceE2E:
    ...
    def test_connection_pool_metrics(self, api_client):
        ...
        if response.status_code == 200:
            ...
            assert pool_health["healthy_connections"] <= pool_health["total_connections"]
    ...
    def test_resilience_score_calculation(self, api_client):
        ...
        if response.status_code == 200:
            ...
            working_phases = sum(1 for phase in [phase1, phase2, phase3] 
                               if phase.get("status") == "working")
            
            if working_phases == 3:
                # All phases working should give high score (65+ points minimum)
                assert score >= 65, f"Expected high score for all working phases, got {score}"
    ...
    def test_resilience_performance(self, api_client):
        ...
        assert response_time < 10.0, f"Resilience endpoint too slow: {response_time:.2f}s"
        
        # Should be reasonably fast for status check
        if response.status_code == 200:
            assert response_time < 5.0, f"Expected faster response: {response_time:.2f}s"
