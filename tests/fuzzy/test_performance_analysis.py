...
class TestFuzzyPerformanceAnalyzer:
    ...
    def test_benchmark_pipeline_performance(self, mock_service, sample_test_configurations):
    ...
    def test_average_metrics(self):
        ...
        metrics_list = [
            PerformanceMetrics(total_time=1.0, fuzzy_values_count=10, memory_peak_mb=100.0),
            PerformanceMetrics(total_time=2.0, fuzzy_values_count=20, memory_peak_mb=120.0),
            PerformanceMetrics(total_time=1.5, fuzzy_values_count=15, memory_peak_mb=110.0)
        ]
    ...
    def test_generate_performance_report(self, mock_service, sample_test_configurations):
    ...
    def test_generate_performance_report_with_file(self, mock_service, sample_test_configurations):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert saved_report["summary"]["total_benchmarks"] == report["summary"]["total_benchmarks"]
    ...
    def test_scalability_analysis_with_failures(self, mock_service):
        ...
        if "error" not in scalability_result:
            assert len(scalability_result["data_points"]) >= 2  # At least first two should succeed