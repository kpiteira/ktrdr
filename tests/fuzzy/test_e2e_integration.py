...
from ktrdr.data.manager import DataManager
...
class TestEnd2EndFuzzyIntegration:
    ...
    def reference_fuzzy_config(self):
        ...
        return {
            "timeframes": {
                "1h": {
                    ...
                    "fuzzy_sets": {
                        ...
                        "macd": {
                            "bearish": {"type": "triangular", "parameters": [-2, -1, 0]},
                            "neutral": {"type": "triangular", "parameters": [-0.5, 0, 0.5]},
                ...
                "4h": {
                    ...
                    "fuzzy_sets": {
                        "rsi": {
                            "low": {"type": "trapezoidal", "parameters": [0, 10, 30, 40]},
                            "medium": {"type": "trapezoidal", "parameters": [30, 40, 60, 70]},
                            "high": {"type": "trapezoidal", "parameters": [60, 70, 90, 100]}
                        },
                        "ema": {
                        ...
                        "bb": {
                            ...
                            "middle": {"type": "triangular", "parameters": [-0.5, 0, 0.5]},
                ...
                "1d": {
                    ...
                    "fuzzy_sets": {
                        ...
                        "sma_200": {
                            "long_bearish": {"type": "triangular", "parameters": [-20, -10, 0]},
                            "long_neutral": {"type": "triangular", "parameters": [-5, 0, 5]},
                            "long_bullish": {"type": "triangular", "parameters": [0, 10, 20]}
                        },
                        "atr": {
                            "low_volatility": {"type": "triangular", "parameters": [0, 1, 2]},
                            "medium_volatility": {"type": "triangular", "parameters": [1.5, 3, 4.5]},
                            "high_volatility": {"type": "triangular", "parameters": [4, 6, 10]}
                        }
                    },
                    "weight": 0.2,
                    "enabled": True
            ...
            "indicators": ["rsi", "macd", "sma", "ema", "bb", "sma_50", "sma_200", "atr"]
        }
    ...
    def test_end_to_end_single_symbol_processing(self, reference_indicator_config, 
    ...
    def test_end_to_end_multiple_timeframes(self, reference_indicator_config,
                                          reference_fuzzy_config,
                                          mock_data_manager_with_reference_data):
    ...
    def test_end_to_end_fuzzy_value_consistency(self, reference_indicator_config,
                                              reference_fuzzy_config,
                                              mock_data_manager_with_reference_data):
        ...
        for timeframe in ["1h", "4h"]:
            if timeframe in indicator_data and "rsi" in indicator_data[timeframe]:
                ...
                rsi_fuzzy_keys = [k for k in fuzzy_values.keys() if f"rsi_" in k and f"_{timeframe}" in k]
                
                if rsi_fuzzy_keys:
                    # Validate RSI fuzzy logic
                    if rsi_value < 30:
                        # Should have higher membership in oversold/low
                        low_keys = [k for k in rsi_fuzzy_keys if "oversold" in k or "low" in k]
                        if low_keys:
                        ...
                        high_keys = [k for k in rsi_fuzzy_keys if "overbought" in k or "high" in k]
                        if high_keys:
                            assert max(fuzzy_values[k] for k in high_keys) > 0.1, \
                                f"High RSI ({rsi_value}) should have higher overbought membership"
    
    def test_end_to_end_error_recovery(self, reference_indicator_config,
                                     reference_fuzzy_config):
        ...
        service = FuzzyPipelineService(
            data_manager=PartialDataManager()
        )
    ...
    def test_end_to_end_performance_characteristics(self, reference_indicator_config,
                                                  reference_fuzzy_config,
                                                  mock_data_manager_with_reference_data):
    ...
    def test_end_to_end_multiple_symbols(self, reference_indicator_config,
                                       reference_fuzzy_config,
                                       mock_data_manager_with_reference_data):
    ...
    def test_end_to_end_summary_report(self, reference_indicator_config,
                                     reference_fuzzy_config,
                                     mock_data_manager_with_reference_data):
    ...
    def test_end_to_end_with_config_files(self, reference_indicator_config,
                                        reference_fuzzy_config,
                                        mock_data_manager_with_reference_data):
...
class TestFuzzyValueValidation:
    ...
    def test_rsi_oversold_pattern(self):
        ...
        if oversold_keys:
            ...
            assert oversold_value > 0.0, "Strong price decline should produce some RSI oversold membership"
    ...
    def test_membership_function_types_integration(self):
        ...
        assert len(rsi_keys) > 0, "Should have RSI fuzzy values from trapezoidal functions"
        assert len(macd_keys) > 0, "Should have MACD fuzzy values from gaussian functions"
