...
def sample_fuzzy_config():
    ...
    config_dict = {
        ...
        "macd": {
            "negative": {
                "type": "triangular",
                "parameters": [-10.0, -10.0, 0.0]
            },
            "positive": {
                "type": "triangular",
                "parameters": [0.0, 10.0, 10.0]
            }
        }
    }
...
class TestBatchFuzzyCalculator:
    ...
    def test_calculate_memberships_with_nan_values(self, batch_calculator, sample_timestamps):
    ...
    def test_calculate_memberships_empty_series(self, batch_calculator, sample_timestamps):
        """Test calculation with empty series."""
        empty_series = pd.Series([], dtype=float, index=pd.Index([], dtype='datetime64[ns]'))
    ...
    def test_calculate_memberships_all_nan_series(self, batch_calculator, sample_timestamps):
        """Test calculation with series containing only NaN values."""
        nan_series = pd.Series([np.nan] * len(sample_timestamps), index=sample_timestamps)
    ...
    def test_unknown_indicator_error(self, batch_calculator, sample_rsi_series):
        ...
        with pytest.raises(ProcessingError) as exc_info:
            batch_calculator.calculate_memberships("unknown_indicator", sample_rsi_series)
    ...
    def test_invalid_value_type(self, batch_calculator):
        ...
        with pytest.raises(ProcessingError) as exc_info:
            batch_calculator.calculate_memberships("rsi", [1, 2, 3])  # list instead of Series
