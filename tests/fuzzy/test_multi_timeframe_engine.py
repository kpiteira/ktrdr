...
class TestMultiTimeframeFuzzyEngine:
        ...
        return {
            "timeframes": {
                ...
                "1d": {
                    ...
                    "fuzzy_sets": {
                        "trend": {
                            "downtrend": {"type": "gaussian", "parameters": [-1, 0.3]},
                            "sideways": {"type": "gaussian", "parameters": [0, 0.2]},
                            "uptrend": {"type": "gaussian", "parameters": [1, 0.3]},
                        }
                    },
                    "weight": 0.2,
                    "enabled": True,
        ...
        return FuzzyConfig(
            {
                "rsi": FuzzySetConfig(
                    {
                        "low": TriangularMFConfig(
                            type="triangular", parameters=[0, 20, 40]
                        ),
                        "high": TriangularMFConfig(
                            type="triangular", parameters=[60, 80, 100]
                        ),
                    }
                )
            }
    ...
    def test_single_timeframe_backward_compatibility(
        self, sample_single_timeframe_config
    ):
    ...
    def test_invalid_configuration(self):
        ...
        with pytest.raises(ConfigurationError) as exc_info:
            MultiTimeframeFuzzyEngine(
                {
                    "timeframes": {"1h": {"indicators": ["rsi"]}},  # Missing fuzzy_sets
                    "indicators": ["rsi"],
                }
            )
        ...
        with pytest.raises(ConfigurationError) as exc_info:
            MultiTimeframeFuzzyEngine(
                {
                    "timeframes": {
                        "1h": {"indicators": [], "fuzzy_sets": {}}  # Empty indicators
                    },
                    "indicators": ["rsi"],
                }
            )
    ...
    def test_fuzzify_multi_timeframe_basic(
        self, sample_multi_timeframe_config, sample_indicator_data
    ):
    ...
    def test_fuzzify_with_timeframe_filter(
        self, sample_multi_timeframe_config, sample_indicator_data
    ):
        ...
        result = engine.fuzzify_multi_timeframe(
            sample_indicator_data, timeframe_filter=["1h"]
        )
    ...
    def test_invalid_indicator_data(self, sample_multi_timeframe_config):
        ...
        with pytest.raises(DataValidationError) as exc_info:
            engine.fuzzify_multi_timeframe({"1h": {"rsi": "not a number"}})
        assert "Invalid value" in str(exc_info.value)

        # NaN value
        with pytest.raises(DataValidationError) as exc_info:
            engine.fuzzify_multi_timeframe({"1h": {"rsi": np.nan}})
        assert "Invalid value" in str(exc_info.value)

    def test_disabled_timeframe(
        self, sample_multi_timeframe_config, sample_indicator_data
    ):
    ...
    def test_missing_indicator_data_graceful_handling(
        self, sample_multi_timeframe_config
    ):
        ...
        assert (
            any("macd" in key for key in result.timeframe_results["1h"].keys()) is False
        )
    ...
    def test_membership_function_types(self, sample_multi_timeframe_config):
        ...
        assert len(result.timeframe_results) == 3
        assert all(
            len(tf_result) > 0 for tf_result in result.timeframe_results.values()
        )
...
class TestTimeframeConfig:
    ...
    def test_timeframe_config_defaults(self):
        ...
        config = TimeframeConfig(timeframe="4h", indicators=["sma"], fuzzy_sets={})
