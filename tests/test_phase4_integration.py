...
class TestPhase4Integration:
    ...
    async def training_host_client(self):
        ...
        async with httpx.AsyncClient(base_url="http://localhost:5002", timeout=30.0) as client:
    ...
    async def ib_host_client(self):
        ...
        async with httpx.AsyncClient(base_url="http://localhost:5001", timeout=30.0) as client:
            # Verify service is running
            try:
                response = await client.get("/health")
                if response.status_code != 200:
