...
class TestAsyncDataLoaderUnifiedCancellation:
    ...
    def mock_data_manager(self):
        ...
        manager = Mock()
        manager.load_data = Mock(return_value=pd.DataFrame({
            'open': [100, 101, 102],
            'high': [101, 102, 103], 
            'low': [99, 100, 101],
    ...
    def test_data_loading_job_should_not_use_asyncio_event(self):
        ...
        assert legacy_pattern_removed, "DataLoadingJob still uses legacy asyncio.Event pattern"
        
        # Should have unified cancellation token instead
        assert hasattr(job, '_cancellation_token'), "Should have unified cancellation token"
        assert hasattr(job, 'cancellation_token'), "Should have cancellation_token property"
        
    def test_async_data_loader_should_use_unified_cancellation_token(self, async_data_loader):
    ...
    @pytest.mark.asyncio
    async def test_job_execution_with_unified_cancellation_token(self, async_data_loader, mock_data_manager):
    ...
    @pytest.mark.asyncio
    async def test_job_cancellation_through_unified_token(self, async_data_loader, mock_data_manager):
        ...
        async def slow_load_data(*args, **kwargs):
            ...
            return pd.DataFrame({'open': [100], 'high': [101], 'low': [99], 'close': [100.5], 'volume': [1000]})
