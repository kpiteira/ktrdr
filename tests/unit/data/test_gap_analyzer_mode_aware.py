...
class TestGapInfo:
    ...
    def __init__(self, start_time: datetime, end_time: datetime, gap_type: str, priority: int = 1):
...
class TestModeAwareGapAnalysis:
    ...
    def test_classify_gap_type_missing_data(self, gap_analyzer):
        ...
        trading_start = datetime(2023, 1, 11, 14, 0, tzinfo=timezone.utc)  # Wednesday 2pm
    ...
    def test_classify_gap_type_holiday(self, gap_analyzer):
        ...
        # New Year's Day gap
        holiday_start = datetime(2023, 12, 31, 16, 0, tzinfo=timezone.utc)  # Dec 31 close
        ...
        with patch('pandas.tseries.holiday.USFederalHolidayCalendar') as mock_calendar:
            mock_calendar.return_value.holidays.return_value = pd.DatetimeIndex(['2023-01-01'])
    ...
    def test_prioritize_gaps_by_mode_tail(self, gap_analyzer):
        ...
        gaps = [
            TestGapInfo(datetime(2023, 1, 1), datetime(2023, 1, 2), "missing_data", 1),  # Old
            TestGapInfo(datetime(2023, 1, 8), datetime(2023, 1, 9), "missing_data", 1),  # Recent
            TestGapInfo(datetime(2023, 1, 5), datetime(2023, 1, 6), "missing_data", 1),  # Middle
        ]
        ...
        assert prioritized[0].start_time > prioritized[1].start_time > prioritized[2].start_time
    ...
    def test_prioritize_gaps_by_mode_backfill(self, gap_analyzer):
        ...
        prioritized = gap_analyzer.prioritize_gaps_by_mode(gaps, DataLoadingMode.BACKFILL)
    ...
    def test_prioritize_gaps_by_mode_full(self, gap_analyzer):
        ...
        gaps = [
            TestGapInfo(datetime(2023, 1, 1), datetime(2023, 1, 2), "missing_data", 2),  # Old, high priority
            TestGapInfo(datetime(2023, 1, 8), datetime(2023, 1, 9), "missing_data", 1),  # Recent, low priority
            TestGapInfo(datetime(2023, 1, 5), datetime(2023, 1, 6), "market_closure", 1),  # Middle, market closure
        ]
        ...
        assert prioritized[0].gap_type == "missing_data" and prioritized[0].priority == 2
    ...
    def test_analyze_gaps_by_mode_backfill(self, gap_analyzer, sample_data):
        ...
        with patch.object(gap_analyzer, '_analyze_historical_gaps') as mock_historical:
            mock_historical.return_value = [(datetime(2022, 12, 20), datetime(2023, 1, 1))]
    ...
    def test_progress_manager_integration(self, gap_analyzer, progress_manager, sample_data):
    ...
    def test_mode_specific_step_descriptions(self, gap_analyzer, progress_manager):
        ...
        for mode in DataLoadingMode:
            ...
            expected_descriptions = {
                ...
                DataLoadingMode.BACKFILL: ["Analyzing historical gaps", "Prioritizing backfill data"],
                DataLoadingMode.FULL: ["Analyzing complete range", "Combining strategies", "Prioritizing all gaps"]
            }
