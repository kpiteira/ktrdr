...
class TestIbAdapterAsyncHostServiceIntegration(unittest.TestCase):
    ...
    def test_adapter_inherits_from_async_host_service(self):
        ...
        adapter = IbDataAdapter(use_host_service=True, host_service_url=self.host_service_url)
    ...
    def test_adapter_uses_inherited_http_methods_for_host_service(self):
        ...
        async def test_http_methods():
            async with adapter:  # Use async context manager
                ...
                with patch.object(adapter, '_call_host_service_post', new=AsyncMock()) as mock_post:
                ...
                with patch.object(adapter, '_call_host_service_get', new=AsyncMock()) as mock_get:
                    mock_get.return_value = {"healthy": True, "ib_status": {"connected": True}}
    ...
    def test_existing_api_compatibility_preserved(self):
        ...
        async def test_compatibility():
            async with adapter:
                ...
                with patch.object(adapter, '_call_host_service_post', new=AsyncMock()) as mock_post, \
                     patch.object(adapter, '_call_host_service_get', new=AsyncMock()) as mock_get:
    ...
    def test_error_handling_preserved(self):
        ...
        async def test_error_handling():
            async with adapter:
                with patch.object(AsyncHostService, '_call_host_service_post', new=AsyncMock()) as mock_post:
                    # Connection errors should still map to DataProviderConnectionError
                    from ktrdr.managers.async_host_service import HostServiceConnectionError
                    mock_post.side_effect = HostServiceConnectionError("Connection failed", "IB Data Service")
                    ...
                    from ktrdr.managers.async_host_service import HostServiceTimeoutError
                    mock_post.side_effect = HostServiceTimeoutError("Timeout", "IB Data Service")
                    
                    with self.assertRaises(DataProviderConnectionError):  # Should be translated
                        await adapter.validate_and_get_metadata("AAPL", ["1h"])
