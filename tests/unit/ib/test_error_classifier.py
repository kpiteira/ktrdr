...
class TestIbErrorClassifier(unittest.TestCase):
    ...
    def test_permission_errors(self):
        ...
        error_type, wait_time = IbErrorClassifier.classify(
            10197, "No market data permissions"
        )
        self.assertEqual(error_type, IbErrorType.PERMISSION_ERROR)
        self.assertEqual(wait_time, 0.0)
    ...
    def test_permission_keyword_detection(self):
        ...
        error_type, wait_time = IbErrorClassifier.classify(
            999, "market data not subscribed"
        )
        self.assertEqual(error_type, IbErrorType.PERMISSION_ERROR)
        self.assertEqual(wait_time, 0.0)
    ...
    def test_historical_pacing_vs_unavailable(self):
        ...
        error_type, wait_time = IbErrorClassifier.classify(
            999, "historical data pacing violation"
        )
        self.assertEqual(error_type, IbErrorType.PACING_VIOLATION)
        self.assertEqual(wait_time, 60.0)
        ...
        error_type, wait_time = IbErrorClassifier.classify(
            888, "historical data not available"
        )
        self.assertEqual(error_type, IbErrorType.DATA_UNAVAILABLE)
        self.assertEqual(wait_time, 0.0)
    ...
    def test_client_id_conflict_detection(self):
        ...
        # Direct error code
        self.assertTrue(
            IbErrorClassifier.is_client_id_conflict(
    ...
    def test_retry_delay_calculation(self):
        ...
        self.assertEqual(
            IbErrorClassifier.get_retry_delay(IbErrorType.PERMISSION_ERROR), 0.0
        )

        # Pacing violations - fixed delay (no exponential backoff)
        self.assertEqual(
            IbErrorClassifier.get_retry_delay(IbErrorType.PACING_VIOLATION, 1), 60.0
        )
        self.assertEqual(
            IbErrorClassifier.get_retry_delay(IbErrorType.PACING_VIOLATION, 3), 60.0
        )

        # Connection errors - exponential backoff with cap
        self.assertEqual(
            IbErrorClassifier.get_retry_delay(IbErrorType.CONNECTION_ERROR, 1), 2.0
        )
        self.assertEqual(
            IbErrorClassifier.get_retry_delay(IbErrorType.CONNECTION_ERROR, 2), 4.0
        )
        self.assertEqual(
            IbErrorClassifier.get_retry_delay(IbErrorType.CONNECTION_ERROR, 3), 8.0
        )
        # Should cap at 60 seconds
        self.assertEqual(
            IbErrorClassifier.get_retry_delay(IbErrorType.CONNECTION_ERROR, 10), 60.0
        )

        # Data unavailable - no delay
        self.assertEqual(
            IbErrorClassifier.get_retry_delay(IbErrorType.DATA_UNAVAILABLE, 1), 0.0
        )
