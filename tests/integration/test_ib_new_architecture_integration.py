...
class TestIbNewArchitectureIntegration:
    ...
    def connection_pool(self, ib_config):
        ...
        pool = IbConnectionPool(host=ib_config.host, port=ib_config.port, max_connections=2)
        yield pool
    ...
    def data_adapter(self, ib_config):
        ...
        adapter = IbDataAdapter(host=ib_config.host, port=ib_config.port, max_connections=2)
        yield adapter
    ...
    async def test_connection_persistence_across_operations(self, connection_pool):
        ...
        assert len(connection_ids) <= 2, f"Too many connections created: {connection_ids}"
    ...
    async def test_no_handler_closed_errors(self, connection_pool):
        ...
        assert len(errors_caught) == 0, f"Handler closed errors detected: {errors_caught}"
    ...
    async def test_error_classifier_accuracy(self):
        ...
        assert error_type.value == "data_unavail", "162 should be data unavailable, not pacing"
