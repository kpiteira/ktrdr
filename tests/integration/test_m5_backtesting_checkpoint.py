...
class IntegrationCheckpointService:
    """Checkpoint service with in-memory repository for backtesting tests.

    Unlike training, backtesting has NO artifacts - all state fits in DB JSONB.
    """
    ...
    async def save_checkpoint(
        ...
        """Save checkpoint (no artifacts for backtesting)."""
        import json
        ...
        # Save to in-memory repository (no artifacts for backtesting)
        await self._mock_repo.save(
            ...
            state_size_bytes=state_size_bytes,
            artifacts_size_bytes=None,
        )

    async def load_checkpoint(
        self,
        operation_id: str,
        load_artifacts: bool = False,  # Default False for backtesting
    ) -> Optional[CheckpointData]:
        """Load checkpoint from in-memory repo."""
    ...
    async def delete_checkpoint(self, operation_id: str) -> bool:
        """Delete checkpoint from in-memory repo."""
