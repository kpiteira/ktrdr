...
class TestRichProgressContext:
    """Test rich progress context preservation through operations API."""

    @pytest.mark.asyncio
    async def test_training_operation_rich_progress_context(self):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            start_response = Mock()
            start_response.json = Mock(
                return_value={"task_id": "train-rich-context-123"}
            )
            start_response.status_code = 200
            start_response.raise_for_status = Mock()
            mock_client.post.return_value = start_response

            # Mock polling responses with rich context
            poll_responses = [
                Mock(
                    json=Mock(
                        return_value={
                            "success": True,
                            "data": {
                                "operation_id": "train-rich-context-123",
                                "status": "running",
                                "progress": {
                                    "percentage": 30,
                                    "current_step": "Epoch 3/10",
            ...
            success = await executor.execute_operation(
                ...
                show_progress=True,  # Enable progress to test callback invocation
            )

            # Assert
            assert success is True

            # Verify rich context was preserved and passed to callback
            assert len(progress_messages) >= 1, "Progress callback should be invoked"

            # Verify first progress update has rich context
            first_context = progress_messages[0]
            assert "epoch_index" in first_context, "Context should contain epoch_index"
            assert first_context["epoch_index"] == 3
            assert first_context["total_epochs"] == 10
            assert first_context["batch_number"] == 50
            assert first_context["batch_total_per_epoch"] == 100

            # Verify GPU context preserved
            resource_usage = first_context.get("resource_usage", {})
            assert resource_usage.get("gpu_used") is True
            assert resource_usage.get("gpu_name") == "RTX 3090"
            assert resource_usage.get("gpu_utilization_percent") == 85.5
