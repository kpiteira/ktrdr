...
class TestMultiTimeframeIndicatorPipelineIntegration:
    ...
    def comprehensive_ohlcv_data(self):
        ...
        data_1h = pd.DataFrame(
            {
                ...
                "high": prices
                * (1 + np.abs(np.random.normal(0, noise_factor * 2, n_points))),
                "low": prices
                * (1 - np.abs(np.random.normal(0, noise_factor * 2, n_points))),
                "close": prices,
                "volume": np.random.lognormal(9, 0.5, n_points).astype(
                    int
                ),  # Realistic volume
            }
        )
        ...
        data_1h["high"] = np.maximum(
            data_1h["high"], np.maximum(data_1h["open"], data_1h["close"])
        )
        data_1h["low"] = np.minimum(
            data_1h["low"], np.minimum(data_1h["open"], data_1h["close"])
        )

        # Create 4h data by resampling
        data_4h = (
            data_1h.set_index("timestamp")
            .resample("4h")
            .agg(
                {
                    "open": "first",
    ...
    def test_end_to_end_multi_timeframe_indicator_pipeline(
        ...
        engine = create_multi_timeframe_engine_from_config(
            comprehensive_indicator_config
        )
        ...
        for timeframe, df in indicator_results.items():
            ...
            for col in indicator_columns:
                assert col.endswith(
                    f"_{timeframe}"
                ), f"Column {col} missing timeframe suffix"
        ...
        for timeframe, df in indicator_results.items():
            ...
            rsi_cols = [
                col
                for col in df.columns
                if col.startswith("rsi_") or col.startswith("RSI")
            ]
        ...
        for timeframe, df in indicator_results.items():
            ...
            for col in df.columns:
                if "sma_fast" in col.lower() or ("sma" in col.lower() and "10" in col):
                    sma_fast_col = col
                elif "sma_slow" in col.lower() or (
                    "sma" in col.lower() and "20" in col
                ):
                    sma_slow_col = col
            ...
            if sma_fast_col and sma_slow_col:
                # Calculate SMA cross
                sma_cross = (
                    (df[sma_fast_col] - df[sma_slow_col]) / df[sma_slow_col]
                ).fillna(0)

                # Test fuzzy processing on cross values
                for i, cross_val in enumerate(
                    sma_cross.tail(10)
                ):  # Test last 10 values
            ...
            for tf_config in mt_config.timeframes:
                timeframe_configs.append(
                    TimeframeIndicatorConfig(
                        ...
                        indicators=[
                            ind.model_dump() for ind in tf_config.indicators
    ...
    def test_cross_timeframe_feature_integration(self, comprehensive_ohlcv_data):
        ...
        timeframe_configs = [
            TimeframeIndicatorConfig(
                timeframe="1h", indicators=[{"type": "RSI", "params": {"period": 14}}]
            ),
            TimeframeIndicatorConfig(
                timeframe="4h", indicators=[{"type": "RSI", "params": {"period": 14}}]
            ),
        ]
    ...
    def test_performance_with_large_dataset(self):
        ...
        timeframe_configs = [
            ...
            TimeframeIndicatorConfig(
                timeframe="1d",
                indicators=[{"type": "SimpleMovingAverage", "params": {"period": 20}}],
            ),
        ]
        ...
        print(
            f"Processed {n_points} data points across 3 timeframes in {processing_time:.2f}s"
        )
        ...
        try:
            ...
            if "1h" in results:
                sma_cols = [
                    ...
                    if "sma" in col.lower() or "simplemovingaverage" in col.lower()
                ]
                if sma_cols:
    ...
    def test_integration_with_data_manager(self, comprehensive_ohlcv_data):
        ...
        timeframe_configs = [
            ...
            TimeframeIndicatorConfig(
                timeframe="1d", indicators=[{"type": "RSI", "params": {"period": 14}}]
            ),
        ]
        ...
        for timeframe in ["1h", "4h", "1d"]:
            ...
            rsi_cols = [
                col
                for col in results[timeframe].columns
