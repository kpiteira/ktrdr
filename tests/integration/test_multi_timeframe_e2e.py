...
class TestMultiTimeframeEndToEnd:
            ...
            "fuzzy_rules": [
                {
                    ...
                    "conditions": [
                        ...
                        {"indicator": "sma_position", "set": "below", "timeframe": "1d"}
                    ],
                ...
                },
                {
                    ...
                    "conditions": [
                        ...
                        {"indicator": "sma_position", "set": "above", "timeframe": "1d"}
                    ],
    ...
    async def test_complete_pipeline_data_to_decision(
        ...
        with patch('ktrdr.decision.multi_timeframe_orchestrator.DataManager') as mock_dm:
            ...
            assert decision.current_position in [Position.FLAT, Position.LONG, Position.SHORT]
    ...
    async def test_pipeline_with_missing_data(self, temp_strategy_file):
        ...
        partial_data = {
            '1h': pd.DataFrame({
                ...
                'close': [151], 'volume': [1000000]
            }).set_index('timestamp'),
            '4h': pd.DataFrame({
                'timestamp': [base_time],
                'open': [150], 'high': [153], 'low': [147], 
                'close': [151.5], 'volume': [4000000]
            }).set_index('timestamp')
            # '1d' missing
        }
    ...
    async def test_consensus_building_with_conflicting_signals(
        ...
        with patch('ktrdr.decision.multi_timeframe_orchestrator.DataManager') as mock_dm:
            ...
            with patch('ktrdr.decision.multi_timeframe_orchestrator.DecisionOrchestrator') as mock_base:
                mock_dm_instance = Mock()
    ...
    @pytest.mark.asyncio
    async def test_performance_metrics_collection(self, sample_data, temp_strategy_file):
    ...
    @pytest.mark.asyncio 
    async def test_concurrent_decision_generation(self, sample_data, temp_strategy_file):
