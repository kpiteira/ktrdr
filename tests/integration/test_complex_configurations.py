...
class TestComplexConfigurationHandler:
    ...
    def small_data(self):
        ...
        data_4h = data_1h.iloc[::4].reset_index(drop=True)  # Every 4th hour = 7-8 points
        
        # Even smaller daily data
        data_1d = data_1h.iloc[::24].reset_index(drop=True)  # Every 24th hour = 1-2 points
        ...
        return [
            TimeframeIndicatorConfig(
                ...
                indicators=[
                    ...
                    {'type': 'SimpleMovingAverage', 'params': {'period': 50}},  # Needs 50 points
            ...
            TimeframeIndicatorConfig(
                timeframe='4h',
                indicators=[
                    {'type': 'SimpleMovingAverage', 'params': {'period': 200}},  # Impossible with 7 points
            ...
            TimeframeIndicatorConfig(
                timeframe='1d',
                indicators=[
                    {'type': 'StochasticOscillator', 'params': {'k_period': 14, 'd_period': 3}},
    ...
    def test_configuration_validation_with_insufficient_data(self, small_data, demanding_configuration):
        """Test configuration validation when data is insufficient."""
        handler = ComplexConfigurationHandler()
        
        availability = handler.analyze_data_availability(small_data)
        issues, corrected_configs = handler.validate_configuration(demanding_configuration, availability)
        ...
        assert 'SimpleMovingAverage' in issue_types  # SMA(200) impossible with small data
    ...
    def test_fallback_strategy_skip(self, small_data, demanding_configuration):
        ...
        for config in corrected_configs:
            if config.timeframe == '4h':
                ...
                assert len(config.indicators) <= len(demanding_configuration[1].indicators)
    ...
    def test_fallback_strategy_reduce_period(self, small_data, demanding_configuration):
        ...
        for config in corrected_configs:
            for indicator in config.indicators:
                ...
                if indicator['type'] == 'SimpleMovingAverage' and config.timeframe == '4h':
                    period = params.get('period', 20)
    ...
    def test_create_robust_configuration(self, small_data, demanding_configuration):
        ...
        engine, issues = create_robust_configuration(
            ...
            small_data,
            FallbackStrategy.REDUCE_PERIOD
        )
    ...
    def test_validate_configuration_feasibility(self, small_data, demanding_configuration):
    ...
    def test_minimum_data_requirements(self, demanding_configuration):
        ...
        requirements = handler.suggest_minimum_data_requirements(demanding_configuration)
    ...
    def test_complex_real_world_scenario(self):
        ...
        complex_config = [
            ...
            TimeframeIndicatorConfig(
                ...
                indicators=[
                    {'type': 'RSI', 'params': {'period': 14}},
                    {'type': 'SimpleMovingAverage', 'params': {'period': 100}},  # May need reduction
            ...
            TimeframeIndicatorConfig(
                timeframe='1d',
                indicators=[
                    {'type': 'SimpleMovingAverage', 'params': {'period': 50}},  # Will need reduction
        ...
        for strategy in [FallbackStrategy.REDUCE_PERIOD, FallbackStrategy.SKIP]:
            engine, issues = create_robust_configuration(complex_config, complex_data, strategy)
    ...
    def test_edge_cases(self):
        ...
        simple_config = [
            TimeframeIndicatorConfig(
                timeframe='1h',
                indicators=[
                    {'type': 'SimpleMovingAverage', 'params': {'period': 40}}  # Needs reduction
                ]
            )
        ]
    ...
    def test_unknown_indicators(self):
        ...
        rsi_indicator = next(ind for ind in corrected[0].indicators if ind['type'] == 'RSI')
