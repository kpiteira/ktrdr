...
class TestApiDataLoading:
    ...
    def test_ib_service_load_data_tail_mode(self, test_data_manager):
        ...
        request = IbLoadRequest(
            symbol=TEST_SYMBOL, timeframe=TEST_TIMEFRAME, mode="tail"
        )
    ...
    def test_ib_service_load_data_full_mode(self, test_data_manager):
        ...
        request = IbLoadRequest(symbol=ALT_SYMBOL, timeframe=ALT_TIMEFRAME, mode="full")
    ...
    def test_ib_service_progressive_loading(self, test_data_manager):
        ...
        if response.status == "success":
            print(
                f"✅ Progressive loading: {response.requests_made} requests, {response.fetched_bars} bars"
            )
            assert response.requests_made >= 1, "Should have made at least one request"
        else:
            print(
                f"ℹ️  Progressive loading failed (might be expected): {response.error_message}"
            )
...
class TestApiEndpoints:
    ...
    def test_ib_load_endpoint_tail_mode(self):
        ...
        payload = {"symbol": TEST_SYMBOL, "timeframe": TEST_TIMEFRAME, "mode": "tail"}
        ...
        if data["success"]:
            ...
            print(
                f"ℹ️  API tail mode failed: {data.get('error', {}).get('message', 'Unknown')}"
            )

    def test_ib_load_endpoint_full_mode(self):
        """Test POST /api/v1/ib/load endpoint with full mode."""
        url = "http://localhost:8000/api/v1/ib/load"
        payload = {"symbol": ALT_SYMBOL, "timeframe": ALT_TIMEFRAME, "mode": "full"}
        ...
        if data["success"]:
            ...
            print(
                f"❌ API full mode failed: {data.get('error', {}).get('message', 'Unknown')}"
            )
    ...
    def test_datamanager_ib_fallback(self, test_data_manager):
        ...
        try:
            ...
            if result is not None and not result.empty:
                ...
                print(
                    "ℹ️  DataManager IB fallback: No data returned (might be expected)"
                )
...
class TestRangeDiscovery:
    ...
    def test_range_discovery_single_symbol(self):
        ...
        try:
            ranges_response = ib_service.get_data_ranges(
            ...
            if range_info:
                print(
                    f"✅ Range discovery: {TEST_SYMBOL} from {range_info.earliest_date} to {range_info.latest_date}"
                )
    ...
    def test_range_discovery_multiple_symbols(self):
        ...
        try:
            ...
            for symbol_data in ranges_response.symbols:
                print(
                    f"✅ Range discovery: {symbol_data.symbol} has {len(symbol_data.ranges)} timeframes"
                )
...
class TestConnectionIsolation:
    ...
    def test_connection_cleanup(self):
        ...
        ), f"Too many new connections created: {connection_increase}"

        print(
            f"✅ Connection reuse: {connection_increase} new connections for 3 requests"
        )
