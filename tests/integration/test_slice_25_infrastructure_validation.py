...
class TestSlice25InfrastructureValidation:
            ...
            if py_file.is_file():
                try:
                    ...
                    for line_num, line in enumerate(lines, 1):
                        ...
                        if 'hasattr(' in line and any(pattern in line_lower for pattern in ['cancel', 'is_set']):
    ...
    def test_unified_cancellation_token_usage(self, ktrdr_root: Path):
        ...
        if base_py.exists():
            ...
            lines = content.split('\n')
            for line_num, line in enumerate(lines, 1):
                if 'hasattr(' in line and any(pattern in line.lower() for pattern in ['cancel', 'is_set']):
                    pytest.fail(f"ServiceOrchestrator in {base_py} still contains hasattr() cancellation patterns at line {line_num}: {line.strip()}")

            # Should contain unified cancellation checking
            assert 'token.is_cancelled()' in content, \
        ...
        if async_host_py.exists():
            ...
            for line_num, line in enumerate(lines, 1):
                if 'hasattr(' in line and any(pattern in line.lower() for pattern in ['cancel', 'is_set']):
                    pytest.fail(f"AsyncHostService in {async_host_py} still contains hasattr() cancellation patterns at line {line_num}: {line.strip()}")
    ...
    def test_progress_manager_file_size_reduction(self, ktrdr_root: Path):
        ...
        assert not progress_manager_py.exists(), \
    ...
    def test_time_estimation_engine_location(self, ktrdr_root: Path):
        ...
        if old_location.exists():
            ...
            assert 'class TimeEstimationEngine(' not in old_content, \
        ...
        for py_file in cli_dir.rglob("*.py"):
            if py_file.is_file():
                try:
                    ...
                    if 'from ktrdr.data.components.progress_manager import' in content and 'ProgressState' in content:
                        problematic_imports.append(str(py_file.relative_to(ktrdr_root)))

                    # Should import GenericProgressState
                    if 'ProgressState' in content and 'GenericProgressState' not in content:
                        # Check if it's actually using progress state functionality
                        if any(pattern in content for pattern in ['progress_callback', 'progress_state', 'ProgressDisplay']):
                            problematic_imports.append(str(py_file.relative_to(ktrdr_root)))
    ...
    def test_multi_timeframe_coordinator_migration(self, ktrdr_root: Path):
        ...
        assert 'from ktrdr.data.components.progress_manager import ProgressManager' not in content, \
            "MultiTimeframeCoordinator should not import old ProgressManager"

        # Should import GenericProgressManager
        assert 'GenericProgressManager' in content, \
        ...
        for py_file in ktrdr_root.rglob("*.py"):
            if py_file.is_file():
                try:
                    ...
                    lines = content.split('\n')
                    for line_num, line in enumerate(lines, 1):
                        if 'ProgressManager(' in line and 'GenericProgressManager(' not in line:
                            # Skip the actual progress_manager.py file (may have deprecation examples)
                            if 'progress_manager.py' not in str(py_file):
                                problematic_files.append(f"{py_file.relative_to(ktrdr_root)}:{line_num}")
            ...
            if py_file.is_file():
                try:
                    ...
                    if 'threading.Event' in content:
                        threading_event_files.append(str(py_file.relative_to(ktrdr_root)))
    ...
    def test_import_paths_validation(self, ktrdr_root: Path):
        ...
        for py_file in ktrdr_root.rglob("*.py"):
            if py_file.is_file() and 'progress_manager.py' not in str(py_file):  # Skip the deprecated file itself
                try:
                    content = py_file.read_text()
    ...
    def test_complete_test_suite_passes(self):
        ...
        if result.returncode != 0:
            pytest.fail(f"Test suite failed after infrastructure cleanup:\n{result.stdout}\n{result.stderr}")
    ...
    def test_quality_checks_pass(self):
        ...
        if result.returncode != 0:
            pytest.fail(f"Quality checks failed after infrastructure cleanup:\n{result.stdout}\n{result.stderr}")
    ...
    def test_architecture_documentation_updated(self, ktrdr_root: Path):
        ...
        arch_spec = ktrdr_root.parent / "docs" / "architecture" / "async" / "async-architecture-spec.md"

        if arch_spec.exists():
            content = arch_spec.read_text()

            # Should document unified patterns
            assert 'CancellationToken' in content, \
                "Architecture documentation should reference unified CancellationToken protocol"

            assert 'GenericProgressManager' in content or 'ServiceOrchestrator' in content, \
