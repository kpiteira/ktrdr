...
class IbClientIdRegistry:
    ...
    def __init__(self):
        ...
        logger.info(
            f"IbClientIdRegistry initialized with {len(self._allocations)} existing allocations"
        )
    ...
    def _load_state(self) -> None:
        ...
        try:
            ...
            if stale_count > 0:
                logger.info(
                    f"Cleaned up {stale_count} stale client ID allocations on startup"
                )
            ...
            logger.info(
                f"Loaded {len(self._allocations)} client ID allocations in {load_time:.3f}s"
            )
    ...
    def _save_state(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Saved client ID registry state with {len(self._allocations)} allocations"
            )
        ...
        with self._allocation_lock:
            ...
            if preferred_id is not None:
                if preferred_id in available_ids:
                    ...
                    logger.info(
                        f"Allocated preferred client ID {client_id} for {purpose.value}"
                    )
                else:
                    logger.warning(
                        f"Preferred client ID {preferred_id} not available for {purpose.value}"
                    )
                    if preferred_id in self._allocations:
                        existing = self._allocations[preferred_id]
                        logger.warning(
                            f"Client ID {preferred_id} already allocated to {existing.allocated_by} for {existing.purpose.value}"
                        )
            ...
            logger.info(
                f"✅ Allocated client ID {client_id} to {allocated_by} for {purpose.value}"
            )
            return client_id
    ...
    def deallocate_client_id(self, client_id: int, deallocated_by: str) -> bool:
        ...
        with self._allocation_lock:
            if client_id not in self._allocations:
                logger.warning(
                    f"Attempted to deallocate unknown client ID {client_id} by {deallocated_by}"
                )
                return False
            ...
            logger.info(
                f"🧹 Deallocated client ID {client_id} (was allocated to {allocation.allocated_by}) by {deallocated_by}"
            )
            return True
    ...
    def get_allocations_by_purpose(
        self, purpose: ClientIdPurpose
    ) -> List[ClientIdAllocation]:
        """
        ...
        with self._allocation_lock:
            ...
            return [
                self._allocations[cid] for cid in client_ids if cid in self._allocations
            ]

    def get_allocations(self) -> Dict[int, Dict[str, Any]]:
        """
        Get all current allocations.

        Returns:
            Dictionary mapping client_id to allocation info
        """
        with self._allocation_lock:
            return {
                client_id: {
                    "purpose": allocation.purpose.value,
                    "allocated_by": allocation.allocated_by,
                    "allocated_at": allocation.allocated_at,
                    "last_seen": allocation.last_seen,
                    "is_active": allocation.is_active,
                    "connection_count": allocation.connection_count,
                }
                for client_id, allocation in self._allocations.items()
            }
    ...
    def _get_available_ids_for_purpose(self, purpose: ClientIdPurpose) -> Set[int]:
        ...
        try:
            ...
            logger.error(
                f"Error getting available IDs for purpose {purpose.value}: {e}"
            )
            return set()
    ...
    def _cleanup_stale_allocations(self) -> int:
        ...
        for client_id in stale_ids:
            ...
            self._purpose_allocations[allocation.purpose].discard(client_id)
            logger.info(
                f"🧹 Cleaned up stale client ID {client_id} (allocated to {allocation.allocated_by})"
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._allocation_lock:
            ...
            if self._allocations:
                ages = [
                    current_time - a.allocated_at for a in self._allocations.values()
                ]
            ...
            return {
                ...
                "active_allocations": len(
                    [a for a in self._allocations.values() if a.is_active]
                ),
                "inactive_allocations": len(
                    [a for a in self._allocations.values() if not a.is_active]
                ),
    ...
    def force_cleanup(self) -> Dict[str, int]:
        ...
        with self._allocation_lock:
            ...
            for client_id in ids_to_remove:
                ...
                self._purpose_allocations[allocation.purpose].discard(client_id)
                logger.info(
                    f"🧹 Force cleanup: removed client ID {client_id} (allocated to {allocation.allocated_by})"
                )
...
def allocate_client_id(
    purpose: ClientIdPurpose, allocated_by: str, preferred_id: Optional[int] = None
) -> Optional[int]:
    """Convenience function to allocate a client ID."""
    return get_client_id_registry().allocate_client_id(
        purpose, allocated_by, preferred_id
    )
