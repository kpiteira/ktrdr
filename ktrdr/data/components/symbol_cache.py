...
class SymbolCache:
    ...
    def __init__(self, cache_file: str = "data/backend_symbol_cache.json"):
        ...
        self._load_cache()
        logger.info(
            f"💾 SymbolCache initialized with {len(self._cache)} cached symbols"
        )

    def get(self, symbol: str) -> CachedSymbolInfo | None:
    ...
    def store(self, symbol: str, validation_result: ValidationResult) -> None:
        ...
        info = CachedSymbolInfo(
            validation_result=serialized_result, cached_at=time.time()
        )
    ...
    def _load_cache(self) -> None:
        ...
        if not self._cache_file.exists():
            logger.debug(
                f"💾 Cache file {self._cache_file} does not exist, starting fresh"
            )
            return
