...
from ktrdr.data.data_quality_validator import DataQualityValidator, DataQualityReport
...
class DataProcessor:
    ...
    def process_raw_data(
        ...
        """
        with self._lock:
            logger.debug(f"Processing raw data for {symbol} {timeframe} ({len(raw_data)} rows)")
            ...
            logger.debug(f"Processing complete: {len(raw_data)} -> {len(transformed_data)} rows")
    ...
    def validate_data_integrity(self, data: pd.DataFrame) -> ValidationResult:
        ...
        try:
            ...
            if quality_report.corrections_made > 0:
                ...
                for issue in quality_report.issues:
                    if issue.corrected and ('high' in issue.description or 'OHLC' in issue.description.upper()):
    ...
    def clean_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.config.remove_duplicates and not cleaned_data.index.is_unique:
            ...
            if len(cleaned_data) < original_len:
                logger.debug(f"Removed {original_len - len(cleaned_data)} duplicate rows")
        ...
        if self.config.auto_correct:
            ...
            try:
                ...
                if self.config.remove_duplicates or len(validated_data) == len(cleaned_data):
