...
class IbContextManager:
    ...
    def _fetch_direct_sync(
        ...
        context = self._detect_context()
        logger.info(
            f"🔄 Direct sync fetch for {symbol} {timeframe} (context: {context})"
        )
        ...
        try:
            ...
            unique_client_id = 200 + (
                thread_id % 100
            )  # Range 200-299 for background threads
            ...
            logger.info(
                f"📡 Creating thread-specific IB connection for {symbol} (client_id={unique_client_id})"
            )
            ...
            if temp_connection.is_connected():
                ...
                if data is not None and not data.empty:
                    logger.info(
                        f"✅ Successfully fetched {len(data)} bars for {symbol} {timeframe}"
                    )
                    return data
                else:
                ...
                logger.error(
                    f"Failed to create thread-specific connection for {symbol}"
                )
                return None

        except Exception as e:
            logger.error(
                f"❌ Error with thread-specific connection for {symbol} {timeframe}: {e}"
            )
            return None
    ...
    def _fetch_sync_in_async_context(
        ...
        try:
            ...
            return self._call_fetcher_sync_only(
                fetcher, symbol, timeframe, start_date, end_date
            )
    ...
    def _fetch_sync_in_thread_context(
        ...
        try:
            logger.info(
                f"🔄 Starting IB data fetch for {symbol} {timeframe} in background thread"
            )
            ...
            logger.info(
                f"📡 Requesting data from IB for {symbol} {timeframe} ({start_date} to {end_date})"
            )
            ...
            try:
                from ktrdr.data.ib_connection_sync import (
                    IbConnectionSync,
                    ConnectionConfig,
                )
                from ktrdr.data.ib_data_fetcher_sync import IbDataFetcherSync
                ...
                temp_config = ConnectionConfig(
                    host=self.connection.config.host,
                    port=self.connection.config.port,
                    client_id=self.connection.config.client_id
                ...
                logger.debug(
                    f"Creating temporary IB connection for {symbol} with client ID {temp_config.client_id}"
                )
                temp_connection = IbConnectionSync(temp_config)
                ...
                logger.error(
                    f"IB operation failed for {symbol} {timeframe}: {ib_error}"
                )
                return None

        except Exception as e:
            logger.error(
                f"❌ Error in background thread fetch for {symbol} {timeframe}: {e}"
            )
            return None
    ...
    def _call_fetcher_sync_only(
        ...
        try:
            ...
            if thread.is_alive():
                logger.error(
                    f"Fetch operation timed out for {symbol} {timeframe} after {timeout_seconds}s"
                )
                return None
