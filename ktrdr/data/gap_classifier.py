...
class GapClassifier:
    ...
    def classify_gap(self, start_time: datetime, end_time: datetime, 
                    symbol: str, timeframe: str) -> GapClassification:
        """
    ...
    def _spans_weekend(self, start_time: datetime, end_time: datetime, 
        ...
        trading_days = trading_hours.get('trading_days', [0, 1, 2, 3, 4])  # Mon-Fri default
        timezone_str = trading_hours.get('timezone', 'UTC')
        regular_hours = trading_hours.get('regular_hours', {})
        
        try:
            # Convert to exchange timezone
            start_local = start_time.astimezone(pd.Timestamp(start_time).tz_convert(timezone_str).tz)
            end_local = end_time.astimezone(pd.Timestamp(end_time).tz_convert(timezone_str).tz)
    ...
    def _is_forex_weekend_gap(self, start_local: pd.Timestamp, end_local: pd.Timestamp, 
                             regular_hours: Dict) -> bool:
        """
    ...
    def _is_holiday_gap(self, start_time: datetime, end_time: datetime,
                       trading_hours: Dict) -> bool:
        """
    ...
    def _is_holiday_date(self, date_obj: datetime.date) -> bool:
        ...
        if month == 7 and day == 4:
            return True
        if month == 7 and day == 3 and date_obj.weekday() == 4:  # Friday before if July 4 is Saturday
            return True
        if month == 7 and day == 5 and date_obj.weekday() == 0:  # Monday after if July 4 is Sunday
            return True
        ...
        try:
            ...
            if hasattr(start_time, 'tz_convert'):
                ...
                if start_time.tzinfo is None:
                    ...
                    start_local = pd.Timestamp(start_time, tz='UTC').tz_convert(timezone_str)
                    end_local = pd.Timestamp(end_time, tz='UTC').tz_convert(timezone_str)
                else:
            ...
            # Debug logging
            logger.debug(f"Checking trading hours: {start_local} to {end_local} in {timezone_str}")
            ...
            logger.warning(f"Error checking trading hours: {e}, falling back to default")
            return self._is_outside_default_hours(start_time, end_time)
    
    def _is_outside_default_hours(self, start_time: datetime, end_time: datetime) -> bool:
        """Default trading hours check (9:30 AM - 4:00 PM EST)."""
        # Simple check: if gap is during weekend, consider it outside hours
        return self._spans_default_weekend(start_time, end_time)
    
    def _is_time_range_outside_session(self, start_local: pd.Timestamp, 
                                      end_local: pd.Timestamp,
                                      session_start: str, session_end: str) -> bool:
        """
        ...
        try:
            ...
            session_start_time = pd.Timestamp.now().replace(
                hour=start_hour, minute=start_min, second=0, microsecond=0
            ).time()
            session_end_time = pd.Timestamp.now().replace(
            ...
            if start_local.date() == end_local.date():
                ...
                if session_start_time <= session_end_time:
                    ...
                    gap_outside = (gap_start_time >= session_end_time and 
                                 gap_end_time <= session_start_time)
    ...
    def _is_adjacent_to_weekend_gap(self, start_time: datetime, end_time: datetime,
                                   trading_hours: Dict) -> bool:
        """
        ...
        try:
            ...
            if (before_gap.weekday() in weekend_days or 
                after_gap.weekday() in weekend_days):
                return True
    ...
    def _calculate_bars_missing(self, start_time: datetime, end_time: datetime,
                               timeframe: str) -> int:
        """
    ...
    def is_gap_worth_filling(self, gap_info: GapInfo, 
                            priority_threshold: GapClassification = GapClassification.UNEXPECTED) -> bool:
        """
