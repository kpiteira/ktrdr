...
class MultiTimeframeDataManager(DataManager):
    ...
    def load_multi_timeframe_data(
        ...
        try:
            logger.info(f"Loading primary timeframe {config.primary_timeframe} for {symbol}")
            ...
            logger.info(f"Successfully loaded primary timeframe {config.primary_timeframe}")
        ...
        if failed_timeframes and config.enable_synthetic_generation:
            ...
            failed_timeframes = [tf for tf in failed_timeframes if tf not in synthetic_data]
    ...
    def _load_single_timeframe(
            ...
            try:
                ...
                try:
                    ...
                    if len(data) >= 10:  # Minimum viable data
                        ...
                        logger.warning(f"Insufficient local data for {symbol} {timeframe}")
                ...
                if self.enable_ib and hasattr(self, 'ib_fetcher'):
                    logger.info(f"Fetching {symbol} {timeframe} from IB (attempt {attempt + 1})")
                    ...
                    raise DataError(f"No data source available for {symbol} {timeframe}")
        ...
        raise DataError(f"Failed to load {symbol} {timeframe} after {max_retries} attempts")
    ...
    def _synthesize_higher_timeframe(
        ...
        # Resample the data
        resampled = source_data.resample(resample_rule).agg({
            'open': 'first',
            'high': 'max',
            'low': 'min',
            'close': 'last',
        ...
        logger.debug(f"Synthesized {len(resampled)} {target_timeframe} bars from source data")
