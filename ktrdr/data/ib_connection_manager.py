...
class PersistentIbConnectionManager:
    ...
    def _attempt_connection(self) -> None:
        ...
        if (
            ...
            and datetime.now(timezone.utc) < self.status.next_retry_time
        ):
            return
        ...
        try:
            ...
            if new_connection.is_connected():
                # Success!
                logger.info(
                    f"🔗 Assigning successful connection with client ID {client_id}"
                )
                ...
                logger.info(
                    f"✅ Successfully connected to IB with client ID {client_id}"
                )
            else:
                # Connection failed - explicitly clean up to prevent connection leaks
                logger.warning(
                    f"🧹 Connection failed for client ID {client_id}, cleaning up..."
                )
                try:
                    new_connection.disconnect()
                except Exception as cleanup_error:
                    logger.debug(
                        f"Error during failed connection cleanup: {cleanup_error}"
                    )
            ...
            if "new_connection" in locals():
                logger.debug(
                    f"🧹 Cleaning up failed connection after exception for client ID {client_id}"
                )
                try:
                    new_connection.disconnect()
                except Exception:
            ...
            if (
                "already in use" in error_msg.lower()
    ...
    def _check_connection_health(self) -> None:
        ...
        try:
            ...
            if not self.connection.is_connected():
                logger.warning(
                    "Connection lost (basic check), will attempt reconnection"
                )
            ...
            try:
                ...
                if hasattr(ib, "client") and hasattr(ib.client, "conn"):
                    ...
                    if not conn:
                        logger.warning(
                            "🔌 No client connection object, marking as lost"
                        )
                        self._mark_as_disconnected("No connection object")
                        return
                    elif hasattr(conn, "is_closing") and conn.is_closing():
                        logger.warning(
                            "🔌 Transport is closing, marking connection as lost"
                        )
                        self._mark_as_disconnected("Transport closing")
                        return
                    else:
                        logger.debug(
                            f"🔌 Transport check OK: conn={conn}, is_closing={getattr(conn, 'is_closing', 'N/A')}"
                        )
                else:
    ...
    def _schedule_retry(self, delay_seconds: int) -> None:
        ...
        self.status.next_retry_time = datetime.now(timezone.utc) + timedelta(
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "uptime_seconds": (
                (
                    datetime.now(timezone.utc) - self.status.last_connect_time
                ).total_seconds()
