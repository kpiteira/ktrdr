...
class IbErrorHandler:
    ...
    def _classify_error_162(self, error_message: str, use_context: bool) -> IbErrorInfo:
        ...
        if use_context and self.last_request_context:
            ...
            if symbol and start_date:
                head_timestamp_result = self._check_against_head_timestamp(symbol, start_date)
    ...
    def _check_against_head_timestamp(self, symbol: str, start_date: datetime) -> Optional[IbErrorInfo]:
        """
                ...
                if symbol in cached_symbols:
                    ...
                    if head_timestamp_str:
                        ...
                        if start_date < head_timestamp:
                            ...
                            logger.warning(f"📅 ERROR 162 CLASSIFICATION: {symbol} head timestamp is {head_timestamp.date()}")
                            logger.warning(f"📅 Requested start {start_date.date()} is {days_before} days before head timestamp")
                            
                            if days_before > 7:  # Significant gap
                                logger.error(f"📅 DEFINITIVE: Error 162 is HISTORICAL DATA LIMIT (not pace violation)")
                                return IbErrorInfo(
                                    error_code=162,
                                ...
                                logger.info(f"📅 Small gap ({days_before} days) - could be weekend/holiday, treating as pace violation")
                                return None
                        else:
                            logger.debug(f"📅 Request {start_date.date()} is after head timestamp {head_timestamp.date()} - not a head timestamp issue")
                            return None
                    else:
                        logger.debug(f"📅 No head timestamp data available for {symbol} in cache")
                        return None
                else:
    ...
    def wait_for_recovery(self, wait_time: float, cancellation_token: Optional[Any] = None) -> None:
        """
    ...
    def _check_cancellation(self, cancellation_token: Any) -> None:
        ...
        if is_cancelled:
            ...
            import asyncio
            raise asyncio.CancelledError("Operation cancelled during error recovery wait")
    
    def check_proactive_pace_limit(self, symbol: str, timeframe: str, cancellation_token: Optional[Any] = None) -> None:
        """
        ...
        wait_for_identical = self._check_identical_request_limit(request_key, current_time)
        ...
        total_wait = max(wait_for_frequency, wait_for_identical, wait_for_burst, wait_for_minimum)
        ...
        if total_wait > 0:
            ...
            logger.info(f"🚦 Wait breakdown: freq={wait_for_frequency:.1f}s, identical={wait_for_identical:.1f}s, burst={wait_for_burst:.1f}s, min={wait_for_minimum:.1f}s")
    ...
    def _clean_request_history(self, current_time: float) -> None:
        ...
        cutoff_time = current_time - IbLimitsRegistry.PACING_LIMITS["rate_window_seconds"]
        self.request_history = [req for req in self.request_history if req[0] > cutoff_time]
    ...
    def _check_frequency_limit(self, current_time: float) -> float:
        ...
        requests_in_window = len([req for req in self.request_history if req[0] > window_start])
        ...
        if requests_in_window >= safety_threshold:
            ...
            if self.request_history:
                oldest_in_window = min(req[0] for req in self.request_history if req[0] > window_start)
                wait_time = (oldest_in_window + window_seconds) - current_time
                logger.warning(f"🚦 FREQUENCY LIMIT: {requests_in_window}/{max_requests} requests in window, waiting {wait_time:.1f}s")
                return max(0, wait_time)
        
        return 0.0
    
    def _check_identical_request_limit(self, request_key: str, current_time: float) -> float:
        ...
        if request_key in self.identical_request_cache:
            ...
            if time_since_last < cooldown:
                ...
                logger.warning(f"🚦 IDENTICAL REQUEST: {request_key} requested {time_since_last:.1f}s ago, waiting {wait_time:.1f}s")
                return wait_time
            else:
    ...
    def _check_burst_limit(self, current_time: float) -> float:
        ...
        requests_in_burst = len([req for req in self.request_history if req[0] > burst_start])
        
        if requests_in_burst >= max_burst:
            # Wait for the burst window to clear
            if self.request_history:
                oldest_in_burst = min(req[0] for req in self.request_history if req[0] > burst_start)
                wait_time = (oldest_in_burst + burst_window) - current_time
                logger.warning(f"🚦 BURST LIMIT: {requests_in_burst}/{max_burst} requests in {burst_window}s, waiting {wait_time:.1f}s")
                return max(0, wait_time)
    ...
    def _check_minimum_delay(self, current_time: float) -> float:
        ...
        if time_since_last < min_delay:
            ...
            logger.debug(f"🚦 MINIMUM DELAY: Last request {time_since_last:.1f}s ago, waiting {wait_time:.1f}s")
            return wait_time
    ...
    def _create_request_key(self, symbol: str, timeframe: str) -> str:
        ...
        if self.last_request_context and \
           ...
           self.last_request_context.get("timeframe") == timeframe:
            ...
            request_key = f"{symbol}:{timeframe}:{start_date.isoformat()}:{end_date.isoformat()}"
            ...
            logger.warning(f"🔍 FALLBACK REQUEST KEY (no context available): {request_key}")
            return request_key
    ...
    def _record_request(self, symbol: str, timeframe: str, current_time: float) -> None:
        ...
        logger.debug(f"🚦 RECORDED REQUEST: {request_key} at {current_time:.1f} (total history: {len(self.request_history)})")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        requests_10min = len([req for req in self.request_history if req[0] > window_10min])
        requests_2sec = len([req for req in self.request_history if req[0] > window_2sec])
        ...
        return {
            ...
            "configured_delays": {
                ...
                "identical_requests": IbLimitsRegistry.SAFE_DELAYS["identical_requests"],
