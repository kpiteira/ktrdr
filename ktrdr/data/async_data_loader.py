...
class AsyncDataLoader:
    ...
    async def start_job(self, 
        ...
        if job.status != JobStatus.PENDING:
            raise DataError(f"Job {job_id} is not in pending status (current: {job.status.value})")
        
        # Create and start the async task
        task = asyncio.create_task(
            self._execute_job(job, progress_callback),
    ...
    async def _execute_job(self, 
        ...
        try:
            ...
            if result_df is not None and not result_df.empty:
                ...
                logger.info(f"✅ Job {job.job_id} completed successfully: {len(result_df)} bars")
            else:
    ...
    async def _load_data_with_progress(self, 
                                      job: DataLoadingJob, 
                                      progress_callback: Optional[Callable[[ProgressInfo], None]] = None) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            for i in range(job.progress.total_segments):
                ...
                if job.is_cancelled_requested:
                    logger.info(f"🛑 Job {job.job_id} cancelled at segment {i+1}/{job.progress.total_segments}")
                    raise asyncio.CancelledError()
                
                # Update current segment
                job.progress.current_segment = f"Segment {i+1}/{job.progress.total_segments}"
