...
class IbPaceManager:
    ...
    async def check_pace_limits_async(
        ...
        if start_date and end_date:
            self.error_handler.set_request_context(symbol, start_date, end_date, timeframe)
        
        # Create request key
        request_key = self._create_request_key(symbol, timeframe, operation, start_date, end_date)
    ...
    async def _calculate_required_wait_async(self, request_key: str, component: str) -> float:
    ...
    def _calculate_required_wait_sync(self, request_key: str, component: str) -> float:
        ...
        with self._pace_lock:
            ...
            identical_wait = self._check_identical_request_limit(request_key, current_time)
            if identical_wait > 0:
            ...
            if waits:
                ...
                logger.debug(f"🚦 Pace limit triggered: {max_wait[0]} requires {max_wait[1]:.1f}s wait")
                return max_wait[1]
            
            return 0.0
    
    async def _handle_pace_wait_async(self, request_key: str, wait_time: float, component: str):
        ...
        logger.warning(f"🚦 PROACTIVE PACE LIMIT: Waiting {wait_time:.1f}s for {request_key}")
    ...
    def _handle_pace_wait_sync(self, request_key: str, wait_time: float, component: str):
    ...
    async def _handle_pace_violation_async(
        ...
        violation_type = (
            PaceViolationType.IB_ERROR_162 if error_code == 162
            else PaceViolationType.IB_ERROR_165 if error_code == 165
    ...
    def _log_error(self, error_info: IbErrorInfo, component: str, req_id: Optional[int]):
        ...
        if error_info.error_type == IbErrorType.INFORMATIONAL:
            logger.debug(f"{prefix} IB Info {error_info.error_code}: {error_info.error_message}")
        elif error_info.error_type == IbErrorType.PACING_VIOLATION:
            logger.error(f"{prefix} 🚦 PACE VIOLATION {error_info.error_code}: {error_info.error_message}")
            ...
            level = "ERROR" if not error_info.is_retryable else "WARNING"
            logger.log(getattr(logger, level.lower()), f"{prefix} IB {level} {error_info.error_code}: {error_info.error_message}")
    ...
    def get_pace_statistics(self) -> Dict[str, Any]:
        ...
        with self._pace_lock:
            ...
            requests_10min = len([req for req in self._request_history if req[0] > window_10min])
            requests_2sec = len([req for req in self._request_history if req[0] > window_2sec])
            ...
            for violation_type in PaceViolationType:
                violations = [v for v in self._violation_history if v.violation_type == violation_type]
            ...
            return {
                "current_state": {
                    ...
                    "time_since_last_request": current_time - self._last_request_time if self._last_request_time > 0 else 0,
                ...
                "configuration": {
                    ...
                    "max_requests_per_10min": IbLimitsRegistry.PACING_LIMITS["max_requests_per_10min"],
                ...
                "history_stats": {
                    ...
                    "oldest_request": min(req[0] for req in self._request_history) if self._request_history else 0
                }
            }
