...
class DataQualityIssue:
    ...
    def __init__(self, issue_type: str, severity: str, description: str, 
                 location: Optional[str] = None, corrected: bool = False,
...
class DataQualityReport:
    ...
    def __init__(self, symbol: str, timeframe: str, total_bars: int, 
...
class DataQualityValidator:
    ...
    def validate_data(self, df: pd.DataFrame, symbol: str, timeframe: str,
                     validation_type: str = "general") -> Tuple[pd.DataFrame, DataQualityReport]:
        """
        ...
        logger.info(f"Starting data quality validation for {symbol} {timeframe} ({len(df)} bars, type: {validation_type})")
        ...
        try:
            ...
            logger.info(f"Validation complete: {summary['total_issues']} issues found, {summary['corrections_made']} corrected")
    ...
    def _validate_basic_structure(self, df: pd.DataFrame, report: DataQualityReport) -> pd.DataFrame:
    ...
    def _detect_and_fix_duplicates(self, df: pd.DataFrame, report: DataQualityReport) -> pd.DataFrame:
    ...
    def _ensure_sorted_index(self, df: pd.DataFrame, report: DataQualityReport) -> pd.DataFrame:
    ...
    def _validate_ohlc_relationships(self, df: pd.DataFrame, report: DataQualityReport) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in ['open', 'high', 'low', 'close']):
            ...
            high_too_low = (df['high'] < max_oc) & df[['open', 'high', 'low', 'close']].notna().all(axis=1)
            ...
            low_too_high = (df['low'] > min_oc) & df[['open', 'high', 'low', 'close']].notna().all(axis=1)
    ...
    def _handle_missing_values(self, df: pd.DataFrame, report: DataQualityReport) -> pd.DataFrame:
            ...
            if col in df.columns:
                ...
                if missing_count > 0:
                    ...
                    issue = DataQualityIssue(
                        ...
                        metadata={'count': missing_count, 'percentage': missing_percentage, 'column': col}
                    )
                    report.add_issue(issue)
                    ...
                    if can_correct:
                        ...
                        logger.info(f"Interpolated {missing_count} missing values in {col}")
        
        return df_corrected
    
    def _detect_timestamp_gaps(self, df: pd.DataFrame, timeframe: str, report: DataQualityReport):
        ...
        if gaps:
            ...
            issue = DataQualityIssue(
                ...
                metadata={
                    ...
                    'gaps': [(start.isoformat(), end.isoformat()) for start, end in gaps[:5]]  # First 5 gaps
                }
            )
            report.add_issue(issue)
            
            logger.info(f"Detected {len(gaps)} gaps in data ({gap_percentage:.2f}% missing)")
    
    def _detect_price_outliers(self, df: pd.DataFrame, report: DataQualityReport, 
                              std_threshold: float = 2.5, context_window: Optional[int] = None):
        ...
        for col in columns:
            ...
            if outliers.any():
                ...
                logger.debug(f"Found {count} outliers in {col}: mean={mean_val:.2f}, std={std_val:.2f}, max_z={max_z_score:.2f}")
    ...
    def _validate_volume_patterns(self, df: pd.DataFrame, report: DataQualityReport) -> pd.DataFrame:
        ...
        if volume_median > 0:
            ...
            if extreme_volume.any():
                ...
                issue = DataQualityIssue(
                    ...
                    metadata={'count': count, 'max_ratio': max_ratio, 'median_volume': volume_median}
                )
                report.add_issue(issue)
        
        return df_corrected
    
    def _detect_extreme_price_movements(self, df: pd.DataFrame, report: DataQualityReport):
        ...
        if extreme_moves.any():
            ...
            issue = DataQualityIssue(
                ...
                metadata={'count': count, 'max_change': max_change, 'threshold': extreme_threshold}
            )
            report.add_issue(issue)
        ...
        suspicious_moves = (np.abs(price_changes) > suspicious_threshold) & ~extreme_moves
