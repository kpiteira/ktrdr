...
class RSIIndicator(BaseIndicator):
    ...
    def compute(self, df: pd.DataFrame) -> pd.Series:
        ...
        self.validate_sufficient_data(df, period + 1)  # Need at least period+1 data points
        
        logger.debug(f"Computing RSI with period={period} on DataFrame with {len(df)} rows")
        ...
        try:
            ...
            if first_avg_loss != 0:
                ...
                if first_avg_gain > 0:
                    rsi.iloc[period] = 100.0  # If only gains and no losses, RSI is 100 (extremely overbought)
