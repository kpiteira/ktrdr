...
class MultiTimeframeIndicatorEngine:
    ...
    def apply_multi_timeframe(
        ...
        if not multi_timeframe_data:
            raise ConfigurationError(
                "No multi-timeframe data provided",
                "CONFIG-EmptyData",
                {}
            )
        ...
        for timeframe, engine in self.engines.items():
            ...
            try:
                ...
                logger.error(f"Error computing indicators for timeframe {timeframe}: {str(e)}")
    ...
    def _standardize_column_names(
        ...
        if column_mapping:
            ...
            logger.debug(f"Renamed {len(column_mapping)} indicator columns for timeframe {timeframe}")
    ...
    def create_cross_timeframe_features(
        ...
        for feature_name, spec in feature_specs.items():
            try:
                ...
                if primary_tf not in multi_timeframe_indicators or secondary_tf not in multi_timeframe_indicators:
                    logger.warning(f"Missing timeframe data for cross-feature {feature_name}")
                    continue
                ...
                if primary_col not in primary_data.columns or secondary_col not in secondary_data.columns:
                ...
                if operation == "ratio":
                    ...
                    cross_features[feature_name] = primary_values.rolling(window).corr(secondary_values)
                else:
                    logger.warning(f"Unknown operation {operation} for cross-feature {feature_name}")
    ...
    def validate_configuration(self) -> Dict[str, Any]:
        ...
        for i, tf in enumerate(timeframes):
            if tf in expected_hierarchy:
                ...
                higher_tfs = [t for t in timeframes if t in expected_hierarchy[tf_index+1:]]
                if not higher_tfs and i < len(timeframes) - 1:
                    hierarchy_violations.append(f"Timeframe {tf} lacks higher timeframe context")
        ...
        total_indicators = sum(len(engine.indicators) for engine in self.engines.values())
        ...
        if redundant_pairs:
            validation_results["warnings"].append(f"Redundant indicators found: {redundant_pairs}")
        
        # Final validation status
        if validation_results["errors"]:
            validation_results["valid"] = False
        
        logger.info(f"Multi-timeframe indicator configuration validation completed: {validation_results['valid']}")
        return validation_results


def create_multi_timeframe_engine_from_config(config: Dict[str, Any]) -> MultiTimeframeIndicatorEngine:
    """
