...
class AroonIndicator(BaseIndicator):
    ...
    def __init__(self, period: int = 14, include_oscillator: bool = False):
        ...
        logger.debug(f"Initialized Aroon indicator with period={period}, include_oscillator={include_oscillator}")
    ...
    def _validate_params(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(include_oscillator, bool):
            raise DataError(
                ...
                error_code="INDICATOR-InvalidParameter",
                details={"parameter": "include_oscillator", "value": include_oscillator, "expected_type": "bool"}
            )
    ...
    def compute(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.debug(f"Computing Aroon with period={period}, include_oscillator={include_oscillator}")
        ...
        logger.debug(f"Aroon computation completed. Valid Aroon Up values: {(~pd.isna(aroon_up)).sum()}")
