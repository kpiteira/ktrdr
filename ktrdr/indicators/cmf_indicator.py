...
class CMFIndicator(BaseIndicator):
    ...
    def get_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        cmf_signal_cross_up = (cmf_values > cmf_signal) & (cmf_values.shift(1) <= cmf_signal.shift(1))
        cmf_signal_cross_down = (cmf_values < cmf_signal) & (cmf_values.shift(1) >= cmf_signal.shift(1))
    ...
    def get_analysis(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if len(cmf_values) >= 5:
            ...
            trend_direction = "Improving" if recent_trend > 0 else "Deteriorating" if recent_trend < 0 else "Stable"
        else:
            recent_trend = 0
        ...
        # Signal analysis
        signal_state = "Above Signal" if current_cmf > current_signal else "Below Signal"
        
        # Zero line analysis
        zero_line_state = "Above Zero (Buying Pressure)" if current_cmf > 0 else "Below Zero (Selling Pressure)"
        
        # Divergence analysis (compare with price trend)
        price_change = recent_data["close"].iloc[-1] - recent_data["close"].iloc[0] if len(recent_data) >= period else 0
        cmf_change = cmf_values.iloc[-1] - cmf_values.iloc[0] if len(cmf_values) >= period else 0
        ...
        elif (price_change > 0 and cmf_change > 0) or (price_change < 0 and cmf_change < 0):
            divergence_state = "Confirmation"
        
        # Volatility analysis
        cmf_volatility = cmf_values.std() if len(cmf_values) > 1 else 0
        volatility_state = "High" if cmf_volatility > 0.15 else "Medium" if cmf_volatility > 0.05 else "Low"
        ...
        return {
            ...
            "strength_analysis": {
                ...
                "intensity": "Extreme" if abs(current_cmf) > 0.25 else "Strong" if abs(current_cmf) > 0.1 else "Moderate"
            },
            "trend_analysis": {
            ...
            "divergence_analysis": {
                ...
                "alignment": "Aligned" if (price_change * cmf_change) >= 0 else "Diverging"
            },
            "volatility_analysis": {
            ...
            "signals": {
                ...
                "divergence_warning": divergence_state in ["Bullish Divergence", "Bearish Divergence"]
            }
        }
