...
from ktrdr.indicators import BaseIndicator, RSIIndicator, SimpleMovingAverage, ExponentialMovingAverage
...
class IndicatorFactory:
    ...
    def __init__(self, config: Union[IndicatorsConfig, List[IndicatorConfig]]):
        ...
        logger.debug(f"Initialized IndicatorFactory with {len(self.indicators_config.indicators)} indicator configurations")
    ...
    def build(self) -> List[BaseIndicator]:
        ...
        for config in self.indicators_config.indicators:
            try:
                ...
                logger.info(f"Successfully created indicator: {indicator.name} ({config.type})")
        ...
        try:
            ...
            for attr_name in dir(module):
                ...
                if (isinstance(attr, type) and 
                    ...
                    attr.__name__ == indicator_type):
                    logger.debug(f"Found indicator class {attr.__name__} in module {module_path}")
                    return attr
            ...
            logger.error(f"Failed to import module for indicator {indicator_type}: {str(e)}")
            raise ConfigurationError(
