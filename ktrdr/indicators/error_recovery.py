...
class ResilientProcessor:
    ...
    def process_with_recovery(
        ...
        self.logger.info(f"Processing {len(data)} timeframes with {self.recovery_strategy.value} recovery")
        
        for timeframe, df in data.items():
            try:
                result = self._process_timeframe_with_recovery(
                    engine, timeframe, df
                )
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries:
                    self.logger.warning(f"Attempt {attempt + 1} failed for {timeframe}: {e}. Retrying...")
                    time.sleep(self.retry_delay)
                    continue
                else:
                    # All retries failed, try recovery
                    return self._attempt_recovery(timeframe_engine, timeframe, data, error_context)
    ...
    def _skip_indicator_recovery(
        ...
        for i, indicator in enumerate(original_indicators):
            try:
                ...
                remaining_indicators = [ind for j, ind in enumerate(original_indicators) if j != i]
                ...
                self.logger.info(f"Successfully processed {timeframe} after removing indicator {i}")
    ...
    def _fallback_recovery(
        ...
        if len(data) > 0:
            ...
            if 'close' in data.columns:
                fallback_data[f'SMA_fallback_{timeframe}'] = data['close'].rolling(
                    window=min(10, len(data)), min_periods=1
                ).mean()
                
                # Simple momentum fallback
                fallback_data[f'momentum_fallback_{timeframe}'] = data['close'].pct_change().fillna(0)
    ...
    def _partial_processing_recovery(
        ...
        for i, indicator in enumerate(engine.indicators):
            try:
                ...
                self.logger.warning(f"Failed to process indicator {i} in {timeframe}: {e}")
                continue
    ...
    def check_data_quality(self, data: Dict[str, pd.DataFrame]) -> Dict[str, List[str]]:
        ...
        for timeframe, df in data.items():
            ...
            for col in required_columns:
                if col in df.columns:
                    ...
                    if nan_count > 0:
                        ...
                        timeframe_issues.append(f"Column '{col}' has {nan_count} NaN values ({nan_percentage:.1f}%)")
            ...
            for col in numeric_columns:
                ...
                if inf_count > 0:
                    timeframe_issues.append(f"Column '{col}' has {inf_count} infinite values")
            
            # Check for OHLC constraints
            if all(col in df.columns for col in ['open', 'high', 'low', 'close']):
                # High should be >= max(open, close)
                high_violations = (df['high'] < np.maximum(df['open'], df['close'])).sum()
                if high_violations > 0:
                    timeframe_issues.append(f"{high_violations} rows violate high >= max(open, close)")
                
                # Low should be <= min(open, close)
                low_violations = (df['low'] > np.minimum(df['open'], df['close'])).sum()
                if low_violations > 0:
                    timeframe_issues.append(f"{low_violations} rows violate low <= min(open, close)")
            ...
            if 'close' in df.columns:
                ...
                if extreme_returns > 0:
                    timeframe_issues.append(f"{extreme_returns} extreme price movements (>50%)")
                ...
                for col in ['open', 'high', 'low', 'close']:
                    if col in fixed_df.columns:
                        ...
                        if fixed_df[col].isna().any():
                            ...
                            self.logger.warning(f"Used default value {default_value} for NaN in {timeframe}.{col}")
            ...
            if fix_inf:
                ...
                for col in numeric_columns:
                    ...
                    if inf_mask.any():
                        ...
                        self.logger.warning(f"Replaced {inf_mask.sum()} infinite values in {timeframe}.{col}")
            
            # Fix OHLC constraints
            if fix_ohlc and all(col in fixed_df.columns for col in ['open', 'high', 'low', 'close']):
...
def error_recovery_context(
    ...
    try:
        ...
        if processor.error_history:
            logger.info(f"Processing completed with {len(processor.error_history)} errors")
...
def process_with_comprehensive_error_handling(
    ...
    if check_data_quality:
        ...
        if fix_data_issues:
            ...
            if has_issues:
                ...
                error_report['warnings'].append("Data quality issues were automatically fixed")
    ...
    error_report['success_rate'] = {
        ...
        'success_percentage': (successful_timeframes / total_timeframes * 100) if total_timeframes > 0 else 0
    }
