...
class KeltnerChannelsIndicator(BaseIndicator):
    ...
    def compute(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        channel_position = (data["close"] - lower_channel) / (upper_channel - lower_channel)
        ...
        logger.debug(f"Computed Keltner Channels with period={period}, atr_period={atr_period}, multiplier={multiplier}")
    ...
    def get_analysis(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        width_percentile = (
            ...
            * 100
        ) if recent_data[width_col].max() > recent_data[width_col].min() else 50
        
        # Trend analysis
        ema_slope = (current_middle - data[middle_col].iloc[-5]) / 4 if len(data) >= 5 else 0
        trend_direction = "Uptrend" if ema_slope > 0 else "Downtrend" if ema_slope < 0 else "Sideways"
        ...
        for i in range(len(recent_data)):
            if recent_data.iloc[-(i+1)]["close"] > recent_data.iloc[-(i+1)][upper_col]:
                days_since_upper_breakout = i
                break
        
        for i in range(len(recent_data)):
            if recent_data.iloc[-(i+1)]["close"] < recent_data.iloc[-(i+1)][lower_col]:
