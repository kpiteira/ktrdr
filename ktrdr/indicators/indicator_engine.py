...
class IndicatorEngine:
    ...
    def __init__(self, indicators: Optional[Union[List[Dict], List[BaseIndicator]]] = None):
        """
        ...
        logger.info(f"Initialized IndicatorEngine with {len(self.indicators)} indicators")
    ...
    def apply(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if data is None or data.empty:
            raise ConfigurationError(
                "Cannot compute indicators on empty data.",
                "CONFIG-EmptyData",
                {}
            )
        ...
        for indicator in self.indicators:
            try:
                ...
                indicator_name = getattr(indicator, 'name', str(indicator.__class__.__name__))
                ...
                logger.error(f"Error computing indicator {indicator.__class__.__name__}: {str(e)}")
    ...
    def compute_rsi(self, data: pd.DataFrame, period: int = 14, source: str = 'close') -> pd.DataFrame:
        """
    ...
    def compute_sma(self, data: pd.DataFrame, period: int = 20, source: str = 'close') -> pd.DataFrame:
        """
    ...
    def compute_ema(self, data: pd.DataFrame, period: int = 20, source: str = 'close') -> pd.DataFrame:
        """
    ...
    def compute_macd(self, data: pd.DataFrame, fast_period: int = 12, slow_period: int = 26, signal_period: int = 9, source: str = 'close') -> pd.DataFrame:
        """
