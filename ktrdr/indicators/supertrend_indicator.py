...
class SuperTrendIndicator(BaseIndicator):
    ...
    def compute(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.debug(f"Computed SuperTrend with period={period}, multiplier={multiplier}")
    ...
    def get_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        price_at_support = bullish_trend & (close <= st_values * 1.005)  # Within 0.5% of SuperTrend
        price_at_resistance = bearish_trend & (close >= st_values * 0.995)  # Within 0.5% of SuperTrend
    ...
    def get_analysis(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if st_slope_col in recent_data.columns:
            recent_slope = recent_data[st_slope_col].iloc[-1] if not pd.isna(recent_data[st_slope_col].iloc[-1]) else 0
            slope_direction = "Rising" if recent_slope > 0.2 else "Falling" if recent_slope < -0.2 else "Stable"
        else:
            recent_slope = 0
        ...
        # Risk assessment
        if current_direction == 1:
            risk_level = "Low" if current_distance > 2 else "Medium" if current_distance > -1 else "High"
        else:
            risk_level = "Low" if current_distance < -2 else "Medium" if current_distance < 1 else "High"
        ...
        return {
            ...
            "historical_context": {
                ...
                "trend_consistency": round((max(uptrend_days, downtrend_days) / len(directions)) * 100, 1)
            },
            "risk_assessment": {
                "level": risk_level,
                "stop_loss_level": round(current_st, 4),
                "trend_reversal_risk": "High" if current_strength < 3 else "Medium" if current_strength < 7 else "Low"
            },
