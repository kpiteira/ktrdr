...
class ComplexConfigurationHandler:
    ...
    def __init__(self, fallback_strategy: FallbackStrategy = FallbackStrategy.REDUCE_PERIOD):
        """
        ...
        self.indicator_requirements = {
            ...
            'MACD': IndicatorRequirement(
                ...
                fallback_parameters={'fast_period': 6, 'slow_period': 13, 'signal_period': 5}
            ),
    ...
    def analyze_data_availability(self, data: Dict[str, pd.DataFrame]) -> Dict[str, DataAvailability]:
        """
    ...
    def validate_configuration(
        ...
        for config in timeframe_configs:
            ...
            for indicator_config in config.indicators:
                ...
                required_points = self._calculate_required_points(indicator_type, params)
                ...
                if required_points > availability.valid_points:
                    ...
                    issue = ConfigurationIssue(
                        ...
                        suggested_fix=self._suggest_fix(indicator_type, params, availability.valid_points)
                    )
                    issues.append(issue)
    ...
    def _calculate_required_points(self, indicator_type: str, params: Dict[str, Any]) -> int:
    ...
    def _suggest_fix(self, indicator_type: str, params: Dict[str, Any], available_points: int) -> str:
    ...
    def _apply_fallback_strategy(
        ...
        if self.fallback_strategy == FallbackStrategy.SKIP:
            self.logger.info(f"Skipping {indicator_config['type']} due to insufficient data")
            return None
            ...
            self.logger.warning(f"Computing {indicator_config['type']} with insufficient data")
            ...
            self.logger.warning(f"PAD_DATA strategy not implemented, using REDUCE_PERIOD")
            return self._reduce_indicator_periods(indicator_config, available_points)
    ...
    def _reduce_indicator_periods(
        ...
        if indicator_type in ['SimpleMovingAverage', 'ExponentialMovingAverage']:
            ...
            if new_period != original_period:
                self.logger.info(f"Reduced {indicator_type} period from {original_period} to {new_period}")
            ...
            if new_period != original_period:
                self.logger.info(f"Reduced RSI period from {original_period} to {new_period}")
            ...
            if new_slow != original_slow or new_fast != original_fast:
                self.logger.info(f"Reduced MACD periods: {original_fast}/{original_slow}/{original_signal} "
            ...
            if new_period != original_period:
                self.logger.info(f"Reduced BollingerBands period from {original_period} to {new_period}")
            ...
            if new_k != original_k:
                self.logger.info(f"Reduced Stochastic k_period from {original_k} to {new_k}")
            ...
            if new_period != original_period:
                self.logger.info(f"Reduced ATR period from {original_period} to {new_period}")
    ...
    def create_adaptive_configuration(
        ...
        issues, corrected_configs = self.validate_configuration(base_configs, availability)
        
        # Log summary
        if issues:
            self.logger.warning(f"Found {len(issues)} configuration issues")
            for issue in issues:
                self.logger.warning(f"{issue.timeframe}/{issue.indicator_type}: {issue.message}")
        
        self.logger.info(f"Created adaptive configuration with {len(corrected_configs)} timeframes")
