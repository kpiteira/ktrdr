...
class RVIIndicator(BaseIndicator):
    ...
    def __init__(self, period: int = 10, signal_period: int = 4):
        ...
        logger.debug(f"Initialized RVI indicator with period={period}, signal_period={signal_period}")
    ...
    def _validate_params(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(signal_period, int) or signal_period < 1:
            raise DataError(
                ...
                error_code="INDICATOR-InvalidParameter",
                details={"parameter": "signal_period", "value": signal_period, "minimum": 1}
            )
        ...
        if signal_period > 50:
            raise DataError(
                ...
                error_code="INDICATOR-InvalidParameter",
                details={"parameter": "signal_period", "value": signal_period, "maximum": 50}
            )
    ...
    def compute(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.debug(f"Computing RVI with period={period}, signal_period={signal_period}")
        ...
        for i in range(3, len(data)):
            ...
            num_sum = (close_open.iloc[i-3] + 
                      2 * close_open.iloc[i-2] + 
                      2 * close_open.iloc[i-1] + 
                      close_open.iloc[i])
            
            den_sum = (high_low.iloc[i-3] + 
                      2 * high_low.iloc[i-2] + 
                      2 * high_low.iloc[i-1] + 
                      high_low.iloc[i])
        ...
        numerator_ma = numerator_weighted.rolling(window=period, min_periods=period).mean()
        denominator_ma = denominator_weighted.rolling(window=period, min_periods=period).mean()
        ...
        logger.debug(f"RVI computation completed. Valid RVI values: {(~pd.isna(rvi)).sum()}")
