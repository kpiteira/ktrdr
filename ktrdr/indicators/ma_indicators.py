...
class SimpleMovingAverage(BaseIndicator):
    ...
    def __init__(self, period: int = 20, source: str = 'close'):
        ...
        logger.debug(f"Initialized SMA indicator with period {period} using {source} prices")
    ...
    def _validate_params(self, params):
        ...
        if 'period' in params:
            ...
            if not isinstance(period, int):
                raise DataError(
                    ...
                    details={"parameter": "period", "expected": "int", "received": type(period).__name__}
                )
            if period < 2:
                raise DataError(
        ...
        if 'source' in params and not isinstance(params['source'], str):
            raise DataError(
                ...
                error_code="DATA-InvalidType",
                details={"parameter": "source", "expected": "str", "received": type(params['source']).__name__}
            )
    ...
    def compute(self, df: pd.DataFrame) -> pd.Series:
        ...
        logger.debug(f"Computing SMA with period={period} on DataFrame with {len(df)} rows")
...
class ExponentialMovingAverage(BaseIndicator):
    ...
    def __init__(self, period: int = 20, source: str = 'close', adjust: bool = True):
        ...
        if not isinstance(adjust, bool):
            raise DataError(
                ...
                error_code="DATA-InvalidType",
                details={"parameter": "adjust", "expected": "bool", "received": type(adjust).__name__}
            )
        ...
        logger.debug(f"Initialized EMA indicator with period {period} using {source} prices (adjust={adjust})")
    ...
    def get_column_name(self, suffix: str = None) -> str:
        ...
        key_params = {k: v for k, v in self.params.items() if k not in ['source', 'adjust']}
    ...
    def compute(self, df: pd.DataFrame) -> pd.Series:
        ...
        logger.debug(f"Computing EMA with period={period} on DataFrame with {len(df)} rows")
