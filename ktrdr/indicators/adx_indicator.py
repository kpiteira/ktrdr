...
class ADXIndicator(BaseIndicator):
    ...
    def get_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        di_bullish_cross = (di_plus > di_minus) & (di_plus.shift(1) <= di_minus.shift(1))
        di_bearish_cross = (di_minus > di_plus) & (di_minus.shift(1) <= di_plus.shift(1))
    ...
    def get_analysis(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        direction_consistency = (di_values > 0).sum() / len(di_values) * 100 if len(di_values) > 0 else 50
        ...
        return {
            ...
            "market_character": {
                ...
                "volatility": "High" if adx_max - adx_min > 30 else "Medium" if adx_max - adx_min > 15 else "Low"
            },
            "directional_analysis": {
                "dominant_force": "+DI" if current_di_plus > current_di_minus else "-DI",
                "spread_magnitude": round(abs(current_spread), 2),
                "directional_strength": "Strong" if abs(current_spread) > 10 else "Moderate" if abs(current_spread) > 5 else "Weak"
            },
