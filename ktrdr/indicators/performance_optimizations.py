...
class ChunkedProcessor:
    ...
    def process_timeframe_chunked(
        ...
        self.logger.info(f"Processing {len(data)} rows in {total_chunks} chunks for {timeframe}")
        ...
        for i in range(0, len(data), self.chunk_size):
            ...
            if total_chunks > 10 and (i // self.chunk_size + 1) % (total_chunks // 10) == 0:
                progress = ((i // self.chunk_size + 1) / total_chunks) * 100
                self.logger.debug(f"Progress: {progress:.0f}% ({i // self.chunk_size + 1}/{total_chunks})")
...
class ParallelProcessor:
    ...
    def process_timeframe_parallel(
        ...
        try:
            ...
            self.logger.debug(f"Processed {timeframe}: {len(data)} rows in {processing_time:.2f}s")
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    self.logger.error(f"Failed to process timeframe {failed_timeframe}: {e}")
                    raise
    ...
    def update_incremental(
        ...
        for timeframe, new_df in new_data.items():
            ...
            if timeframe in self.cached_data:
                ...
                combined_data = pd.concat([self.cached_data[timeframe], new_df], ignore_index=True)
...
class OptimizedMultiTimeframeEngine:
    ...
    def __init__(
        ...
        self.chunked_processor = ChunkedProcessor(chunk_size=chunk_size) if enable_chunking else None
    ...
    def apply_multi_timeframe_optimized(
        ...
        self.logger.info(f"Processing {total_rows} total rows across {len(data)} timeframes")
        
        # Determine processing strategy
        use_chunking = (
            force_chunking or 
            (self.enable_chunking and total_rows > 10000)
        )
        ...
        if use_chunking and use_parallel:
            ...
            results = self.chunked_processor.process_multi_timeframe_chunked(data, self.base_engine)
        elif use_parallel:
            self.logger.info("Using parallel processing")
            results = self.parallel_processor.process_multi_timeframe_parallel(data, self.base_engine)
        else:
    ...
    def _process_chunked_parallel(self, data: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        def process_timeframe_chunked_parallel(timeframe: str, df: pd.DataFrame) -> tuple[str, pd.DataFrame]:
            if timeframe in self.base_engine.engines:
                engine = self.base_engine.engines[timeframe]
                result = self.chunked_processor.process_timeframe_chunked(df, engine, timeframe)
        ...
        with ThreadPoolExecutor(max_workers=self.parallel_processor.max_workers) as executor:
            futures = {
