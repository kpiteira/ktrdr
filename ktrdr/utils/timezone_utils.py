...
class TimestampManager:
    ...
    def to_exchange_time(utc_timestamp: pd.Timestamp, exchange_tz: str) -> pd.Timestamp:
        ...
        try:
            ...
            logger.error(f"Failed to convert UTC to {exchange_tz}: {utc_timestamp}, error: {e}")
    ...
    def format_for_display(utc_timestamp: pd.Timestamp, display_tz: str = 'UTC') -> str:
        ...
        try:
            if display_tz == 'UTC':
                ...
                local_time = TimestampManager.to_exchange_time(utc_timestamp, display_tz)
                return local_time.strftime('%Y-%m-%d %H:%M:%S %Z')
        except Exception as e:
            logger.error(f"Failed to format timestamp for display: {utc_timestamp}, tz: {display_tz}, error: {e}")
    ...
    @staticmethod
    def validate_timezone_consistency(df: pd.DataFrame, operation_name: str = "operation") -> None:
        """
        ...
        if str(df.index.tz) != 'UTC':
            raise ValueError(f"{operation_name}: DataFrame has non-UTC timezone: {df.index.tz}")
        
        # Additional sanity checks
        if len(df) > 0:
            first_ts = df.index[0]
            last_ts = df.index[-1]
            logger.debug(f"{operation_name}: Timezone validation passed - {first_ts} to {last_ts} (UTC)")
    ...
    @staticmethod
    def is_market_hours(timestamp: pd.Timestamp, exchange_tz: str = 'America/New_York') -> bool:
        """
    ...
    @staticmethod 
    def get_trading_session(timestamp: pd.Timestamp, exchange_tz: str = 'America/New_York') -> str:
        """
        ...
        try:
            ...
            if (hour == 4 and minute >= 0) or (4 < hour < 9) or (hour == 9 and minute < 30):
                ...
                return 'regular'
            elif (hour == 16 and minute > 0) or (16 < hour < 20) or (hour == 20 and minute == 0):
                return 'after_hours'
            else:
                return 'closed'
