...
class MultiTimeframeDecisionOrchestrator:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized MultiTimeframeDecisionOrchestrator for {len(self.timeframes)} timeframes: {self.timeframes}"
        )
    ...
    def make_multi_timeframe_decision(
        ...
        logger.info(
            f"Multi-timeframe decision for {symbol}: {final_decision.signal.value} "
    ...
    def _create_timeframe_config(self, timeframe: str) -> str:
        ...
        return Path(
            self.strategy_config.get(
                "config_path", "strategies/multi_timeframe_config.yaml"
            )
        )
    ...
    def _load_multi_timeframe_model(self, model_path: str):
        ...
        try:
            ...
            model_data = self.model_storage.load_multi_timeframe_model(
                strategy_name=self.strategy_name, symbol=symbol, timeframes=timeframes
            )
            ...
            logger.info(
                f"Loaded multi-timeframe model for {symbol} with timeframes {timeframes}"
            )
    ...
    def _generate_timeframe_decisions(
        self, context: MultiTimeframeDecisionContext
        ...
        for timeframe in self.timeframes:
            ...
            try:
                ...
                logger.debug(
                    f"Timeframe {timeframe} decision: {decision.signal.value} "
                    f"(confidence: {quality_adjusted_confidence:.3f})"
                )

            except Exception as e:
                logger.error(
                    f"Failed to generate decision for timeframe {timeframe}: {e}"
                )
                continue
    ...
    def _build_multi_timeframe_consensus(
        ...
        most_common_signal = (
            max(set(signals), key=signals.count) if signals else Signal.HOLD
        )
        conflicting_timeframes = [
            tf
    ...
    def _weighted_majority_consensus(
        ...
        if primary_decision:
            primary_influence = (
                1.0
                if primary_decision.signal == final_signal
        ...
        reasoning = {
            ...
            "winning_vote_ratio": signal_votes[final_signal]
            / sum(signal_votes.values()),
            "primary_timeframe": context.primary_timeframe,
            "primary_alignment": (
                primary_decision.signal == final_signal if primary_decision else None
            ),
        }
    ...
    def _hierarchical_consensus(
        ...
        reasoning = {
            ...
            "primary_timeframe": context.primary_timeframe,
            "primary_confidence": (
                primary_decision.confidence if primary_decision else 0.0
            ),
            "primary_signal": (
                primary_decision.signal.value if primary_decision else "NONE"
            ),
            "fallback_used": False,
        }
    ...
    def _simple_consensus(
        ...
        primary_influence = 0.0
        if primary_decision:
            primary_influence = 0.5 if primary_decision.signal == final_signal else -0.5
    ...
    def _apply_multi_timeframe_logic(
        self, consensus: MultiTimeframeConsensus, context: MultiTimeframeDecisionContext
    ) -> TradingDecision:
        ...
        data_qualities = [
            d.data_quality for d in consensus.timeframe_decisions.values()
        ]
    ...
    def _update_multi_timeframe_state(
        ...
        if primary_tf_context and not primary_tf_context.current_bar.empty:
            position_state.update_from_decision(
                decision, primary_tf_context.current_bar
            )
    ...
    def get_consensus_history(
        ...
        if symbol:
            ...
            filtered_consensus = []
            decision_symbols = [
                d.reasoning.get("symbol") for d in self.decision_history
            ]
            for i, consensus in enumerate(consensus_list):
                if (
                    i < len(decision_symbols)
                    and decision_symbols[-(len(consensus_list) - i)] == symbol
                ):
