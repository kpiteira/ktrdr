            ...
            try:
                ...
                if strategy == FallbackStrategy.DEFAULT_VALUE:
                    ...
                    if args_hash in result_cache:
                        log.info(
                            f"Returning last known good result for {func.__name__}"
                        )
                        return result_cache[args_hash]
                    else:
                        log.warning(
                            f"No last known good result available for {func.__name__}, returning default value"
                        )
                    ...
                    raise ValueError(f"Unhandled fallback strategy: {strategy}") from None
