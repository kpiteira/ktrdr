...
class IbDataFetcher:
    ...
    async def fetch_historical_data(
        ...
        try:
            ...
            logger.debug(f"Successfully fetched {len(result)} bars for {symbol} {timeframe}")
            return result
    ...
    def _fetch_historical_data_impl(
        self, ib, symbol: str, timeframe: str, start: datetime, end: datetime, instrument_type: str
    ) -> pd.DataFrame:
        """
        ...
        logger.info(f"🔍 DIAGNOSIS: Parameters - start={start}, end={end}, instrument_type={instrument_type}")
        ...
        try:
            ...
            logger.info(f"🔍 DIAGNOSIS: Contract details - symbol={getattr(contract, 'symbol', 'N/A')}, secType={getattr(contract, 'secType', 'N/A')}, exchange={getattr(contract, 'exchange', 'N/A')}")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"🔍 DIAGNOSIS: IB parameters - duration={duration}, bar_size={ib_bar_size}")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"✅ DIAGNOSIS: Successfully processed {len(df)} bars for {symbol} {timeframe}")
            return df
    ...
    def _create_contract(self, symbol: str, instrument_type: str) -> Contract:
        ...
        Returns:
            IB Contract object
        """
        logger.info(f"🔍 DIAGNOSIS: Creating contract for symbol='{symbol}', instrument_type='{instrument_type}'")
        ...
        if instrument_type == "CASH" or "." in symbol:
            ...
            if "." in symbol:
                ...
                logger.info(f"🔍 DIAGNOSIS: Dot-separated forex: {symbol} → {forex_symbol}")
            ...
            logger.info(f"🔍 DIAGNOSIS: Unknown instrument type '{instrument_type}', defaulting to stock")
            contract = Stock(symbol, "SMART", "USD")
    ...
    def _get_what_to_show(self, instrument_type: str, symbol: str) -> str:
        ...
        if instrument_type == "CASH" or "." in symbol:
            ...
            # Default fallback
            logger.warning(f"Unknown instrument type '{instrument_type}' for {symbol}, defaulting to TRADES")
            return "TRADES"
