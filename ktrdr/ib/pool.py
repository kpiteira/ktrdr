...
class IbConnectionPool:
    ...
    def __init__(self, host: str = "localhost", port: int = 4002, max_connections: int = 5):
        """
        ...
        self.max_client_id_attempts = 3  # Conservative limit to avoid overwhelming IB Gateway
        ...
        logger.info(f"IbConnectionPool initialized for {host}:{port} (max_connections={max_connections})")
    ...
    async def acquire_connection(self, timeout: float = 30.0) -> IbConnection:
        ...
        async with self.lock:
            ...
            logger.info(f"Created new connection {connection.client_id} (pool size: {len(self.connections)})")
            return connection
        ...
        for conn in self.connections:
            if conn.is_healthy():
                ...
                try:
                    ...
                    logger.warning(f"Error stopping unhealthy connection {conn.client_id}: {e}")
    ...
    async def _create_new_connection(self, timeout: float) -> IbConnection:
        ...
        for attempt in range(self.max_client_id_attempts):
            ...
            try:
                ...
                if not connection.start():
                    logger.warning(f"Failed to start connection thread for client_id={client_id}")
                ...
                await self._wait_for_connection_ready(connection, min(remaining_time, 20.0))
        ...
        raise ConnectionError(f"Failed to create connection after {self.max_client_id_attempts} attempts")
    
    async def _wait_for_connection_ready(self, connection: IbConnection, timeout: float):
        """
        ...
        logger.warning(f"Connection {connection.client_id} not ready within {timeout:.1f}s")
    ...
    async def get_connection(self):
        ...
        """
        logger.info(f"🔍 POOL DIAGNOSIS: get_connection called, acquiring connection...")
        connection = await self.acquire_connection()
        logger.info(f"🔍 POOL DIAGNOSIS: Connection {connection.client_id} acquired successfully")
        try:
            yield connection
        finally:
            # Note: We don't explicitly release connections since they
            # auto-timeout after 3 minutes. This simplifies the design.
            logger.info(f"🔍 POOL DIAGNOSIS: Releasing connection {connection.client_id}")
            pass
    ...
    async def execute_with_connection_sync(self, func, *args, **kwargs):
        ...
        """
        logger.info(f"🔍 POOL DIAGNOSIS: Starting execute_with_connection_sync for {func.__name__}")
        logger.info(f"🔍 POOL DIAGNOSIS: Args: {args[:2] if args else 'None'}")  # Only log first 2 args to avoid clutter
        
        try:
            logger.info(f"🔍 POOL DIAGNOSIS: Acquiring connection...")
            async with self.get_connection() as conn:
                logger.info(f"🔍 POOL DIAGNOSIS: Got connection {conn.client_id}, calling execute_sync_request")
                result = conn.execute_sync_request(func, *args, **kwargs)
                logger.info(f"🔍 POOL DIAGNOSIS: execute_sync_request completed successfully")
                return result
        except Exception as e:
    ...
    async def cleanup_all(self):
        ...
        """
        async with self.lock:
            logger.info(f"Cleaning up all connections in pool ({len(self.connections)} connections)")
    ...
    async def health_check(self) -> dict:
        ...
        async with self.lock:
            ...
            return {
                ...
                "pool_utilization": healthy_count / self.max_connections if self.max_connections > 0 else 0,
