...
class IbErrorClassifier:
    ...
    ERROR_MAPPINGS = {
        # === PACING VIOLATIONS ===
        100: ("Max rate of messages per second has been exceeded", IbErrorType.PACING_VIOLATION, 60.0),
        420: ("Invalid real-time query (pacing violation)", IbErrorType.PACING_VIOLATION, 60.0),
        
        # === CONNECTION ERRORS (retryable) ===
        1100: ("Connectivity between IB and the TWS has been lost", IbErrorType.CONNECTION_ERROR, 5.0),
        1101: ("Connectivity between IB and TWS has been restored", IbErrorType.CONNECTION_ERROR, 0.0),
        1102: ("Connectivity between IB and TWS has been restored", IbErrorType.CONNECTION_ERROR, 0.0),
        ...
        200: ("No security definition has been found for the request", IbErrorType.FATAL, 0.0),
        354: ("Requested market data is not subscribed", IbErrorType.PERMISSION_ERROR, 0.0),
        10090: ("Part of requested market data is not subscribed", IbErrorType.PERMISSION_ERROR, 0.0),
        ...
        162: ("Historical Market Data Service error message", IbErrorType.DATA_UNAVAILABLE, 0.0),
        165: ("Historical Market Data Service query message", IbErrorType.DATA_UNAVAILABLE, 0.0),
        430: ("Fundamentals data for the security specified is not available", IbErrorType.DATA_UNAVAILABLE, 0.0),
        
        # === CONNECTIVITY STATUS (informational) ===
        2103: ("Market data farm connection is broken", IbErrorType.CONNECTION_ERROR, 2.0),
        2104: ("Market data farm connection is OK", IbErrorType.CONNECTION_ERROR, 0.0),
        2105: ("HMDS data farm connection is broken", IbErrorType.CONNECTION_ERROR, 2.0),
        2106: ("HMDS data farm connection is OK", IbErrorType.CONNECTION_ERROR, 0.0),
        2107: ("HMDS data farm connection is inactive", IbErrorType.CONNECTION_ERROR, 2.0),
        2108: ("Market data farm connection is inactive", IbErrorType.CONNECTION_ERROR, 2.0),
        2119: ("Market data farm connection is OK", IbErrorType.CONNECTION_ERROR, 0.0),
        
        # === FATAL ERRORS (do not retry) ===
        154: ("Orders cannot be transmitted for a halted security", IbErrorType.FATAL, 0.0),
    ...
    def classify(cls, error_code: int, error_message: str) -> Tuple[IbErrorType, float]:
        ...
        """
        logger.debug(f"Classifying IB error: code={error_code}, message='{error_message}'")
        ...
        if error_code in cls.ERROR_MAPPINGS:
            ...
            logger.debug(f"Found explicit mapping: {description} -> {error_type.value}, wait={wait_time}s")
        ...
        if any(keyword in error_message_lower for keyword in cls.TRANSPORT_ERROR_KEYWORDS):
        ...
        if any(keyword in error_message_lower for keyword in cls.PERMISSION_ERROR_KEYWORDS):
