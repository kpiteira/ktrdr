...
class IBTradingHoursParser:
    ...
    @classmethod
    def parse_ib_trading_hours(
        cls, ib_info: IBTradingHoursInfo
        ...
        try:
            ...
            if not all_sessions:
                logger.warning(
                    f"Could not parse trading hours: {ib_info.trading_hours}"
                )
                return None
    ...
    def _extract_extended_hours(
        ...
        regular_start, regular_end = (
            regular_sessions[0] if regular_sessions else (None, None)
        )
        ...
        for session_start, session_end in all_sessions:
            ...
            if regular_start and session_start < regular_start:
                extended.append(
                    TradingSession(session_start, regular_start, "Pre-Market")
                )
