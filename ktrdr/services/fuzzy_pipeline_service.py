...
from ktrdr.indicators.config import IndicatorConfig, TimeframeIndicatorConfig
from ktrdr.fuzzy.config import FuzzyConfigLoader
from ktrdr.errors import ServiceError, ConfigurationError, DataError
...
class FuzzyPipelineService:
    ...
    def process_symbol_fuzzy(
        ...
        Raises:
            ServiceError: If processing fails and fail_fast is True
        """
        logger.info(f"Processing fuzzy analysis for symbol: {symbol}")
        
        try:
            # Load and validate configurations
            indicator_config_dict = self._load_configuration(indicator_config, "indicator")
            ...
            market_data = self._load_market_data(symbol, required_timeframes, data_period_days)
            ...
            logger.info(f"Completed fuzzy analysis for {symbol}: {len(result.fuzzy_result.fuzzy_values)} fuzzy values")
            return result
    ...
    def create_fuzzy_summary_report(
        ...
        if isinstance(results, IntegratedFuzzyResult):
            # Single result
            return self._create_single_symbol_report(results, include_performance_metrics)
        else:
            # Multiple results
            return self._create_multi_symbol_report(results, include_performance_metrics)
    ...
    def _create_single_symbol_report(
        ...
        if fuzzy_values:
            sorted_values = sorted(fuzzy_values.items(), key=lambda x: x[1], reverse=True)
    ...
    def _create_multi_symbol_report(
        self,
        results: Dict[str, IntegratedFuzzyResult],
        include_performance: bool
    ) -> Dict[str, Any]:
        ...
        # Aggregated metrics
        if results:
            all_fuzzy_counts = [len(r.fuzzy_result.fuzzy_values) for r in results.values()]
            all_processing_times = [r.total_processing_time for r in results.values()]
            
            report["aggregated_metrics"] = {
                "avg_fuzzy_values_per_symbol": sum(all_fuzzy_counts) / len(all_fuzzy_counts),
