...
class MultiTimeframeFeatureEngineer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            f"Initialized MultiTimeframeFeatureEngineer with {len(self.timeframe_specs)} timeframes"
        )
    ...
    def prepare_multi_timeframe_features(
        ...
        scaled_features, scaler = self._apply_feature_scaling(
            feature_array, saved_scaler
        )
        ...
        feature_stats = self._calculate_feature_stats(
            feature_array, all_feature_names, timeframe_feature_map
        )

        logger.info(
            f"Prepared {features_tensor.shape[1]} total features from {len(timeframe_feature_map)} timeframes"
        )
    ...
    def _get_timeframe_order(self) -> List[str]:
        ...
        return sorted(
            available_timeframes, key=lambda tf: timeframe_priority.get(tf, 999)
        )
    ...
    def _extract_timeframe_features(
        ...
        if (
            feature_spec.indicator_features
    ...
    def _extract_fuzzy_features(
        self, fuzzy_data: pd.DataFrame, timeframe: str, expected_features: List[str]
    ) -> Tuple[List[float], List[str]]:
        """
        ...
        for expected_feature in expected_features:
            ...
            if expected_feature in latest_row:
                ...
                logger.warning(
                    f"Missing fuzzy feature {expected_feature} for timeframe {timeframe}"
                )
                value = 0.0
    ...
    def _extract_indicator_features(
        self, indicators: pd.DataFrame, timeframe: str, expected_features: List[str]
    ) -> Tuple[List[float], List[str]]:
        """
        ...
        for expected_feature in expected_features:
            ...
            if expected_feature in latest_row:
                ...
                logger.warning(
                    f"Missing indicator {expected_feature} for timeframe {timeframe}"
                )
                value = 0.0
    ...
    def _apply_feature_scaling(
        self, features: np.ndarray, saved_scaler: Optional[Any] = None
    ...
    def _calculate_feature_stats(
        ...
        zero_features = [
            feature_names[i] for i in range(len(feature_names)) if features[0, i] == 0.0
        ]
    ...
    def prepare_batch_features(
        ...
        for i in range(batch_size):
            ...
            all_batch_features.append(
                result.features_tensor.squeeze(0)
    ...
    def get_feature_template(self) -> Dict[str, Any]:
        ...
        template = {"timeframes": {}, "total_expected_features": 0}
        ...
        for timeframe, spec in self.timeframe_specs.items():
            if spec.enabled:
                tf_template = {
                    ...
                    "weight": spec.weight,
                    "expected_count": len(spec.fuzzy_features)
                    + len(spec.indicator_features or []),
                }
        ...
        for timeframe, spec in self.timeframe_specs.items():
            ...
            if timeframe in fuzzy_data and len(fuzzy_data[timeframe]) > 0:
                ...
                validation_report["errors"].append(
                    f"Missing fuzzy data for timeframe '{timeframe}'"
                )
