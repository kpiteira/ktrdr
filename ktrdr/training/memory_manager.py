...
class MemoryManager:
    ...
    def __init__(self, budget: Optional[MemoryBudget] = None, output_dir: Optional[Path] = None):
        ...
        if self.has_gpu:
            ...
            logger.info(f"GPU memory management enabled - {self.gpu_count} GPU(s) detected")
        else:
            self.gpu_count = 0
    ...
    def _check_memory_warnings(self, snapshot: MemorySnapshot):
        ...
        if (self.budget.max_process_memory_mb and 
            snapshot.process_memory_mb > self.budget.max_process_memory_mb * self.budget.warning_threshold_percent):
            logger.warning(
        ...
        # GPU memory warning
        if self.has_gpu and snapshot.gpu_memory_percent > self.budget.warning_threshold_percent * 100:
            logger.warning(
        ...
        # Critical warnings
        if snapshot.system_memory_percent > self.budget.critical_threshold_percent * 100:
            logger.critical(
                f"CRITICAL: System memory usage at {snapshot.system_memory_percent:.1f}% - "
                f"Training may be unstable"
            )
        
        if self.has_gpu and snapshot.gpu_memory_percent > self.budget.critical_threshold_percent * 100:
            logger.critical(
    ...
    def _get_recommendations(self, snapshot: MemorySnapshot) -> List[str]:
        ...
        if (self.budget.max_process_memory_mb and 
            snapshot.process_memory_mb > self.budget.max_process_memory_mb * 0.7):
        ...
        if self.has_gpu and snapshot.gpu_memory_percent > 70:
            ...
            recommendations.append("Reduce model size or batch size for GPU memory efficiency")
        
        # Tensor recommendations
        if snapshot.tensors_count > 10000:
            recommendations.append("High tensor count detected - check for tensor leaks")
    ...
    def export_memory_log(self, file_path: Optional[Path] = None) -> Path:
        ...
        if file_path is None:
            if self.output_dir:
                file_path = self.output_dir / f"memory_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            else:
                file_path = Path(f"memory_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    ...
    def get_peak_usage(self) -> Dict[str, float]:
        ...
        peak_gpu_allocated = max(s.gpu_memory_allocated_mb for s in self.snapshots) if self.has_gpu else 0
        peak_gpu_percent = max(s.gpu_memory_percent for s in self.snapshots) if self.has_gpu else 0
