...
class DataValidator:
    ...
    def validate_features(self, features: torch.Tensor, feature_names: Optional[List[str]] = None) -> List[ValidationResult]:
        ...
        if self.rules["tensor_dtype_check"].enabled:
            results.append(self._check_tensor_dtype(features, "features", torch.float32))
    ...
    def validate_labels(self, labels: torch.Tensor, num_classes: int = 3) -> List[ValidationResult]:
    ...
    def validate_symbols(self, symbol_indices: torch.Tensor, symbols: List[str]) -> List[ValidationResult]:
    ...
    def validate_tensor_consistency(self, *tensors: torch.Tensor) -> List[ValidationResult]:
    ...
    def sanitize_features(self, features: torch.Tensor) -> Tuple[torch.Tensor, List[str]]:
        ...
        if nan_mask.any():
            ...
            fixes_applied.append(f"Replaced {nan_mask.sum().item()} NaN values with zeros")
        ...
        if extreme_mask.any():
            ...
            fixes_applied.append(f"Clipped {extreme_mask.sum().item()} extreme values to ±{extreme_threshold}")
        
        return sanitized, fixes_applied
    
    def sanitize_labels(self, labels: torch.Tensor, num_classes: int = 3) -> Tuple[torch.Tensor, List[str]]:
        ...
        if invalid_mask.any():
            ...
            fixes_applied.append(f"Fixed {invalid_mask.sum().item()} out-of-range labels")
    ...
    def _check_infinite_values(self, tensor: torch.Tensor, tensor_name: str) -> ValidationResult:
        """Check for infinite values."""
        inf_mask = torch.isinf(tensor)
        inf_count = inf_mask.sum().item()
        
        if inf_count > 0:
            affected_indices = torch.where(inf_mask.any(dim=1) if tensor.dim() > 1 else inf_mask)[0].tolist()
    ...
    def _check_nan_values(self, tensor: torch.Tensor, tensor_name: str) -> ValidationResult:
        """Check for NaN values."""
        nan_mask = torch.isnan(tensor)
        nan_count = nan_mask.sum().item()
        
        if nan_count > 0:
            affected_indices = torch.where(nan_mask.any(dim=1) if tensor.dim() > 1 else nan_mask)[0].tolist()
            return ValidationResult(
    ...
    def _check_feature_variance(self, features: torch.Tensor, feature_names: Optional[List[str]]) -> ValidationResult:
        """Check for low variance features."""
        feature_vars = torch.var(features, dim=0)
        low_variance_threshold = 1e-6
        low_var_features = torch.where(feature_vars < low_variance_threshold)[0].tolist()
    ...
    def _check_label_range(self, labels: torch.Tensor, num_classes: int) -> ValidationResult:
    ...
    def _check_label_distribution(self, labels: torch.Tensor, num_classes: int) -> ValidationResult:
        ...
        if min_prop < 0.05 or max_prop > 0.8:
            distribution_info = {f"class_{i}": f"{count} ({prop:.1%})" 
                               for i, (count, prop) in enumerate(zip(class_counts.tolist(), class_props.tolist()))}
    ...
    def _check_symbol_indices(self, symbol_indices: torch.Tensor, num_symbols: int) -> ValidationResult:
        """Check symbol indices validity."""
    ...
    def _check_symbol_balance(self, symbol_indices: torch.Tensor, symbols: List[str]) -> ValidationResult:
        """Check symbol balance."""
        ...
        if min_prop < 0.15 or max_prop > 0.6:  # More lenient than class balance
            distribution_info = {}
            for i, (count, prop) in enumerate(zip(symbol_counts.tolist(), symbol_props.tolist())):
    ...
    def _check_tensor_shape_consistency(self, tensors: Tuple[torch.Tensor, ...]) -> ValidationResult:
    ...
    def _check_tensor_dtype(self, tensor: torch.Tensor, tensor_name: str, expected_dtype: torch.dtype) -> ValidationResult:
    ...
    def _check_tensor_size(self, tensor: torch.Tensor, tensor_name: str) -> ValidationResult:
    ...
    def _check_timeframe_naming(self, feature_names: List[str]) -> ValidationResult:
        ...
        common_timeframes = ['1m', '5m', '15m', '30m', '1h', '2h', '4h', '8h', '12h', '1d', '1w']
    ...
    def comprehensive_validation(self, 
        ...
        error_count = sum(1 for result in all_results if not result.passed and result.severity == "error")
        warning_count = sum(1 for result in all_results if not result.passed and result.severity == "warning")
        ...
        report = {
            ...
            'data_summary': {
                ...
                'symbol_indices_shape': list(symbol_indices.shape) if symbol_indices is not None else None,
            ...
            'failed_validations': [result.to_dict() for result in all_results if not result.passed],
        ...
        # Log summary
        if error_count > 0:
            logger.error(f"Validation completed with {error_count} errors and {warning_count} warnings")
    ...
    def _generate_recommendations(self, results: List[ValidationResult]) -> List[str]:
        ...
        if any(r.rule_name in ["no_infinite_values", "no_nan_values"] for r in error_results):
            recommendations.append("Critical data quality issues detected. Clean data before training.")
        
        if any(r.rule_name == "feature_variance_check" for r in warning_results):
            recommendations.append("Remove low-variance features to improve model performance.")
        ...
        if any(r.rule_name == "feature_range_check" for r in warning_results):
            recommendations.append("Apply feature scaling/normalization before training.")
