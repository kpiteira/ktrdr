...
class ProductionAlert:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'resolution_datetime': datetime.fromtimestamp(self.resolution_time).isoformat() if self.resolution_time else None
        }
...
class ProductionErrorHandler:
    ...
    def __init__(self, 
        ...
        self.system_health_history: deque = deque(maxlen=288)  # 24 hours at 5-min intervals
        ...
        logger.info(f"ProductionErrorHandler initialized on {self.hostname} (PID: {self.process_id})")
    ...
    def _setup_production_logging(self):
        ...
        json_formatter = logging.Formatter(
            '{"timestamp": "%(asctime)s", "level": "%(levelname)s", "component": "%(name)s", "message": "%(message)s", "hostname": "' + self.hostname + '", "pid": ' + str(self.process_id) + '}'
        )
    ...
    def _capture_system_health(self) -> SystemHealth:
        ...
        try:
            for p in psutil.process_iter(['name', 'cmdline']):
                try:
                    if (p.info['name'] and 'python' in p.info['name'].lower() and 
                        p.info['cmdline'] and 'train' in ' '.join(p.info['cmdline']).lower()):
    ...
    def _check_system_health_alerts(self, health: SystemHealth):
        ...
        if not health.training_active and health.uptime_seconds > 3600:  # After 1 hour uptime
            self.send_alert(
                AlertLevel.WARNING,
                "TrainingStatus",
                "Training appears to be inactive",
                {"training_active": False, "uptime_hours": health.uptime_seconds / 3600},
                health
            )
    ...
    def handle_production_error(self,
                              error: Exception,
                              component: str,
                              operation: str,
    ...
    def _send_console_alert(self, alert: ProductionAlert):
        ...
        print(f"{color}[{alert.level.value.upper()}] {datetime.fromtimestamp(alert.timestamp).isoformat()} "
              f"{alert.component}: {alert.message}{reset_color}")
    
    def _send_file_alert(self, alert: ProductionAlert):
        """Send alert to file."""
        alert_file = Path(self.config.alert_file_path) if self.config.alert_file_path else self.log_dir / "alerts.log"
    ...
    def _send_email_alert(self, alert: ProductionAlert):
        ...
        try:
            ...
            msg['Subject'] = f"[{alert.level.value.upper()}] KTRDR Production Alert - {alert.component}"
...
"""
    ...
    def get_production_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'error_rate': {
                ...
                'average_per_minute': sum(self.error_count_history) / max(len(self.error_count_history), 1)
            },
            'configuration': {
    ...
    def export_production_report(self, file_path: Optional[Path] = None) -> Path:
        ...
        report_data = {
            ...
            'recent_alerts': [alert.to_dict() for alert in list(self.alerts)[-100:]],  # Last 100 alerts
            'system_health_history': [health.to_dict() for health in list(self.system_health_history)],
