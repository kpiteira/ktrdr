...
class PerformanceConfig:
    ...
    def __post_init__(self):
        ...
        if self.num_workers > cpu_count:
            ...
            logger.info(f"Adjusted num_workers to {self.num_workers} based on CPU count")
...
class PerformanceMetrics:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'throughput': {
                'mean_samples_per_second': np.mean(self.samples_per_second) if self.samples_per_second else 0,
                'max_samples_per_second': np.max(self.samples_per_second) if self.samples_per_second else 0
            },
            'batch_size': {
            ...
            'memory_usage': {
                'peak_mb': np.max(self.memory_usage_peak_mb) if self.memory_usage_peak_mb else 0,
                'mean_mb': np.mean(self.memory_usage_peak_mb) if self.memory_usage_peak_mb else 0
            },
            'gpu_utilization': {
                'mean_percent': np.mean(self.gpu_utilization_percent) if self.gpu_utilization_percent else 0,
                'max_percent': np.max(self.gpu_utilization_percent) if self.gpu_utilization_percent else 0
            }
        }
...
class PerformanceOptimizer:
    ...
    def optimize_model(self, model: nn.Module) -> nn.Module:
        ...
        if self.config.use_multiple_gpus and torch.cuda.device_count() > 1:
            if torch.cuda.device_count() > 1:
                ...
                logger.info(f"DataParallel enabled for {torch.cuda.device_count()} GPUs")
    ...
    def find_optimal_batch_size(self, 
        ...
        while current_batch_size <= self.config.max_batch_size:
            try:
                ...
                # Increase batch size
                current_batch_size = int(current_batch_size * self.config.batch_size_growth_factor)
                
            except RuntimeError as e:
                if "out of memory" in str(e).lower() or "cuda out of memory" in str(e).lower():
                    logger.info(f"OOM at batch size {current_batch_size}, using {max_successful_batch_size}")
                    break
                else:
        ...
        optimal_batch_size = max(self.config.min_batch_size, int(max_successful_batch_size * 0.8))
    ...
    def training_step(self, 
                     ...
                     criterion: nn.Module,
                     optimizer: torch.optim.Optimizer) -> Tuple[torch.Tensor, Dict[str, float]]:
    ...
    def get_optimization_recommendations(self) -> List[str]:
        ...
        total_time = (summary.get('training_time', {}).get('mean_seconds', 0) + 
                     summary.get('validation_time', {}).get('mean_seconds', 0))
        
        if total_time > 0 and data_time / total_time > 0.2:  # > 20% of time in data loading
            recommendations.append("Data loading is slow - consider increasing num_workers")
