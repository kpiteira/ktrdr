...
class EfficientMultiSymbolDataset(Dataset):
    ...
    def __init__(self, 
        ...
        logger.info(f"EfficientMultiSymbolDataset initialized: "
    ...
    def get_symbol_batch(self, symbol: str, batch_size: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if len(symbol_indices) < batch_size:
            ...
            selected_indices = symbol_indices.repeat((batch_size // len(symbol_indices)) + 1)[:batch_size]
        else:
            selected_indices = symbol_indices[torch.randperm(len(symbol_indices))[:batch_size]]
    ...
    def get_balanced_batch(self, batch_size: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for class_idx in range(self.num_classes):
            ...
            if len(class_indices) < num_samples:
                # Repeat indices if not enough samples
                class_selected = class_indices.repeat((num_samples // len(class_indices)) + 1)[:num_samples]
            else:
                class_selected = class_indices[torch.randperm(len(class_indices))[:num_samples]]
...
class SymbolBalancedSampler(Sampler):
    ...
    def __init__(self, dataset: EfficientMultiSymbolDataset, batch_size: int, drop_last: bool = True):
        ...
        min_symbol_samples = min(len(indices) for indices in dataset.symbol_to_indices.values())
        ...
        logger.debug(f"SymbolBalancedSampler: {self.num_batches} batches, "
            ...
            for batch in self.dataloader:
                ...
                if self.device is not None:
                    if isinstance(batch, (list, tuple)):
                        batch = tuple(item.to(self.device, non_blocking=True) if torch.is_tensor(item) else item 
                                    for item in batch)
...
class DataLoadingOptimizer:
    ...
    def create_optimized_dataset(self,
                                ...
                                feature_names: List[str],
                                symbols: List[str],
                                timeframes: List[str]) -> EfficientMultiSymbolDataset:
    ...
    def create_optimized_dataloader(self,
                                  ...
                                  batch_size: int,
                                  device: Optional[torch.device] = None,
                                  shuffle: bool = True) -> Union[DataLoader, PrefetchingDataLoader]:
    ...
    def benchmark_dataloader(self, dataloader: Union[DataLoader, PrefetchingDataLoader], 
    ...
    def get_data_statistics(self, dataset: EfficientMultiSymbolDataset) -> Dict[str, Any]:
        ...
        for i, symbol in enumerate(dataset.symbols):
            ...
            for class_idx in range(dataset.num_classes):
                count = ((dataset.symbol_indices == i) & (dataset.label_tensor == class_idx)).sum().item()
