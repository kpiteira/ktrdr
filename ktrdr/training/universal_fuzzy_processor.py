...
class UniversalFuzzyNeuralProcessor:
    ...
    def prepare_universal_input(
        ...
        """
        logger.debug(f"Creating universal features from {len(fuzzy_data)} fuzzy samples")
    ...
    def _create_universal_features(self, indicators: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'rsi' in indicators.columns:
            ...
            universal_features['rsi_extreme_signal'] = np.where(
                rsi > 70, (rsi - 70) / 30,  # Overbought: 0 to 1
        ...
        if 'BollingerBands' in indicators.columns and 'close' in indicators.columns:
            ...
            universal_features['bb_position'] = (close - bb) / (bb * 0.04)  # Assuming 2% bands
            universal_features['bb_position'] = np.clip(universal_features['bb_position'], -2, 2)
        ...
        logger.debug(f"Created {len(universal_features.columns)} universal indicator features")
    ...
    def _add_universal_temporal_features(self, fuzzy_data: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.debug(f"Added temporal features, total: {len(temporal_features.columns)}")
