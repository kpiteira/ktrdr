...
class FeatureEngineer:
    ...
    def prepare_features(
        ...
        if self.config.get("include_volume_context", True):
            volume_features, volume_names = self._extract_volume_features(price_data)
        ...
        for i, feat_array in enumerate(features):
            if feat_array.size > 0:
                if i == 0:  # fuzzy features
                    feature_names_filtered.extend(
                        feature_names[start_idx : start_idx + feat_array.shape[1]]
                    )
                else:
                    # Calculate how many names belong to this feature group
                    feat_count = feat_array.shape[1] if len(feat_array.shape) > 1 else 1
                    feature_names_filtered.extend(
                        feature_names[start_idx : start_idx + feat_count]
                    )
    ...
    def _extract_price_features(
        self, price_data: pd.DataFrame, indicators: pd.DataFrame
    ) -> Tuple[np.ndarray, List[str]]:
        ...
        if all(col in price_data.columns for col in ["high", "low"]):
            daily_position = (close - price_data["low"]) / (
    ...
    def calculate_feature_importance(
        self, model, X: torch.Tensor, y: torch.Tensor
    ) -> Dict[str, float]:
