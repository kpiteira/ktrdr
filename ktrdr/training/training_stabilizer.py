...
class TrainingStabilizer:
    ...
    def __init__(self, 
        ...
        logger.info(f"TrainingStabilizer initialized, checkpoint dir: {self.checkpoint_dir}")
    ...
    def save_checkpoint(self,
        ...
        checkpoint_name = f"checkpoint_epoch_{epoch:04d}_step_{step:06d}_{int(timestamp)}.pt"
        ...
        checkpoint_data = {
            ...
            'stability_metrics': self.stability_history[-1].to_dict() if self.stability_history else {}
        }
        ...
        try:
            ...
            recovery_action = self.error_handler.handle_error(
                e, "TrainingStabilizer", "save_checkpoint",
    ...
    def load_checkpoint(self, checkpoint_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            recovery_action = self.error_handler.handle_error(
                e, "TrainingStabilizer", "load_checkpoint",
                {'checkpoint_path': str(checkpoint_path)}
            )
            
            if recovery_action == RecoveryAction.FALLBACK:
                # Try to load last stable checkpoint
                if self.last_stable_checkpoint and self.last_stable_checkpoint != checkpoint_path:
                    logger.warning(f"Loading fallback checkpoint: {self.last_stable_checkpoint}")
    ...
    def resume_training(self, 
                       model: nn.Module,
                       optimizer: torch.optim.Optimizer,
                       checkpoint_path: Optional[Path] = None) -> Tuple[int, int, Dict[str, Any]]:
        ...
        self.current_status = TrainingStatus(checkpoint_data.get('training_status', 'stable'))
        
        logger.info(f"Training resumed from epoch {epoch}, step {step}")
        return epoch, step, metrics
    
    def monitor_training_stability(self,
                                 loss: float,
                                 gradients: Optional[torch.Tensor] = None,
                                 learning_rate: float = 0.001) -> StabilityMetrics:
    ...
    def _calculate_stability_metrics(self, 
                                   current_loss: float,
                                   grad_norm: float,
                                   learning_rate: float) -> StabilityMetrics:
    ...
    def _update_training_status(self, metrics: StabilityMetrics):
        ...
        elif abs(metrics.convergence_rate) < stall_threshold and len(self.loss_history) > 20:
        ...
        if self.current_status != TrainingStatus.STABLE and previous_status == TrainingStatus.STABLE:
            self.instability_count += 1
            logger.warning(f"Training instability detected: {self.current_status.value}")
        
        # Log status changes
        if self.current_status != previous_status:
            logger.info(f"Training status changed: {previous_status.value} → {self.current_status.value}")
    ...
    def auto_recovery(self,
        ...
        try:
            ...
            elif self.current_status in [TrainingStatus.UNSTABLE, TrainingStatus.STALLED]:
            ...
            self.error_handler.handle_error(
                e, "TrainingStabilizer", "auto_recovery",
                {'recovery_attempts': self.recovery_attempts, 'status': self.current_status.value}
            )
            return False
    ...
    def get_training_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'training_health': {
                ...
                'instability_events': self.instability_count,
                'recovery_attempts': self.recovery_attempts,
                'last_stable_checkpoint': str(self.last_stable_checkpoint) if self.last_stable_checkpoint else None
            },
