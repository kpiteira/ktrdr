...
class TrainingAnalyzer:
    ...
    def __init__(self, run_id: str, output_dir: Path, config: dict[str, Any]):
        ...
        analytics_config = (
            config.get("model", {}).get("training", {}).get("analytics", {})
        )
    ...
    def collect_epoch_metrics(
        ...
        try:
            ...
            prediction_metrics = self.metrics_collector.collect_prediction_metrics(
                model_outputs
            )
            ...
            detailed_metrics = DetailedTrainingMetrics(
                ...
                high_confidence_predictions=prediction_metrics[
            ...
            detailed_metrics.learning_signal_strength = learning_indicators[
                "learning_signal_strength"
            ]
            detailed_metrics.overfitting_score = learning_indicators[
                "overfitting_score"
            ]
            detailed_metrics.class_balance_score = learning_indicators[
                "class_balance_score"
            ]
            detailed_metrics.convergence_indicator = learning_indicators[
            ...
            logger.debug(
                f"Collected metrics for epoch {epoch}: {detailed_metrics.learning_signal_strength} signal"
            )
            ...
            basic_metrics = DetailedTrainingMetrics(
                ...
                val_accuracy=val_metrics.get("accuracy"),
                learning_rate=(
                    optimizer.param_groups[0]["lr"] if optimizer.param_groups else 0.001
                ),
            )
    ...
    def check_alerts(self, metrics: DetailedTrainingMetrics) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if (
                ...
                and metrics.val_accuracy < 0.4
                and metrics.epoch > 10
            ):
                alerts.append(
                    {
                        "epoch": metrics.epoch,
        ...
        self.final_analysis = {
            ...
            "best_epoch": (
                max(
                    ...
                    key=lambda x: (
                        x[1].val_accuracy if x[1].val_accuracy is not None else 0.0
                    ),
                )[0]
                + 1
            ...
            "final_gradient_norm": (
                self.metrics_history[-1].gradient_norm_avg
                if self.metrics_history
                else 0.0
            ),
    ...
    def export_json(self) -> Path:
        ...
        try:
            ...
            json_data = {
                ...
                "epoch_metrics": [
                    metrics.to_json_dict() for metrics in self.metrics_history
                ],
                "alerts": self.alerts,
            ...
            with open(alerts_path, "w") as f:
                ...
                if not self.alerts:
                    ...
                    for alert in self.alerts:
                        f.write(
                            f"Epoch {alert['epoch']} [{alert['severity'].upper()}]: {alert['message']}\n"
                        )
