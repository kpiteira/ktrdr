...
class ErrorHandler:
    ...
    def add_recovery_strategy(self, strategy: RecoveryStrategy):
        ...
        logger.debug(f"Added recovery strategy for {[e.__name__ for e in strategy.error_types]}")
    
    def handle_error(self, 
                    error: Exception,
                    component: str,
                    operation: str,
                    additional_data: Optional[Dict[str, Any]] = None) -> RecoveryAction:
        ...
        if strategy and recovery_action in [RecoveryAction.RETRY, RecoveryAction.FALLBACK]:
    ...
    def _determine_recovery_action(self, 
                                 error_context: ErrorContext, 
                                 strategy: Optional[RecoveryStrategy]) -> RecoveryAction:
        ...
        return strategy.recovery_actions[0] if strategy.recovery_actions else RecoveryAction.CONTINUE
        ...
        with self.recovery_lock:
            ...
            try:
                if action == RecoveryAction.RETRY:
                    logger.info(f"Retrying {error_context.operation} (attempt {error_context.retry_count + 1})")
    ...
    def get_error_summary(self) -> Dict[str, Any]:
        ...
        for component, health in self.component_health.items():
            component_summary[component] = {
                ...
                'most_common_error': max(health['error_types'].items(), key=lambda x: x[1])[0] if health['error_types'] else None
            }
        ...
        return {
            ...
            'most_recent_errors': [error.to_dict() for error in self.error_history[-5:]]
        }
