...
class GPUMemoryManager:
    ...
    def __init__(self, config: Optional[GPUMemoryConfig] = None, device_ids: Optional[List[int]] = None):
        ...
        while self.monitoring_active:
            try:
                for device_id in self.device_ids:
                    ...
                    if device_id not in self.peak_memory_usage or current_usage > self.peak_memory_usage[device_id]:
                        self.peak_memory_usage[device_id] = current_usage
                    
                    # Check for cleanup
                    if snapshot.free_mb < self.config.cleanup_threshold_mb:
                        logger.debug(f"GPU {device_id} low memory ({snapshot.free_mb:.1f}MB free) - cleaning up")
    ...
    def memory_efficient_context(self, device_id: int = 0):
        ...
        try:
            ...
            if memory_diff > 100 * 1024 * 1024:  # > 100MB difference
                logger.warning(f"Potential memory leak detected: {memory_diff / (1024**2):.1f}MB")
    
    @contextmanager
    def mixed_precision_context(self):
        """Context manager for mixed precision operations."""
        if not self.enabled or not self.config.enable_mixed_precision or self.scaler is None:
            yield None
            return
    ...
    def optimize_batch_size(self, 
        ...
        while current_batch_size <= max_batch_size:
            try:
                ...
                # Create test batch
                features, labels = sample_batch[0][:current_batch_size], sample_batch[1][:current_batch_size]
                ...
                if "out of memory" in str(e).lower():
                    logger.info(f"OOM at batch size {current_batch_size}, optimal: {optimal_batch_size}")
                    break
                else:
                    raise e
            except Exception as e:
    ...
    def get_optimization_recommendations(self) -> List[str]:
        ...
        for device_id in self.device_ids:
            ...
            if snapshot.utilization_percent > 90:
                recommendations.append(f"GPU {device_id}: Very high memory usage ({snapshot.utilization_percent:.1f}%) - consider reducing batch size")
            elif snapshot.utilization_percent < 50:
                recommendations.append(f"GPU {device_id}: Low memory usage ({snapshot.utilization_percent:.1f}%) - consider increasing batch size")
            
            # Temperature recommendations
            if snapshot.temperature_celsius and snapshot.temperature_celsius > 80:
                recommendations.append(f"GPU {device_id}: High temperature ({snapshot.temperature_celsius}°C) - check cooling")
            
            # Power recommendations
            if snapshot.power_usage_watts and snapshot.power_usage_watts > 250:
                recommendations.append(f"GPU {device_id}: High power usage ({snapshot.power_usage_watts}W) - monitor for efficiency")
        
        # General recommendations
        if not self.config.enable_mixed_precision:
            recommendations.append("Consider enabling mixed precision training for memory efficiency")
        
        if not self.config.enable_memory_pooling:
            recommendations.append("Consider enabling memory pooling for better allocation")
        ...
        def __enter__(self):
            if self.manager.enabled:
                for device_id in self.manager.device_ids:
                    self.start_snapshots[device_id] = self.manager.capture_snapshot(device_id)
            return self
        ...
        def __exit__(self, exc_type, exc_val, exc_tb):
            if self.manager.enabled:
                for device_id in self.manager.device_ids:
                    ...
                    memory_delta = end_snapshot.allocated_mb - start_snapshot.allocated_mb
                    
                    if abs(memory_delta) > 1.0:  # > 1MB change
                        logger.debug(f"Memory profile '{self.name}' GPU {device_id}: "
