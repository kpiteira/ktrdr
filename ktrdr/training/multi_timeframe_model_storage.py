...
class MultiTimeframeModelStorage(ModelStorage):
    ...
    def save_multi_timeframe_model(
        ...
        """
        logger.info(f"Saving multi-timeframe model: {strategy_name}/{symbol} ({timeframes})")
        ...
        try:
            ...
            self._save_feature_engineering_results(feature_engineering_result, model_dir)
            ...
            self._save_multi_timeframe_metadata(
                strategy_name, symbol, timeframes, model, config,
                training_result, feature_engineering_result, label_result,
        ...
        try:
            ...
            if load_full_results:
                result.update({
                    ...
                    "feature_engineering_result": self._load_feature_engineering_results(model_dir),
    ...
    def list_multi_timeframe_models(
        ...
        if strategy_name:
            ...
            search_dirs = [self.base_path / strategy_name] if (self.base_path / strategy_name).exists() else []
        else:
    ...
    def _save_feature_engineering_results(
        ...
        feature_data = {
            ...
            "selected_features_mask": (
                ...
                if feature_result.selected_features_mask is not None else None
            ),
    ...
    def _save_multi_timeframe_metadata(
        ...
        performance_summary = {
            "final_train_loss": (
                ...
                if training_result.training_history.get("train_loss") else None
            ),
            "final_val_loss": (
                training_result.training_history.get("val_loss", [])[-1]
                if training_result.training_history.get("val_loss") else None
            ),
            "final_train_accuracy": (
                training_result.training_history.get("train_accuracy", [])[-1]
                if training_result.training_history.get("train_accuracy") else None
            ),
            "final_val_accuracy": (
                training_result.training_history.get("val_accuracy", [])[-1]
                if training_result.training_history.get("val_accuracy") else None
            ),
        ...
        label_quality = {
            ...
            "validation_rate": (
                label_result.metadata.get("validation_statistics", {})
                .get("validation_rate", 0.0)
            )
        }
        ...
        metadata = {
            ...
            "feature_summary": {
                "original_feature_count": (
                    feature_result.transformation_metadata.get("original_feature_count", 0)
                    if feature_result.transformation_metadata else 0
                ),
                "final_feature_count": len(feature_result.feature_names),
                "feature_names": feature_result.feature_names,
                "scaling_method": (
                    feature_result.transformation_metadata.get("scaling_method", "none")
                    if feature_result.transformation_metadata else "none"
                ),
                "selection_method": (
                    feature_result.transformation_metadata.get("selection_method", "none")
                    if feature_result.transformation_metadata else "none"
                )
            },
    ...
    def _find_any_latest_version(self, strategy_dir: Path, symbol: str) -> Path:
        ...
        matching_dirs = [
            d for d in strategy_dir.glob(pattern) 
    ...
    def _load_model_files(self, model_dir: Path) -> torch.nn.Module:
        ...
        try:
            ...
            return torch.load(model_dir / "model_full.pt", map_location="cpu", weights_only=False)
        except Exception as e:
            logger.warning(f"Failed to load full model: {e}, trying state dict")
            
            try:
                # Try loading just state dict
                state_dict = torch.load(model_dir / "model_state_dict.pt", map_location="cpu", weights_only=True)
    ...
    def _load_label_results(self, model_dir: Path) -> Dict[str, Any]:
        ...
        for csv_file in model_dir.glob("labels_*.csv"):
            ...
            if timeframe != "labels":  # Skip main labels.csv
                timeframe_labels[timeframe] = pd.read_csv(csv_file, index_col=0).squeeze()
...
def create_multi_timeframe_model_storage(base_path: str = "models") -> MultiTimeframeModelStorage:
    """
