...
class MultiTimeframeFuzzyEngine(FuzzyEngine):
    ...
    def __init__(self, config: Union[FuzzyConfig, Dict[str, Any]]):
        ...
        if isinstance(config, FuzzyConfig):
            logger.info("Backward compatibility mode: converting single-timeframe config")
    ...
    def _validate_multi_timeframe_config(self) -> None:
        ...
        logger.debug(f"Validated multi-timeframe config with timeframes: {list(timeframes.keys())}")
                ...
                for set_name, mf_config in sets.items():
                    ...
                    if mf_type == "triangular":
                        ...
                        raise ConfigurationError(
                            ...
                            details={"type": mf_type, "supported": ["triangular", "trapezoidal", "gaussian"]}
                        )
                    
                    fuzzy_sets[key] = FuzzySetConfig(root={
                        set_name: mf_instance
                    })
        ...
        for tf_name, tf_config in self._timeframe_configs.items():
            ...
            for indicator in tf_config.indicators:
                ...
                if indicator_fuzzy_sets:
                    merged_indicators[tf_indicator] = FuzzySetConfig(root=indicator_fuzzy_sets)
    ...
    def fuzzify_multi_timeframe(
        ...
        logger.debug(f"Processing multi-timeframe fuzzy logic for {len(indicator_data)} timeframes")
        ...
        for timeframe in timeframes_to_process:
            try:
                tf_result = self._process_timeframe(timeframe, indicator_data[timeframe])
    ...
    def _validate_indicator_data(self, indicator_data: Dict[str, Dict[str, float]]) -> None:
        """
    ...
    def _get_timeframes_to_process(
        ...
        if self._is_multi_timeframe:
            ...
            enabled_timeframes = {
                tf for tf, config in self._timeframe_configs.items() 
                if config.enabled
            }
            # Only process timeframes that are available, configured, and enabled
            candidate_timeframes = available_timeframes.intersection(configured_timeframes).intersection(enabled_timeframes)
        else:
        ...
        if timeframe_filter:
            candidate_timeframes = candidate_timeframes.intersection(set(timeframe_filter))
    ...
    def _process_timeframe(
        ...
        for indicator in configured_indicators:
            if indicator not in indicators:
                logger.warning(f"Indicator {indicator} not available for timeframe {timeframe}")
                continue
            ...
            try:
                ...
                tf_indicator = f"{indicator}_{timeframe}" if self._is_multi_timeframe else indicator
                ...
                for fuzzy_name, membership in fuzzy_result.items():
                    if self._is_multi_timeframe and fuzzy_name.endswith(f"_{timeframe}"):
                ...
                logger.error(f"Failed to fuzzify {indicator} for timeframe {timeframe}: {e}")
...
def create_multi_timeframe_fuzzy_engine(config: Dict[str, Any]) -> MultiTimeframeFuzzyEngine:
    """
