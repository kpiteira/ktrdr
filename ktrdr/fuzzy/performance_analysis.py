...
class PerformanceMetrics:
    ...
    def __post_init__(self):
        ...
        memory_efficiency = max(0, 100 - (self.memory_peak_mb - self.memory_start_mb) / 10)  # Penalty for high memory usage
        time_balance = 100 if self.total_time > 0 else 0
        
        self.optimization_score = (throughput_score + memory_efficiency + time_balance) / 3
...
class FuzzyPerformanceAnalyzer:
    ...
    def benchmark_pipeline_performance(
        ...
        """
        logger.info(f"Starting performance benchmark with {len(test_configurations)} configurations")
        
        results = []
        
        for i, config in enumerate(test_configurations):
            logger.info(f"Benchmarking configuration {i+1}/{len(test_configurations)}: {config.get('name', 'Unnamed')}")
            ...
            for run in range(benchmark_runs):
                try:
                    metrics = self._measure_single_run(
                        service, symbol, indicator_config, fuzzy_config, 
                        timeframes, data_period_days
                    )
                    run_metrics.append(metrics)
                    logger.debug(f"Run {run+1}: {metrics.total_time:.3f}s, {metrics.fuzzy_values_count} FV")
            ...
            if run_metrics:
                ...
                logger.info(f"Configuration {i+1} average: {avg_metrics.total_time:.3f}s, "
    ...
    def _average_metrics(self, metrics_list: List[PerformanceMetrics]) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(
            ...
            timeframes_processed=int(sum(m.timeframes_processed for m in metrics_list) / n),
    ...
    def analyze_scalability(
        ...
        for data_days in data_sizes:
            ...
            try:
                ...
                scalability_data.append({
                    ...
                    'memory_usage': metrics.memory_peak_mb - metrics.memory_start_mb,
        ...
        time_growth_rate = self._calculate_growth_rate(df['data_days'], df['total_time'])
        memory_growth_rate = self._calculate_growth_rate(df['data_days'], df['memory_usage'])
    ...
    def generate_performance_report(
        ...
        all_throughputs = [r.metrics.throughput_fv_per_sec for r in self.benchmark_results]
        ...
        for result in self.benchmark_results:
            report["benchmark_results"].append({
                ...
                "memory_usage": result.metrics.memory_peak_mb - result.metrics.memory_start_mb,
    ...
    def _summarize_config(self, config: Dict[str, Any]) -> str:
        ...
        if 'timeframes' in config:
            timeframes = list(config['timeframes'].keys()) if isinstance(config['timeframes'], dict) else config['timeframes']
            if timeframes:
