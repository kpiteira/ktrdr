...
class FuzzyEngine:
    ...
    def __init__(self, config: FuzzyConfig):
        ...
        logger.info(f"FuzzyEngine initialized with {len(self._membership_functions)} indicators")
    ...
    def _initialize_membership_functions(self) -> None:
        ...
        for indicator, fuzzy_sets in self._config.root.items():
            logger.debug(f"Initializing membership functions for indicator: {indicator}")
            self._membership_functions[indicator] = {}
            
            for set_name, mf_config in fuzzy_sets.root.items():
                try:
                    if mf_config.type == "triangular":
                        self._membership_functions[indicator][set_name] = TriangularMF(mf_config.parameters)
                    else:
                        # This should never happen due to the Literal constraint in MembershipFunctionConfig
                        logger.error(f"Unknown membership function type: {mf_config.type}")
                        raise ConfigurationError(
                            message=f"Unknown membership function type: {mf_config.type}",
                            error_code="ENGINE-UnknownMFType",
                            details={"indicator": indicator, "set_name": set_name, "type": mf_config.type}
                        )
                except Exception as e:
                    logger.error(f"Failed to initialize membership function for {indicator}.{set_name}: {e}")
                    raise ConfigurationError(
                        message=f"Failed to initialize membership function for {indicator}.{set_name}",
                        error_code="ENGINE-MFInitializationError",
                        details={"indicator": indicator, "set_name": set_name, "original_error": str(e)}
                    ) from e
    
    def fuzzify(self, indicator: str, values: Union[float, pd.Series, np.ndarray]) -> Union[Dict[str, float], pd.DataFrame]:
        """
        ...
        if indicator not in self._membership_functions:
            ...
            raise ProcessingError(
                ...
                details={"indicator": indicator, "available_indicators": list(self._membership_functions.keys())}
            )
        ...
        # Handle pandas Series input
            ...
            logger.debug(f"Fuzzifying pandas Series of length {len(values)} for indicator {indicator}")
            return self._fuzzify_series(indicator, values, membership_functions)
        
        # Handle numpy array input
        elif isinstance(values, np.ndarray):
            logger.debug(f"Fuzzifying numpy array of shape {values.shape} for indicator {indicator}")
        ...
        # Handle unsupported input types
            ...
            raise TypeError(f"Unsupported input type: {type(values)}. Expected float, pd.Series, or np.ndarray.")
    
    def _fuzzify_scalar(self, indicator: str, value: float, 
                         membership_functions: Dict[str, MembershipFunction]) -> Dict[str, float]:
        """
    ...
    def _fuzzify_series(self, indicator: str, values: pd.Series, 
                        membership_functions: Dict[str, MembershipFunction]) -> pd.DataFrame:
        """
