...
class BatchFuzzyCalculator:
    ...
    def calculate_memberships(
        ...
        if cached_result is not None:
            ...
            return self._restore_result_format(cached_result, values_series.index, nan_mask)
        ...
        # Restore NaN values and original index
        final_result = self._restore_result_format(result, values_series.index, nan_mask)
        
        total_time = time.time() - start_time
        logger.debug(f"Total batch calculation time for {indicator_name}: {total_time:.3f}s")
    ...
    def _handle_nan_values(self, values_series: pd.Series) -> Tuple[pd.Series, pd.Series]:
        """
        ...
        if nan_mask.any():
            logger.debug(f"Found {nan_mask.sum()} NaN values, will be excluded from calculation")
    ...
    def _compute_memberships(self, indicator_name: str, clean_series: pd.Series) -> Dict[str, pd.Series]:
        """
    ...
    def _create_empty_result(self, indicator_name: str, index: pd.Index) -> Dict[str, pd.Series]:
        """
    ...
    def get_cache_stats(self) -> Dict[str, int]:
        ...
        """
        cache_info = self._get_cached_result.cache_info() if hasattr(self._get_cached_result, 'cache_info') else None
