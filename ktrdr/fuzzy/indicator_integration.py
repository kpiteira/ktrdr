...
from ktrdr.indicators.multi_timeframe_indicator_engine import MultiTimeframeIndicatorEngine, TimeframeIndicatorConfig
from ktrdr.fuzzy.multi_timeframe_engine import MultiTimeframeFuzzyEngine, MultiTimeframeFuzzyResult
...
class MultiTimeframeFuzzyIndicatorPipeline:
    ...
    def __init__(
        ...
        try:
            ...
            logger.debug(f"Initialized indicator engine with {timeframes_count} timeframes")
            
            # Initialize the fuzzy engine
            self.fuzzy_engine = MultiTimeframeFuzzyEngine(fuzzy_config)
            logger.debug(f"Initialized fuzzy engine with {len(self.fuzzy_engine.get_supported_timeframes())} timeframes")
    ...
    def _validate_configuration_compatibility(self) -> None:
        ...
        logger.info(f"Validated compatibility: {len(indicator_timeframes.intersection(fuzzy_timeframes))} common timeframes")
    
    def _create_indicator_engine(self, indicator_config: Dict[str, Any]) -> MultiTimeframeIndicatorEngine:
        """
        ...
        for timeframe, config in indicator_config["timeframes"].items():
            if not isinstance(config, dict):
                raise ConfigurationError(
                    ...
                    details={"timeframe": timeframe, "config_type": type(config).__name__}
                )
    ...
    def process_market_data(
        ...
        logger.info(f"Starting integrated pipeline processing for {len(market_data)} timeframes")
        ...
        try:
            ...
            try:
                indicator_results = self.indicator_engine.apply_multi_timeframe(market_data)
                indicator_processing_time = time.time() - indicator_start
                
                processing_metadata["indicator_processing_time"] = indicator_processing_time
                processing_metadata["processed_indicator_timeframes"] = list(indicator_results.keys())
                
                logger.debug(f"Indicator processing completed in {indicator_processing_time:.3f}s")
            ...
            try:
                ...
                processing_metadata["fuzzy_input_timeframes"] = list(fuzzy_input_data.keys())
            ...
            try:
                ...
                logger.debug(f"Fuzzy processing completed in {fuzzy_processing_time:.3f}s")
            ...
            logger.info(f"Pipeline processing completed in {total_processing_time:.3f}s with {len(errors)} errors")
            ...
            total_processing_time = time.time() - start_time
            logger.error(f"Pipeline processing failed after {total_processing_time:.3f}s: {e}")
            ...
            # Return error result
            return IntegratedFuzzyResult(
                fuzzy_result=MultiTimeframeFuzzyResult({}, {}, processing_metadata, [str(e)], 0.0),
                indicator_data={},
                processing_metadata=processing_metadata,
                errors=[str(e)],
                warnings=warnings,
        ...
        for timeframe, df in indicator_results.items():
            ...
            for column in df.columns:
                if pd.notna(latest_row[column]):
                    ...
                    indicator_name = self._extract_base_indicator_name(column, timeframe)
                    timeframe_data[indicator_name] = float(latest_row[column])
            
            if timeframe_data:
                fuzzy_input[timeframe] = timeframe_data
                logger.debug(f"Converted {len(timeframe_data)} indicators for timeframe {timeframe}")
    ...
    def get_pipeline_health(self) -> Dict[str, Any]:
        ...
        return {
            "indicator_engine": {
                ...
                "supported_timeframes": len(self.indicator_engine.get_supported_timeframes()) if self.indicator_engine else 0
            },
            "fuzzy_engine": {
                "initialized": self.fuzzy_engine is not None,
                "multi_timeframe_enabled": self.fuzzy_engine.is_multi_timeframe_enabled() if self.fuzzy_engine else False,
                "supported_timeframes": len(self.fuzzy_engine.get_supported_timeframes()) if self.fuzzy_engine else 0
            },
