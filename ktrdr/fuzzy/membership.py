...
class TriangularMF(MembershipFunction):
    ...
    def __init__(self, parameters: List[float]):
        ...
        """
        if len(parameters) != 3:
            logger.error(f"Invalid triangular MF parameters: expected 3, got {len(parameters)}")
        ...
        if not (self.a <= self.b <= self.c):
            logger.error(f"Invalid triangular MF parameter order: a={self.a}, b={self.b}, c={self.c}")
            raise ConfigurationError(
        ...
        self._ab_diff = max(self.b - self.a, np.finfo(float).eps)  # Avoid division by zero
        self._bc_diff = max(self.c - self.b, np.finfo(float).eps)  # Avoid division by zero
        
        logger.debug(f"Initialized triangular MF with parameters: a={self.a}, b={self.b}, c={self.c}")
    ...
    def evaluate(self, x: Union[float, pd.Series, np.ndarray]) -> Union[float, pd.Series, np.ndarray]:
        ...
        # For pandas Series
            ...
            logger.debug(f"Evaluating triangular MF for pandas Series of length {len(x)}")
        ...
        # For numpy arrays
            ...
            raise TypeError(f"Unsupported input type: {type(x)}. Expected float, pd.Series, or np.ndarray.")
