...
class TrapezoidalMF(MembershipFunction):
    ...
    def __init__(self, parameters: List[float]):
        ...
        if not (self.a <= self.b <= self.c <= self.d):
            ...
            raise ConfigurationError(
                ...
                error_code="MF-InvalidParameterOrder",
                details={"parameters": {"a": self.a, "b": self.b, "c": self.c, "d": self.d}}
            )
    ...
    def evaluate(
        ...
        # For numpy arrays
            ...
            logger.debug(f"Evaluating trapezoidal MF for numpy array of shape {x.shape}")
            result = np.zeros_like(x, dtype=float)
...
class GaussianMF(MembershipFunction):
    ...
    def evaluate(
        ...
        # For pandas Series
            ...
            logger.debug(
                f"Evaluating Gaussian MF for pandas Series of length {len(x)}"
            )
            return x.apply(self._evaluate_scalar)
