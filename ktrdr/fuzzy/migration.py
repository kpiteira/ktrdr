...
class FuzzyConfigMigrator:
    ...
    def migrate_single_to_multi_timeframe(
        ...
        """
        logger.info(f"Migrating single-timeframe config to timeframe: {target_timeframe}")
        ...
        try:
            ...
            logger.info(f"Successfully migrated config with {len(indicators)} indicators")
            return multi_config
    ...
    def migrate_to_multiple_timeframes(
        ...
        Raises:
            ConfigurationError: If migration fails
        """
        logger.info(f"Migrating single-timeframe config to {len(timeframes)} timeframes")
        ...
        if isinstance(config, dict) and config:
            ...
            for key, value in config.items():
                if isinstance(value, dict) and any(
                    mf_key in value for mf_key in ["low", "high", "neutral", "negative", "positive"]
                ):
