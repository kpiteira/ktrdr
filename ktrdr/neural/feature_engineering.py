...
class MultiTimeframeFeatureEngineer:
    ...
    def fit_transform(
        ...
        """
        logger.info(
            f"Fitting feature engineering pipeline for {len(features)} timeframes"
        )
        ...
        selected_features, selected_mask, feature_selector = (
            self._apply_feature_selection(scaled_features, labels, feature_names)
        )
        ...
        transformation_metadata = {
            ...
            "reduction_method": self.config.get("dimensionality_reduction", {}).get(
                "method", "none"
            ),
        ...
        logger.info(
            f"Feature engineering complete: {transformation_metadata['original_feature_count']} -> "
    ...
    def transform(self, features: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        if not self.is_fitted:
            raise ValueError(
                "Feature engineering pipeline not fitted. Call fit_transform() first."
            )
    ...
    def _apply_scaling(self, features: np.ndarray) -> Tuple[np.ndarray, Optional[Any]]:
        ...
        if method == "standard":
            ...
            scaler = QuantileTransformer(
                ...
                output_distribution=scaling_config.get(
    ...
    def _apply_feature_selection(
        ...
        try:
            ...
            if method == "kbest_f":
                ...
                n_features = min(
                    selection_config.get("n_features", 20), features.shape[1]
                )
            ...
            logger.debug(
                f"Selected {len(self.selected_feature_names)} features using {method}"
            )
    ...
    def _apply_dimensionality_reduction(
        ...
        try:
            ...
            if method == "pca":
                ...
                n_components = min(
                    reduction_config.get("n_components", 10), features.shape[1]
                )
                reducer = FastICA(
            ...
            logger.debug(
                f"Reduced features from {features.shape[1]} to {reduced_features.shape[1]} using {method}"
            )
    ...
    def analyze_timeframe_contributions(
        self, features: Dict[str, np.ndarray], labels: Optional[np.ndarray] = None
    ) -> Dict[str, Any]:
        """
        ...
        if labels is not None:
            ...
            for timeframe, tf_features in features.items():
                try:
                    ...
                    logger.warning(
                        f"Could not calculate importance for {timeframe}: {e}"
                    )
