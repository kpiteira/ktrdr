...
class MultiTimeframeMLP(BaseNeuralModel):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            f"Initialized MultiTimeframeMLP with {len(self.timeframe_configs)} timeframes"
        )
    ...
    def prepare_features(
        ...
        for timeframe in timeframe_order:
            ...
            if timeframe not in fuzzy_data:
                ...
                tf_names = [
                    f"{feat}_{timeframe}" for feat in tf_config.expected_features
                ]
            else:
                tf_features, tf_names = self._extract_timeframe_features(
                    fuzzy_data[timeframe], timeframe, tf_config.expected_features
                )
    ...
    def _extract_timeframe_features(
        ...
        if len(fuzzy_data) == 0:
            ...
            return [0.0] * len(expected_features), [
                f"{feat}_{timeframe}" for feat in expected_features
            ]
        ...
        for expected_feature in expected_features:
            ...
            if feature_col in latest_row.index:
                ...
                logger.warning(
                    f"Missing feature {expected_feature} for timeframe {timeframe}"
                )
                value = 0.0
    ...
    def _scale_features(
        self, features: np.ndarray, scaler: Optional[Any] = None
        ...
        if scaler is not None:
            ...
            # Create new scaler
            scaler_type = self.config.get("feature_processing", {}).get(
    ...
    def train(
        ...
        logger.info(
            f"Training MultiTimeframeMLP with {X.shape[0]} samples, {X.shape[1]} features"
        )
    ...
    def get_model_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "enabled_timeframes": [
                tf for tf, config in self.timeframe_configs.items() if config.enabled
            ],
