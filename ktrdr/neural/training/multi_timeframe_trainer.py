...
from ktrdr.neural.models.multi_timeframe_mlp import MultiTimeframeMLP, MultiTimeframeTrainingResult
...
class CrossTimeframeValidationConfig:
    ...
    method: str = "temporal_split"  # temporal_split, stratified_kfold, timeframe_holdout
    n_splits: int = 5
    test_size: float = 0.2
...
class MultiTimeframeTrainer:
    ...
    def train(
        ...
        labels: np.ndarray,
        validation_data: Optional[Tuple[Dict[str, Dict[str, np.ndarray]], np.ndarray]] = None
        ...
        try:
            ...
            # Prepare features
            features_by_timeframe = self._prepare_timeframe_features(multi_timeframe_data)
            
            # Apply feature engineering
            feature_result = self.feature_engineer.fit_transform(features_by_timeframe, labels)
            X_train = feature_result.transformed_features
            
            # Prepare validation data if provided
            X_val, y_val = None, None
            if validation_data is not None:
                val_features_by_timeframe = self._prepare_timeframe_features(validation_data[0])
            ...
            training_result.model_performance["total_training_time"] = total_training_time
    ...
    def _prepare_timeframe_features(
        ...
        for timeframe, timeframe_data in multi_timeframe_data.items():
            ...
            if isinstance(timeframe_data, dict):
                ...
                if feature_arrays:
                    features_by_timeframe[timeframe] = np.concatenate(feature_arrays, axis=1)
                else:
    ...
    def _create_validation_split(
        ...
        logger.debug(f"Created validation split: {len(X_train)} train, {len(X_val)} val")
    ...
    def _perform_cross_timeframe_validation(
        ...
        combined_features, _ = self.feature_engineer._combine_timeframe_features(features_by_timeframe)
        ...
        try:
            ...
            for fold, (train_idx, val_idx) in enumerate(cv.split(combined_features, labels)):
                logger.debug(f"Processing CV fold {fold + 1}/{val_config.n_splits}")
                
                # Split data
                X_fold_train, X_fold_val = combined_features[train_idx], combined_features[val_idx]
                ...
                fold_result = fold_model.train(
                    X_fold_train_tensor, y_fold_train_tensor,
    ...
    def _calculate_enhanced_metrics(
        ...
        try:
            ...
            consistency_score = self._calculate_cross_timeframe_consistency(features_by_timeframe)
            ...
            enhanced_metrics.update({
                "train_precision": float(precision_score(y_train_np, train_preds_np, average="weighted", zero_division=0)),
                "train_recall": float(recall_score(y_train_np, train_preds_np, average="weighted", zero_division=0)),
                "train_f1": float(f1_score(y_train_np, train_preds_np, average="weighted", zero_division=0)),
                "val_precision": float(precision_score(y_val_np, val_preds_np, average="weighted", zero_division=0)),
                "val_recall": float(recall_score(y_val_np, val_preds_np, average="weighted", zero_division=0)),
                "val_f1": float(f1_score(y_val_np, val_preds_np, average="weighted", zero_division=0))
            })
            ...
            for class_idx, class_name in enumerate(["BUY", "HOLD", "SELL"]):
                ...
                train_class_precision = precision_score(
                    y_train_np, train_preds_np, labels=[class_idx], average=None, zero_division=0
                )
                ...
                val_class_precision = precision_score(
                    y_val_np, val_preds_np, labels=[class_idx], average=None, zero_division=0
                )
                ...
                enhanced_metrics.update({
                    f"train_{class_name.lower()}_precision": float(train_class_precision[0]) if len(train_class_precision) > 0 else 0.0,
                    f"train_{class_name.lower()}_recall": float(train_class_recall[0]) if len(train_class_recall) > 0 else 0.0,
                    f"val_{class_name.lower()}_precision": float(val_class_precision[0]) if len(val_class_precision) > 0 else 0.0,
                    f"val_{class_name.lower()}_recall": float(val_class_recall[0]) if len(val_class_recall) > 0 else 0.0
                })
            
            # Confusion matrices
            enhanced_metrics["train_confusion_matrix"] = confusion_matrix(y_train_np, train_preds_np).tolist()
            enhanced_metrics["val_confusion_matrix"] = confusion_matrix(y_val_np, val_preds_np).tolist()
    ...
    def _calculate_final_metrics(
        ...
        try:
            ...
            if self.feature_engineer:
                ...
                final_metrics["feature_reduction_ratio"] = final_metrics["final_feature_count"] / final_metrics["original_feature_count"]
...
def create_multi_timeframe_trainer(config: Dict[str, Any]) -> MultiTimeframeTrainer:
    ...
    training_config = MultiTimeframeTrainingConfig(
        ...
        validation_config=CrossTimeframeValidationConfig(**config.get("validation_config", {})),
        early_stopping_config=EarlyStoppingConfig(**config.get("early_stopping_config", {})),
