...
class ModelStorageV2:
    ...
    def get_model_path(
        ...
        """
        strategy_dir, model_id = strategy_loader.get_model_storage_path_components(strategy_config)
    ...
    def load_model(
        ...
        logger.info(f"✅ Model loaded successfully: {metadata.strategy_name} v{metadata.model_version}")
    ...
    def delete_model(self, model_path: Union[str, Path], confirm: bool = False) -> bool:
        ...
        """
        if not confirm:
            logger.warning("Model deletion requires explicit confirmation (confirm=True)")
            return False
        ...
        try:
            ...
            try:
                metadata = ModelMetadata.load(model_path)
                logger.info(f"Deleting model: {metadata.strategy_name} v{metadata.model_version}")
            except:
    ...
    def migrate_legacy_model(
        ...
        try:
            ...
            symbols, timeframes = strategy_loader.extract_training_symbols_and_timeframes(strategy_config)
            ...
            logger.info(f"✅ Legacy model migrated successfully: {legacy_path} -> {new_path}")
            return new_path
