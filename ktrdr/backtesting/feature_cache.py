...
class FeatureCache:
                    ...
                    if col.upper().startswith(indicator_type):
                        if indicator_type in ["SMA", "EMA"]:
                            ...
                            if (
                                ...
                                current_bar_indicators[original_name] = self.indicators_df[col].iloc[idx]
                                break
                        else:
    ...
    def get_features_for_timestamp(self, timestamp: pd.Timestamp) -> Tuple[Dict[str, float], Dict[str, float]]:
        ...
        if timestamp not in self.mapped_indicators_df.index:
            ...
            closest_idx = self.mapped_indicators_df.index.get_indexer([timestamp], method='pad')[0]
            if closest_idx == -1:
                raise ValueError(f"Timestamp {timestamp} not found in pre-computed features (no past data available)")
            ...
            if actual_timestamp > timestamp:
                raise ValueError(f"Look-ahead bias detected: {actual_timestamp} > {timestamp}")
            ...
            if time_diff > 3600:  # 1 hour in seconds
                raise ValueError(f"Closest past timestamp {actual_timestamp} is {time_diff/3600:.1f}h before {timestamp}")
    ...
    def is_ready(self) -> bool:
        ...
        """
        return (
            self.indicators_df is not None and 
            self.mapped_indicators_df is not None and 
            self.fuzzy_df is not None
        )