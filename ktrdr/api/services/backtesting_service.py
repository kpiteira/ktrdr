...
class BacktestingService(BaseService):
    ...
    async def start_backtest(
        ...
        asyncio.create_task(
            self._run_backtest_async(backtest_id, strategy_name, symbol, timeframe, 
    ...
    async def _run_backtest_async(
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Successfully converted results to dict, keys: {results_dict.keys()}")
                
                # Add trade details (like CLI does for verbose output)
                results_dict["trades"] = [
                    {
                        "trade_id": str(trade.trade_id),  # Convert to string for Pydantic validation
                        "side": trade.side,
                ...
                if hasattr(results, 'equity_curve') and results.equity_curve is not None:
                    try:
                        ...
                        logger.info(f"Equity curve columns: {equity_df.columns.tolist()}")
                        logger.info(f"Equity curve index type: {type(equity_df.index)}")
                        
                        # Handle different possible column names and index types
                        if 'timestamp' in equity_df.columns:
                            timestamps = equity_df['timestamp'].dt.strftime('%Y-%m-%dT%H:%M:%S.%fZ').tolist()
                        elif hasattr(equity_df.index, 'strftime'):
                            timestamps = equity_df.index.strftime('%Y-%m-%dT%H:%M:%S.%fZ').tolist()
                        else:
                        ...
                        if 'equity' in equity_df.columns:
                            ...
                            values = equity_df[numeric_cols[0]].tolist() if len(numeric_cols) > 0 else [0.0] * len(equity_df)
                        ...
                        logger.info(f"Added equity curve data with {len(results_dict['equity_curve']['timestamps'])} points")
                    except Exception as eq_e:
                        logger.error(f"Failed to process equity curve: {eq_e}", exc_info=True)
            ...
            self._active_backtests[backtest_id]["completed_at"] = datetime.now().isoformat()
    ...
    async def get_backtest_results(self, backtest_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                ...
                "total_trades": metrics_data.get("total_trades", results.get("trade_count", 0))  # Use metrics.total_trades first, fallback to top-level trade_count
            },
            "summary": {
                "initial_capital": initial_capital,
    ...
    async def get_backtest_trades(self, backtest_id: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"First trade sample: {raw_trades[0] if raw_trades else 'No trades'}")
    ...
    async def get_equity_curve(self, backtest_id: str) -> Dict[str, Any]:
        ...
        if backtest["results"] is None or "equity_curve" not in backtest["results"]:
            raise DataError(f"No equity curve data available for backtest {backtest_id}")
