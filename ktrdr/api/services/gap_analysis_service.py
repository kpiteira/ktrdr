...
class GapAnalysisService:
    ...
    async def analyze_gaps(self, request: GapAnalysisRequest) -> GapAnalysisResponse:
        ...
        try:
            ...
            if df is None or df.empty:
                return self._create_empty_response(request, start_date, end_date, "No data found")
            
            # Filter data to analysis period
            df_filtered = self._filter_data_to_period(df, start_date, end_date)
            
            # Get trading metadata
            trading_metadata = self.gap_classifier.get_symbol_trading_hours(request.symbol)
            
            # Detect gaps in the data
            gaps = self._detect_gaps_in_period(df_filtered, start_date, end_date, 
                                             request.symbol, request.timeframe)
            
            # Filter gaps based on mode and include_expected
            filtered_gaps = self._filter_gaps_by_mode(gaps, request.mode, request.include_expected)
            
            # Calculate summary statistics
            summary = self._calculate_summary_statistics(
                df_filtered, gaps, start_date, end_date, request.timeframe, trading_metadata
            )
            ...
            return GapAnalysisResponse(
                ...
                analysis_period={
                    ...
                    "trading_days_in_period": self._count_trading_days(start_date, end_date, trading_metadata)
                },
            ...
            logger.error(f"Gap analysis failed for {request.symbol}_{request.timeframe}: {e}")
            raise
    
    async def analyze_gaps_batch(self, request: BatchGapAnalysisRequest) -> BatchGapAnalysisResponse:
        """
    ...
    def _filter_data_to_period(self, df: pd.DataFrame, start_date: datetime, 
    ...
    def _detect_gaps_in_period(self, df: pd.DataFrame, start_date: datetime, 
                              end_date: datetime, symbol: str, timeframe: str) -> List[GapInfo]:
        """Detect all gaps in the analysis period."""
        gaps = []
        
        if df.empty:
            # Entire period is a gap
            gap_info = self.gap_classifier.analyze_gap(start_date, end_date, symbol, timeframe)
        ...
        if first_timestamp > start_date:
            gap_info = self.gap_classifier.analyze_gap(start_date, first_timestamp, symbol, timeframe)
            gaps.append(gap_info)
        ...
        if expected_last < end_date:
            gap_info = self.gap_classifier.analyze_gap(expected_last, end_date, symbol, timeframe)
            gaps.append(gap_info)
        
        return gaps
    
    def _filter_gaps_by_mode(self, gaps: List[GapInfo], mode: GapAnalysisMode, 
                            include_expected: bool) -> List[GapInfo]:
        ...
        if not include_expected:
            ...
            return [gap for gap in gaps if gap.classification == GapClassification.UNEXPECTED]
        
        if mode == GapAnalysisMode.EXTENDED:
            # Show unexpected gaps + market closures
            return [gap for gap in gaps if gap.classification in [
                GapClassification.UNEXPECTED, GapClassification.MARKET_CLOSURE
            ]]
        
        # Verbose mode shows all gaps
        return gaps
    
    def _calculate_summary_statistics(self, df: pd.DataFrame, gaps: List[GapInfo],
                                     start_date: datetime, end_date: datetime,
                                     timeframe: str, trading_metadata: Optional[Dict]) -> GapAnalysisSummary:
        ...
        if timeframe in ['1d', '1w'] and trading_metadata:
            expected_bars = self._count_trading_days(start_date, end_date, trading_metadata)
        ...
        for classification in GapClassification:
            missing_breakdown[classification.value] = sum(
                gap.bars_missing for gap in gaps 
    ...
    def _count_trading_days(self, start_date: datetime, end_date: datetime,
                           trading_metadata: Optional[Dict]) -> int:
    ...
    def _generate_recommendations(self, gaps: List[GapInfo], summary: GapAnalysisSummary) -> List[str]:
        ...
        if summary.data_completeness_pct >= 99:
            ...
            recommendations.append("Acceptable data completeness - consider improving data collection")
        else:
            recommendations.append("Poor data completeness - immediate attention required")
        
        # Unexpected gap recommendations
        unexpected_gaps = [g for g in gaps if g.classification == GapClassification.UNEXPECTED]
        if unexpected_gaps:
            if len(unexpected_gaps) == 1:
                recommendations.append("1 unexpected gap requires investigation")
            else:
                recommendations.append(f"{len(unexpected_gaps)} unexpected gaps require investigation")
        
        # Market closure recommendations
        market_closures = [g for g in gaps if g.classification == GapClassification.MARKET_CLOSURE]
        if market_closures:
            recommendations.append("Extended market closures detected - verify with broker/exchange")
        
        # Holiday pattern recommendations
        holiday_gaps = [g for g in gaps if g.classification == GapClassification.EXPECTED_HOLIDAY]
        if len(holiday_gaps) > 5:
            recommendations.append("Many holiday gaps detected - verify trading calendar")
    ...
    def _create_empty_response(self, request: GapAnalysisRequest, start_date: datetime,
                              end_date: datetime, reason: str) -> GapAnalysisResponse:
        ...
        return GapAnalysisResponse(
            ...
            summary=summary,
            gaps=[],
            recommendations=[f"No data available: {reason}", "Complete data collection needed"]
        )
    
    def _calculate_batch_summary(self, results: List[GapAnalysisResponse]) -> Dict[str, Any]:
        ...
        avg_completeness = sum(r.summary.data_completeness_pct for r in results) / len(results)
        
        # Aggregate missing breakdown
        aggregated_breakdown = {}
        for classification in GapClassification:
            aggregated_breakdown[classification.value] = sum(
                r.summary.missing_breakdown.get(classification.value, 0) for r in results
            )
        ...
        return {
            ...
            "overall_completeness_pct": round((total_actual / max(total_expected, 1)) * 100, 2),
