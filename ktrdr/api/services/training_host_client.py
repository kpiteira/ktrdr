...
class TrainingHostClient:
    ...
    async def get_service_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise TrainingHostServiceError(f"Failed to get service info: {response.status_code}")
    ...
    async def start_training_session(self, config: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise TrainingHostServiceError(f"Failed to start training: {error_detail}")
    ...
    async def get_training_status(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_detail = response.json().get("detail", "Unknown error")
                raise TrainingHostServiceError(f"Failed to get training status: {error_detail}")
                
        except httpx.RequestError as e:
            raise TrainingHostServiceError(f"Connection error: {str(e)}")
    
    async def stop_training_session(self, session_id: str, save_checkpoint: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_detail = response.json().get("detail", "Unknown error")
                raise TrainingHostServiceError(f"Failed to stop training: {error_detail}")
    ...
    async def list_training_sessions(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_detail = response.json().get("detail", "Unknown error")
                raise TrainingHostServiceError(f"Failed to list sessions: {error_detail}")
    ...
    async def cleanup_session(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            client = await self._get_client()
            response = await client.delete(f"{self.base_url}/training/sessions/{session_id}")
            ...
            if response.status_code == 200:
                ...
                error_detail = response.json().get("detail", "Unknown error")
                raise TrainingHostServiceError(f"Failed to cleanup session: {error_detail}")
        ...
        while True:
            try:
                ...
                if status["status"] in ["completed", "failed", "stopped"]:
                    logger.info(f"Training session {session_id} finished with status: {status['status']}")
                    return status
                
                # Check timeout
                if timeout and (time.time() - start_time) > timeout:
                    raise TrainingHostServiceError(f"Training session {session_id} timed out after {timeout} seconds")
                ...
                logger.warning(f"Error polling training status for session {session_id}: {str(e)}")
                await asyncio.sleep(poll_interval)
    ...
    def map_to_operation_progress(self, status: Dict[str, Any]) -> Dict[str, Any]:
        ...
        batch_progress = (batch / total_batches) / total_epochs if total_epochs > 0 else 0
...
def get_training_host_client(base_url: str = "http://localhost:5002") -> TrainingHostClient:
    """
