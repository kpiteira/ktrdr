...
class TrainingService:
    ...
    async def start_training(
        ...
        try:
            ...
            asyncio.create_task(self._run_training_task(task_id, symbol, timeframe, config, start_date, end_date))
    ...
    async def get_model_performance(self, task_id: str) -> Dict[str, Any]:
        ...
        if task["status"] != "completed":
            raise ValidationError(f"Training task {task_id} is not completed (status: {task['status']})")
