...
class FuzzyService(BaseService):
            ...
            if config_path:
                ...
                try:
                    ...
                    for path in possible_paths:
                        try:
                            if path.exists():
                                self.logger.info(f"Loading fuzzy configuration from: {path}")
                                self.config = config_loader.load_from_yaml(str(path))
                                break
                        except Exception as e:
                            self.logger.warning(f"Failed to load fuzzy config from {path}: {str(e)}")
                    else:
                        # If no config file was found, create a minimal valid configuration
                        self.logger.warning("No fuzzy configuration found. Using minimal default configuration.")
            ...
            for indicator_name in self.fuzzy_engine.get_available_indicators():
                try:
                    ...
                    indicator_info = {
                        ...
                        "output_columns": self.fuzzy_engine.get_output_names(indicator_name)
                    }
                    
                    indicators.append(indicator_info)
                    
                except Exception as e:
                    self.logger.warning(f"Failed to get fuzzy sets for indicator {indicator_name}: {str(e)}")
    ...
    async def get_fuzzy_sets(self, indicator: str) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(f"Retrieved {len(fuzzy_sets)} fuzzy sets for indicator {indicator}")
            return fuzzy_sets
            ...
            self.logger.error(f"Error retrieving fuzzy sets for indicator {indicator}: {str(e)}")
            raise ProcessingError(
    ...
    async def fuzzify_indicator(
        ...
        try:
            ...
            self.logger.error(f"Error fuzzifying values for indicator {indicator}: {str(e)}")
            raise ProcessingError(
        ...
        try:
            ...
            for config in indicator_configs:
                ...
                if source_column in df.columns:
                    ...
                    self.logger.warning(f"Source column {source_column} not found in dataframe")
                    continue
                
                # Fuzzify the indicator values
                try:
                    fuzzy_perf = self.track_performance(f"fuzzify_{indicator_name}")
                    fuzzified = await self.fuzzify_indicator(indicator_name, indicator_values.tolist(), dates)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "sample_fuzzy_sets": sample_fuzzy_sets[:5] if isinstance(sample_fuzzy_sets, list) else list(sample_fuzzy_sets)[:5],  # First 5 fuzzy sets
                "message": message
            }
        except Exception as e:
