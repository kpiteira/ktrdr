...
class OperationsService:
    ...
    def generate_operation_id(self, operation_type: OperationType, prefix: Optional[str] = None) -> str:
        """
    ...
    async def update_progress(
        ...
        async with self._lock:
            if operation_id not in self._operations:
                logger.warning(f"Cannot update progress - operation not found: {operation_id}")
                return
            ...
            if progress.percentage % 10 == 0:  # Log every 10%
                logger.debug(f"Operation {operation_id} progress: {progress.percentage:.1f}%")
    ...
    async def cancel_operation(
        ...
        async with self._lock:
            ...
            logger.info(f"Cancelled operation: {operation_id} (task_cancelled: {cancelled_task})")
        ...
        async with self._lock:
            ...
            if original_operation.status != OperationStatus.FAILED:
                raise DataError(
                    ...
                    details={"operation_id": operation_id, "status": original_operation.status},
                )
            ...
            logger.info(f"Created retry operation: {new_operation_id} (original: {operation_id})")
