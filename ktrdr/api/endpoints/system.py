    ...
    try:
            ...
            try:
                ...
                if actual_connected:
                    ib_connection_details = {
                        ...
                        "transport_open": not hasattr(connection.ib.client, 'conn') or 
                                        (hasattr(connection.ib.client.conn, 'is_closing') and 
                                         not connection.ib.client.conn.is_closing())
                    }
        ...
        return {
            ...
            "ib_connection": {
                ...
                "host": status.host,
                "port": status.port,
                "last_connect_time": status.last_connect_time.isoformat() if status.last_connect_time else None,
                "last_disconnect_time": status.last_disconnect_time.isoformat() if status.last_disconnect_time else None,
...
async def get_gap_filler_status() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting gap filler status: {str(e)}")
