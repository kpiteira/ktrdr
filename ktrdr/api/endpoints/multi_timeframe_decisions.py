...
class MultiTimeframeDecisionRequest(BaseModel):
    ...
    strategy_config_path: str = Field(..., description="Path to strategy configuration file")
    ...
    model_path: Optional[str] = Field(
        None, 
        description="Path to multi-timeframe model (optional)"
    )
...
def prepare_timeframe_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to load timeframe data: {str(e)}"
        )
...
async def analyze_timeframe_performance(
    symbol: str,
    strategy_config_path: str = Query(..., description="Path to strategy configuration"),
    ...
    try:
        ...
        if "latest_consensus" in analysis:
            ...
            consensus_history = orchestrator.get_consensus_history(symbol=symbol, limit=1)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to analyze timeframe performance: {str(e)}"
        )
...
async def check_multi_timeframe_data_status(
    ...
    try:
        ...
        for timeframe in timeframes:
            try:
                ...
                if data is not None and not data.empty:
                    ...
                    timeframe_status.append(TimeframeDataStatus(
                        timeframe=timeframe,
                        available=True,
        ...
        ready_for_analysis = len(quality_scores) >= len(timeframes) / 2 and overall_quality > 0.6
            ...
            try:
                ...
                if timeframe_configs or multi_timeframe_config:
                    strategy_info = {
                        ...
                        "timeframes": list(timeframe_configs.keys()) if timeframe_configs else [],
                        "primary_timeframe": None,
                        "consensus_method": multi_timeframe_config.get("consensus_method", "weighted_majority"),
                        "min_agreement_threshold": multi_timeframe_config.get("min_agreement_threshold", 0.6),
