...
class ChartGridOptions(BaseModel):
    ...
    show_horizontal: bool = Field(True, description="Whether to show horizontal grid lines")
...
class ChartAxisOptions(BaseModel):
    ...
    position: str = Field("right", description="Axis position (left, right, top, bottom)")
...
class ChartSeriesStyle(BaseModel):
    ...
    down_color: Optional[str] = Field(None, description="Down color for candlestick series")
    point_size: Optional[float] = Field(None, description="Point size for scatter series")
...
class ChartSeries(BaseModel):
    ...
    y_axis_id: Optional[str] = Field(None, description="ID of the y-axis for this series")
    style: Optional[ChartSeriesStyle] = Field(None, description="Style options for this series")
...
class ChartPanel(BaseModel):
    ...
    series: List[ChartSeries] = Field(..., description="Series to display in this panel")
...
class ChartOptions(BaseModel):
    ...
    width: Optional[int] = Field(None, description="Chart width in pixels (null for responsive)")
...
class ChartRenderRequest(BaseModel):
    ...
    indicators: List[IndicatorConfig] = Field(default_factory=list, description="Indicators to include in the chart")
    start_date: Optional[str] = Field(None, description="Start date for data range (ISO format)")
    end_date: Optional[str] = Field(None, description="End date for data range (ISO format)")
    panels: Optional[List[ChartPanel]] = Field(None, description="Custom panel configuration")
...
class ChartExportResponse(BaseModel):
    ...
    content: Optional[str] = Field(None, description="Base64-encoded content for direct download")
