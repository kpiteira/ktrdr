...
class IbLimitsRegistry:
    ...
    CLIENT_ID_RANGES = {
        ...
        "api_singleton": list(range(1, 11)),         # 1-10: Singleton connections (API, gap filler)
    ...
    def get_duration_limit(cls, timeframe: str) -> timedelta:
        ...
        raise ValueError(f"Unsupported timeframe: {timeframe}. Supported: {list(cls.SIMPLE_DURATION_LIMITS.keys())}")
    ...
    def get_client_id_for_purpose(cls, purpose: str, index: int = 0) -> int:
        ...
        if purpose not in cls.CLIENT_ID_RANGES:
            raise ValueError(f"Unknown purpose: {purpose}. Available: {list(cls.CLIENT_ID_RANGES.keys())}")
            
        range_list = cls.CLIENT_ID_RANGES[purpose]
        if index >= len(range_list):
            raise ValueError(f"Index {index} out of range for {purpose} (max: {len(range_list)-1})")
            
        return range_list[index]
    
    @classmethod
    def calculate_progressive_chunks(cls, timeframe: str, total_duration: timedelta) -> int:
        """
        ...
        chunks = int((total_duration.total_seconds() / max_duration.total_seconds()) + 0.5)
        return max(1, chunks)
