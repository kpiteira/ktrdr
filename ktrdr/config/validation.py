    ...
    def validate_numeric(
        ...
        if not isinstance(value, (int, float)):
            raise ValidationError(
                ...
                error_code="VAL-TypeError",
                details={"expected_type": "int or float", "actual_type": type(value).__name__}
            )
    ...
    def validate_date(
        ...
        if min_date is not None and dt < min_date:
            raise ValidationError(
                ...
                details={"min_date": min_date.strftime(format_string), "actual_date": value}
            )
        ...
        if max_date is not None and dt > max_date:
            raise ValidationError(
                ...
                details={"max_date": max_date.strftime(format_string), "actual_date": value}
            )
    ...
    def validate_file_path(
        ...
        if file_type is not None and path.suffix.lower() != f".{file_type.lower()}":
            raise ValidationError(
                ...
                details={"expected_type": file_type, "actual_type": path.suffix[1:] if path.suffix else None}
            )
