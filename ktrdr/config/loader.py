"""
Configuration loader for YAML-based domain configuration.

This module provides functionality to load and validate domain configuration
(data paths, indicators, strategies) from YAML files using Pydantic models.

Note: System configuration (API settings, database, auth, etc.) is handled by
the Settings classes in ktrdr/config/settings.py, not this loader.
"""
...
class ConfigLoader:
    """Loads and validates domain configuration from YAML files.

    This loader handles domain-specific configuration such as data paths,
    indicator definitions, and strategy configurations. System configuration
    (API, database, auth, etc.) is managed by Settings classes.

    Typical usage:
        loader = ConfigLoader()
        config = loader.load("config/settings.yaml", KtrdrConfig)
