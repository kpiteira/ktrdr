...
class IbConfig:
    ...
    readonly: bool = field(
        default_factory=lambda: os.getenv("IB_READONLY", "false").lower() == "true"
    )

    # Rate limiting settings
    rate_limit: int = field(
        default_factory=lambda: int(os.getenv("IB_RATE_LIMIT", "50"))
    )
    rate_period: int = field(
        default_factory=lambda: int(os.getenv("IB_RATE_PERIOD", "60"))
    )
    ...
    max_retries: int = field(
        default_factory=lambda: int(os.getenv("IB_MAX_RETRIES", "3"))
    )
    retry_base_delay: float = field(
        default_factory=lambda: float(os.getenv("IB_RETRY_DELAY", "2.0"))
    )
    retry_max_delay: float = field(
        default_factory=lambda: float(os.getenv("IB_RETRY_MAX_DELAY", "60.0"))
    )

    # Pacing settings (based on IB documentation)
    pacing_delay: float = field(
        default_factory=lambda: float(os.getenv("IB_PACING_DELAY", "0.6"))
    )
    max_requests_per_10min: int = field(
        default_factory=lambda: int(os.getenv("IB_MAX_REQUESTS_10MIN", "60"))
    )
    ...
    def _validate(self):
        ...
        if self.retry_base_delay <= 0:
            raise ValueError(
                f"Retry base delay must be positive: {self.retry_base_delay}"
            )
