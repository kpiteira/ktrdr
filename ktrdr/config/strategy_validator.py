...
class StrategyValidator:
    ...
    def validate_strategy(self, config_path: str) -> ValidationResult:
        ...
        for section, expected_type in self.REQUIRED_SECTIONS.items():
            if section not in config:
                ...
                result.is_valid = False
                result.errors.append(f"Section '{section}' must be of type {expected_type.__name__}")
    ...
    def _validate_model_section(self, model_config: Dict[str, Any]) -> ValidationResult:
        ...
        for field, expected_type in self.NEURAL_MODEL_REQUIRED.items():
            if field not in model_config:
                ...
                result.is_valid = False
                result.errors.append(f"Model field '{field}' must be of type {expected_type.__name__}")
        
        # Check for old format
        if 'input_size' in model_config or 'output_size' in model_config:
            result.warnings.append("Detected old model format with input_size/output_size - consider upgrading")
        
        return result
    
    def _validate_training_section(self, training_config: Dict[str, Any]) -> ValidationResult:
        ...
        for field, expected_type in self.TRAINING_REQUIRED.items():
            if field not in training_config:
                ...
                result.is_valid = False
                result.errors.append(f"Training field '{field}' must be of type {expected_type.__name__}")
        
        return result
    
    def _validate_decisions_section(self, decisions_config: Dict[str, Any]) -> ValidationResult:
        ...
        for field, expected_type in self.DECISIONS_REQUIRED.items():
            if field not in decisions_config:
                ...
                result.is_valid = False
                result.errors.append(f"Decisions field '{field}' must be of type {expected_type.__name__}")
            ...
            for indicator, sets in config['fuzzy_sets'].items():
                if isinstance(sets, dict):
                    for set_name, set_config in sets.items():
                        if isinstance(set_config, dict) and 'type' in set_config and 'parameters' in set_config:
                            result.warnings.append(f"Legacy fuzzy set format detected for {indicator}.{set_name}")
        
        # Check for minimal model configuration
        if 'model' in config:
            model = config['model']
            if len(model) < 4:  # Less than required fields
                result.warnings.append("Minimal model configuration detected - consider upgrading")
    ...
    def _generate_suggestions(self, config: Dict[str, Any], result: ValidationResult):
        ...
        if missing_optional:
            result.suggestions.append(f"Consider adding optional sections: {', '.join(missing_optional)}")
        
        if 'orchestrator' not in config:
            result.suggestions.append("Add orchestrator section for advanced decision logic")
        
        if 'risk_management' not in config:
            result.suggestions.append("Add risk_management section for position sizing")
    
    def upgrade_strategy(self, config_path: str, output_path: Optional[str] = None) -> Tuple[bool, str]:
        ...
        if 'name' in upgraded_config and not upgraded_config['name'].startswith('neuro_'):
            ...
            if isinstance(sets, dict):
                for set_name, set_config in sets.items():
                    if isinstance(set_config, dict) and 'parameters' in set_config:
                        ...
                        if set_config.get('type') == 'triangular' and len(set_config['parameters']) == 3:
