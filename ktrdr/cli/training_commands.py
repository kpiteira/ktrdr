...
def train_strategy(
    ...
    return asyncio.run(_train_strategy_async(
        strategy=strategy,
        symbol=symbol,
        timeframe=timeframe,
...
async def _train_strategy_async(
    ...
    try:
        ...
        if not response.get("success"):
            console.print(f"[red]❌ Failed to start training: {response.get('error', 'Unknown error')}[/red]")
            raise typer.Exit(1)
        ...
        if not quiet:
            ...
            if response.get("estimated_duration_minutes"):
                console.print(f"⏱️  Estimated duration: [yellow]{response['estimated_duration_minutes']} minutes[/yellow]")
                ...
                while not cancelled:
                    try:
                        ...
                        if items_processed > 0:
                            if items_total:
                                description += f" ({items_processed:,}/{items_total:,} batches)"
                            else:
                        ...
                        if not quiet:
                            console.print(f"[yellow]Warning: Failed to get training status: {str(e)}[/yellow]")
                        break
        else:
        ...
        try:
            performance_response = await api_client.get_training_performance(operation_id)
            if performance_response.get("success"):
                ...
                if not quiet:
                    ...
                    if training_metrics:
                        ...
                        console.print(f"⏱️  Training Time: [yellow]{training_metrics.get('training_time_minutes', 'N/A'):.1f} minutes[/yellow]")
                ...
                if save_model and not quiet:
                    await _save_trained_model_async(api_client, operation_id, save_model, verbose)
                    
        except Exception as e:
            console.print(f"[yellow]Warning: Failed to get performance metrics: {str(e)}[/yellow]")
...
async def _save_trained_model_async(api_client, task_id: str, model_name: str, verbose: bool):
    ...
    try:
        ...
        if response.get("model_size_mb"):
            console.print(f"💽 Size: [yellow]{response.get('model_size_mb'):.1f} MB[/yellow]")
...
def test_model(
    ...
    """
    # Use asyncio to run the async function
    return asyncio.run(_test_model_async(model_name, symbol, timeframe, test_date, verbose))
...
async def _test_model_async(
    ...
    try:
        ...
        console.print(f"  Test Date: [yellow]{prediction_response.get('test_date')}[/yellow]")
