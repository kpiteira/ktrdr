...
def train_strategy(
        ...
        try:
            ...
            if validation_split <= 0 or validation_split >= 1:
                raise ValidationError(
                    f"Validation split must be between 0 and 1, got {validation_split}"
                )
            ...
            async with httpx.AsyncClient(
                timeout=300.0
            ) as client:  # 5 minute timeout for start
                response = await client.post(
                    "http://localhost:8000/api/v1/training/start", json=payload
                )
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    ...
                    console.print(
                        f"⏱️  Estimated duration: [yellow]{result.get('estimated_duration_minutes', 30)} minutes[/yellow]"
                    )
                    console.print()
                    ...
                    error_console.print(
                        f"[bold red]Training failed:[/bold red] {result.get('message', 'Unknown error')}"
                    )
                    sys.exit(1)
            else:
                error_console.print(
            ...
            error_console.print(
                "[bold red]Connection error:[/bold red] Could not connect to KTRDR API server"
            )
            error_console.print(
            ...
            error_console.print(
                "[bold red]Timeout error:[/bold red] Request took longer than 5 minutes"
            )
            error_console.print(
                "Training startup should be quick. Check API server status."
            )
            sys.exit(1)
    ...
    async def monitor_training_progress(task_id: str, verbose: bool):
                ...
                try:
                    async with httpx.AsyncClient(timeout=30.0) as client:
                        response = await client.get(
                            f"http://localhost:8000/api/v1/training/{task_id}"
                        )
                    ...
                    if response.status_code == 200:
                        ...
                        if (
                            verbose
                            and status.get("current_metrics")
                            and current_progress > last_progress
                        ):
                            metrics = status["current_metrics"]
                            console.print(
                                f"  📊 Epoch {current_epoch}: Loss={metrics.get('train_loss', 'N/A'):.4f}, Accuracy={metrics.get('train_accuracy', 'N/A'):.3f}"
                            )
                        ...
                        if training_status == "completed":
                            ...
                            progress.update(
                                task, description=f"Training failed: {error_msg}"
                            )
                            completed = True
                            sys.exit(1)
                        ...
                        console.print(
                            f"[yellow]Warning: Could not get training status (HTTP {response.status_code})[/yellow]"
                        )
                    ...
                    console.print(
                        "[yellow]Warning: Lost connection to API server, retrying...[/yellow]"
                    )
                    await asyncio.sleep(10)
                except Exception as e:
                    if verbose:
                        console.print(
                            f"[yellow]Warning: Error checking status: {e}[/yellow]"
                        )
                    await asyncio.sleep(10)
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.get(
                    f"http://localhost:8000/api/v1/training/{task_id}/performance"
                )
            ...
            if response.status_code == 200:
                ...
                if performance.get("success"):
                    ...
                    if training_metrics:
                        ...
                        console.print(
                            f"  Training Time: [yellow]{training_metrics.get('training_time_minutes', 'N/A'):.1f} minutes[/yellow]"
                        )
            ...
            if verbose:
                console.print(
                    f"[yellow]Warning: Could not retrieve performance metrics: {e}[/yellow]"
                )
    ...
    async def save_trained_model(task_id: str, model_name: str, verbose: bool):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    "http://localhost:8000/api/v1/models/save", json=payload
                )
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    ...
                    console.print(
                        f"📁 Model Path: [green]{result.get('model_path')}[/green]"
                    )
                    if result.get("model_size_mb"):
                        console.print(
                            f"💽 Size: [yellow]{result.get('model_size_mb'):.1f} MB[/yellow]"
                        )
                else:
                    console.print(
                        f"[red]Failed to save model: {result.get('message', 'Unknown error')}[/red]"
                    )
            else:
                console.print(
                    f"[red]Failed to save model (HTTP {response.status_code})[/red]"
                )
                if verbose:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    ...
                    for model in models:
                        console.print(
                            f"🧠 [bold cyan]{model.get('model_name', 'N/A')}[/bold cyan]"
                        )
                        console.print(
                            f"  Symbol: [blue]{model.get('symbol', 'N/A')}[/blue]"
                        )
                        console.print(
                            f"  Timeframe: [blue]{model.get('timeframe', 'N/A')}[/blue]"
                        )
                        console.print(
                        ...
                        if verbose:
                            console.print(
                                f"  Model ID: [dim]{model.get('model_id', 'N/A')}[/dim]"
                            )
                            console.print(
                    ...
                    error_console.print(
                        f"[bold red]Failed to list models:[/bold red] {result.get('message', 'Unknown error')}"
                    )
                    sys.exit(1)
            else:
                error_console.print(
            ...
            error_console.print(
                "[bold red]Timeout error:[/bold red] Request took longer than expected"
            )
            sys.exit(1)
        except Exception as e:
            error_console.print(f"[bold red]Unexpected error:[/bold red] {str(e)}")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                load_response = await client.post(
                    f"http://localhost:8000/api/v1/models/{model_name}/load"
                )

            if load_response.status_code != 200:
                error_console.print(
                    f"[bold red]Failed to load model:[/bold red] HTTP {load_response.status_code}"
                )
                try:
                    error_detail = load_response.json().get(
                        "detail", load_response.text
                    )
                    error_console.print(f"Details: {error_detail}")
                except:
            ...
            if not load_result.get("success") or not load_result.get("model_loaded"):
                error_console.print(
                    f"[bold red]Failed to load model:[/bold red] {load_result.get('model_info', {}).get('error', 'Unknown error')}"
                )
                sys.exit(1)
            ...
            if verbose:
                ...
                console.print(f"\n📋 Model Information:")
                console.print(
                    f"  Symbol: [blue]{model_info.get('symbol', 'N/A')}[/blue]"
                )
                console.print(
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                pred_response = await client.post(
                    "http://localhost:8000/api/v1/models/predict", json=payload
                )
            ...
            if pred_response.status_code == 200:
                ...
                if result.get("success"):
                    ...
                    console.print(
                        f"  Test Date: [yellow]{result.get('test_date')}[/yellow]"
                    )
                    console.print(
                    ...
                    error_console.print(
                        f"[bold red]Prediction failed:[/bold red] {result.get('message', 'Unknown error')}"
                    )
                    sys.exit(1)
            else:
                error_console.print(
                    f"[bold red]API Error:[/bold red] {pred_response.status_code}"
                )
                try:
                    error_detail = pred_response.json().get(
                        "detail", pred_response.text
                    )
                    error_console.print(f"Details: {error_detail}")
                except:
