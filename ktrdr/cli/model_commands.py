...
def train_model(
    ...
    start_date: str = typer.Option(..., "--start-date", help="Training start date (YYYY-MM-DD)"),
    end_date: str = typer.Option(..., "--end-date", help="Training end date (YYYY-MM-DD)"),
    models_dir: str = typer.Option("models", "--models-dir", help="Directory to save trained models"),
    ...
    try:
        ...
        # Run async operation
        asyncio.run(_train_model_async(
            strategy_file, symbol, timeframe, start_date, end_date,
            models_dir, validation_split, data_mode, dry_run, verbose
        ))
...
def list_models(
    ...
    pattern: Optional[str] = typer.Option(None, "--pattern", "-p", help="Filter models by pattern"),
...
async def _list_models_async(
    ...
    try:
        ...
        if pattern:
            models = [model for model in models if pattern.upper() in model["name"].upper()]
...
def test_model(
    ...
    test_date: Optional[str] = typer.Option(None, "--test-date", help="Specific test date (YYYY-MM-DD)"),
    data_mode: str = typer.Option("local", "--data-mode", help="Data loading mode (local, ib)"),
    ...
    try:
        ...
        # Run async operation
        asyncio.run(_test_model_async(
            model_name, symbol, timeframe, test_date, data_mode, output_file, verbose
        ))
...
async def _test_model_async(
    ...
    try:
        ...
        raise DataError(
            ...
            details={"model": model_name, "symbol": symbol, "timeframe": timeframe, "error": str(e)},
        ) from e
...
def make_prediction(
    ...
    data_mode: str = typer.Option("local", "--data-mode", help="Data loading mode (local, ib)"),
    output_format: str = typer.Option("table", "--format", "-f", help="Output format (table, json)"),
    confidence_threshold: float = typer.Option(0.6, "--confidence", help="Minimum confidence threshold"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable verbose output"),
):
    """
    ...
    try:
        ...
        # Run async operation
        asyncio.run(_make_prediction_async(
            model_name, symbol, timeframe, data_mode, output_format, confidence_threshold, verbose
        ))
...
async def _make_prediction_async(
    ...
    try:
        ...
        # For now, show a placeholder message
        console.print(f"⚠️  [yellow]Model prediction via API not yet implemented[/yellow]")
        ...
        if output_format == "json":
            ...
            console.print(f"Symbol: {symbol} ({timeframe})")
            console.print(f"Confidence threshold: {confidence_threshold:.1%}")
            console.print(f"Predictions: {len(filtered_predictions)}/{len(predictions)}")
            console.print()
            ...
            if filtered_predictions:
                ...
                for pred in filtered_predictions:
                    table.add_row(
                        ...
                        pred["signal"],
                        f"{pred['confidence']:.1%}",
                        f"${pred['price_target']:.2f}" if pred["price_target"] else "N/A",
