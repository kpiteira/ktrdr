...
async def _list_strategies(state: CLIState) -> None:
    ...
    for s in strategies:
        ...
        if td:
            ...
            timeframe = ", ".join(timeframes) if timeframes else s.get("timeframe", "")
        else:
            symbol = s.get("symbol", "")
...
async def _list_models(state: CLIState) -> None:
...
async def _list_checkpoints(state: CLIState) -> None:
