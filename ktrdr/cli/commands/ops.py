...
def ops(
    ...
    async def _list_operations() -> None:
        ...
        if resumable:
            filtered_operations = [op for op in operations if op.get("has_checkpoint")]

        if state.json_mode:
            print(json.dumps(filtered_operations))
            return
