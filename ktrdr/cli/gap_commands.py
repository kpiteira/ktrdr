...
from ktrdr.api.models.gap_analysis import GapAnalysisRequest, GapAnalysisMode, BatchGapAnalysisRequest
...
gap_analysis_app = typer.Typer(name="gap-analysis", help="Gap analysis and management commands")
...
def analyze_gaps(
    ...
    start_date: Optional[str] = typer.Option(None, "--start-date", "-s", help="Start date (YYYY-MM-DD or ISO format)"),
    end_date: Optional[str] = typer.Option(None, "--end-date", "-e", help="End date (YYYY-MM-DD or ISO format)"),
    mode: str = typer.Option("normal", "--mode", "-m", help="Analysis detail level (normal/extended/verbose)"),
    include_expected: bool = typer.Option(False, "--include-expected", help="Include expected gaps in results"),
    output: str = typer.Option("table", "--output", "-o", help="Output format (table/json/summary)")
):
    """
    ...
    try:
        ...
        typer.echo(f"Analyzing gaps for {symbol}_{timeframe} from {start_date} to {end_date}")
...
def analyze_batch_gaps(
    timeframe: str = typer.Argument(..., help="Timeframe (e.g., 1d, 1h)"),
    symbols: List[str] = typer.Argument(..., help="Trading symbols (e.g., AAPL MSFT GOOGL)"),
    start_date: Optional[str] = typer.Option(None, "--start-date", "-s", help="Start date (YYYY-MM-DD or ISO format)"),
    end_date: Optional[str] = typer.Option(None, "--end-date", "-e", help="End date (YYYY-MM-DD or ISO format)"),
    mode: str = typer.Option("normal", "--mode", "-m", help="Analysis detail level (normal/extended/verbose)"),
    include_expected: bool = typer.Option(False, "--include-expected", help="Include expected gaps in results"),
    ...
    try:
        ...
        typer.echo(f"Analyzing gaps for {len(symbols)} symbols ({timeframe}) from {start_date} to {end_date}")
...
gap_service_app = typer.Typer(name="gap-service", help="Gap filler service management commands")
...
def gap_service_status():
    ...
    try:
        ...
        status_data = [
            ...
            ["Fill Unexpected Only", "✅ Yes" if stats['configuration']['fill_unexpected_only'] else "❌ No"],
...
def gap_service_start(
    frequency: Optional[str] = typer.Option(None, "--frequency", help="Override sync frequency (manual/hourly/daily/disabled)")
):
...
def gap_service_scan_now(
    force: bool = typer.Option(False, "--force", help="Force scan even if frequency is disabled")
):
    ...
    try:
        ...
        if 'error' in result:
            ...
            typer.echo("\nScan Results")
            typer.echo("=" * 20)
            typer.echo(tabulate(scan_data, headers=["Metric", "Count"], tablefmt="grid"))
...
def _normalize_date(date_str: str) -> str:
    ...
    try:
        ...
        raise typer.BadParameter(f"Invalid date format: {date_str}. Use YYYY-MM-DD or ISO format.")
...
def _print_gap_summary(result):
    ...
    if result.summary.total_missing > 0:
        ...
        if breakdown_data:
            ...
            typer.echo("=" * 30)
            typer.echo(tabulate(breakdown_data, headers=["Type", "Count"], tablefmt="grid"))
    ...
    if result.gaps and mode in ['extended', 'verbose']:
        ...
        for gap in result.gaps:
            ...
            gap_data.append([
                ...
                gap.day_context,
                start_date,
                gap.note[:50] + "..." if len(gap.note or "") > 50 else (gap.note or "")
            ])
...
def _print_batch_summary(result):
    ...
    if result.overall_summary:
        overall_data = [
            ...
            ["Overall Completeness", f"{result.overall_summary['overall_completeness_pct']:.1f}%"],
            ["Average Completeness", f"{result.overall_summary['average_completeness_pct']:.1f}%"],
        ]
