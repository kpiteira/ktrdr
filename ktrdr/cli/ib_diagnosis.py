...
def detect_ib_issue_from_api_response(response: Dict[str, Any]) -> Tuple[Optional[IBProblemType], Optional[str], Dict[str, Any]]:
    """
    ...
    if any(term in error_message.lower() for term in ["connection", "connect", "unreachable", "refused"]):
        return (
            IBProblemType.UNRECOVERABLE,
    ...
    ib_error_patterns = [
        "IB-", "ib-", "gateway", "tws", "interactive", "brokers"
    ]
    
    if any(pattern in error_code.lower() or pattern in error_message.lower() 
