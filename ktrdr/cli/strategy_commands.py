...
def backtest_strategy(
    ...
    # Validate inputs
    try:
        InputValidator.validate_string(symbol, min_length=1, max_length=10, pattern=r"^[A-Za-z0-9\-\.]+$")
    except ValidationError:
        console.print(f"[red]❌ Error: Invalid symbol format: {symbol}[/red]")
        raise typer.Exit(1)

    try:
        InputValidator.validate_string(timeframe, min_length=1, max_length=5, pattern=r"^[0-9]+[dhm]$")
    except ValidationError:
