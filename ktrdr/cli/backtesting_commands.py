...
def run_backtest(
    ...
    model: Optional[str] = typer.Option(
        None, "--model", "-m", help="Path to specific trained model"
    ),
    capital: float = typer.Option(
        100000, "--capital", "-c", help="Initial capital for backtest"
    ),
    commission: float = typer.Option(
        0.001, "--commission", help="Commission rate as decimal (0.001 = 0.1%)"
    ),
    slippage: float = typer.Option(
        0.0005, "--slippage", help="Slippage rate as decimal (0.0005 = 0.05%)"
    ),
    ...
    try:
        ...
        if verbose and not quiet:
            ...
            console.print(
                f"  Model: [blue]{model if model else 'Latest available'}[/blue]"
            )
        ...
        if output:
            ...
            results_dict["equity_curve"] = results.equity_curve.reset_index().to_dict(
                "records"
            )
        ...
        if not quiet:
            ...
            console.print(
                f"💰 Total Return: [green]${metrics.total_return:,.2f} ({metrics.total_return_pct*100:.2f}%)[/green]"
            )
            console.print(
                f"📈 Sharpe Ratio: [yellow]{metrics.sharpe_ratio:.3f}[/yellow]"
            )
            console.print(
                f"📉 Max Drawdown: [red]${metrics.max_drawdown:,.2f} ({metrics.max_drawdown_pct*100:.2f}%)[/red]"
            )
            console.print(
                f"🎯 Win Rate: [blue]{metrics.win_rate*100:.1f}%[/blue] ({metrics.winning_trades}/{metrics.total_trades})"
            )
