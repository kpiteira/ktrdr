...
def test_model_signals(
    ...
    try:
        ...
        if data is None or len(data) < samples:
            console.print(f"[red]❌ Error: Insufficient data. Need at least {samples} bars, got {len(data) if data is not None else 0}[/red]")
            raise typer.Exit(1)
        ...
        console.print(f"✅ Loaded {len(data)} total bars, testing on last {samples} bars")
        ...
        for i in range(len(test_data)):
            ...
            historical_data = recent_data.iloc[:len(recent_data) - len(test_data) + i + 1]
            ...
            try:
                ...
                console.print(f"[red]❌ Error processing {current_bar.name}: {str(e)}[/red]")
        ...
        if valid_decisions:
            ...
            console.print(f"   HOLD signals: {hold_count} ({hold_count/len(valid_decisions)*100:.1f}%)")
            console.print(f"   BUY signals: {buy_count} ({buy_count/len(valid_decisions)*100:.1f}%)")
            console.print(f"   SELL signals: {sell_count} ({sell_count/len(valid_decisions)*100:.1f}%)")
