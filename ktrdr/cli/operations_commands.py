...
def list_operations(
    status: Optional[str] = typer.Option(None, "--status", "-s", help="Filter by status (running, completed, failed, cancelled)"),
    operation_type: Optional[str] = typer.Option(None, "--type", "-t", help="Filter by operation type (data_load, training, etc.)"),
    limit: int = typer.Option(50, "--limit", "-l", help="Maximum number of operations to show"),
    active_only: bool = typer.Option(False, "--active", "-a", help="Show only active (running/pending) operations"),
    ...
    try:
        # Run async operation
        asyncio.run(_list_operations_async(status, operation_type, limit, active_only, verbose))
...
async def _list_operations_async(
    ...
    try:
        ...
        if verbose:
            console.print(f"🔍 Listing operations (status: {status}, type: {operation_type}, active_only: {active_only})")
        ...
        for op in operations:
            ...
            # Basic row
            row = [
                op["operation_id"][-20:] + "..." if len(op["operation_id"]) > 23 else op["operation_id"],  # Truncate long IDs
                op["operation_type"],
                status_display,
                progress,
                op.get("symbol", "N/A"),
...
def get_operation_status(
    ...
    try:
        # Input validation
        operation_id = InputValidator.validate_string(operation_id, min_length=1, max_length=100)
...
async def _get_operation_status_async(operation_id: str, verbose: bool):
    ...
    try:
        ...
        if progress:
            ...
            progress_table.add_row("Percentage", f"{progress.get('percentage', 0):.1f}%")
            progress_table.add_row("Current Step", progress.get("current_step", "N/A"))
            progress_table.add_row("Steps", f"{progress.get('steps_completed', 0)}/{progress.get('steps_total', 0)}")
            progress_table.add_row("Items", f"{progress.get('items_processed', 0)}/{progress.get('items_total', 'N/A')}")
...
def cancel_operation(
    ...
    reason: Optional[str] = typer.Option(None, "--reason", "-r", help="Reason for cancellation"),
    force: bool = typer.Option(False, "--force", "-f", help="Force cancellation even if operation is in critical section"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show detailed information"),
):
    """
    ...
    try:
        ...
        try:
            ...
            if "404" in str(e) or "not found" in str(e).lower():
                ...
                error_console.print("The operation may already be completed, failed, or cancelled.")
                sys.exit(1)
            else:
                raise
        ...
        if result.get("success"):
            console.print(f"✅ [green]Successfully cancelled operation: {operation_id}[/green]")
