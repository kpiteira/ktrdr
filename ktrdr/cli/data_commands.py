...
def show_data(
    ...
    trading_hours_only: bool = typer.Option(False, "--trading-hours", help="Show only trading hours data"),
    include_extended: bool = typer.Option(False, "--include-extended", help="Include extended hours when filtering"),
    ...
    try:
        ...
        # Run async operation
        asyncio.run(_show_data_async(
            symbol, timeframe, rows, start_date, end_date,
            trading_hours_only, include_extended, output_format, verbose
        ))
    ...
    try:
        ...
        if not data or not data.get("dates"):
            ...
            if start_date or end_date:
                console.print("💡 Try adjusting the date range or loading data first with 'ktrdr data load'")
            else:
...
def load_data(
    ...
    start_date: Optional[str] = typer.Option(None, "--start", help="Start date (YYYY-MM-DD)"),
    end_date: Optional[str] = typer.Option(None, "--end", help="End date (YYYY-MM-DD)"),
    trading_hours_only: bool = typer.Option(False, "--trading-hours", help="Filter to trading hours only"),
    include_extended: bool = typer.Option(False, "--include-extended", help="Include extended hours when filtering"),
    ...
    try:
        ...
        asyncio.run(_load_data_async(
            ...
            trading_hours_only, include_extended, show_progress,
            output_format, verbose, quiet
        ))
...
async def _load_data_async(
    ...
    try:
        ...
        if not quiet:
            ...
            console.print(f"📋 Mode: {mode} | Trading hours filter: {trading_hours_only}")
            if start_date or end_date:
