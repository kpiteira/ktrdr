...
class ProgressDisplayManager:
    ...
    def progress_context(
        ...
        with self._active_progress:
            self._active_task = self._active_progress.add_task(
                title, 
                total=total_items
            )
    ...
    def print_summary(self):
        ...
        table.add_row("Success Rate", f"{((self.stats.completed_items / self.stats.total_items) * 100):.1f}%")
...
class IbProgressTracker:
    ...
    def print_connection_status(self, connection_info: Dict[str, Any]):
        ...
        if "metrics" in connection_info:
            ...
            table.add_row("Failed Connections", str(metrics.get("failed_connections", 0)))
            
            if metrics.get("last_connect_time"):
                last_connect = datetime.fromtimestamp(metrics["last_connect_time"])
                table.add_row("Last Connected", last_connect.strftime("%Y-%m-%d %H:%M:%S"))
    ...
    def print_metrics_summary(self, metrics: Dict[str, Any]):
        ...
        """
        table = Table(title="IB Performance Metrics", show_header=True, header_style="bold blue")
        table.add_column("Metric", style="cyan")
        table.add_column("Value", style="green")
