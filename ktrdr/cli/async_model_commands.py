...
def _is_v3_strategy(strategy_path: Path) -> bool:
    """
...
def _display_v3_dry_run(strategy_path: Path) -> None:
    """
...
def train_model_async(
    ...
    try:
        ...
        if dry_run and _is_v3_strategy(strategy_path):
            _display_v3_dry_run(strategy_path)
            return

        # Load strategy configuration to extract symbols/timeframes if not provided
        try:
            if _is_v3_strategy(strategy_path):
                ...
                v2_config, is_v2 = strategy_loader.load_strategy_config(
                    str(strategy_path)
                )
