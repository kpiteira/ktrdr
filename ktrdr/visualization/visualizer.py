...
class Visualizer:
    ...
    def create_chart(self, 
        ...
        if chart_type == 'candlestick':
            ...
            chart_data = DataAdapter.transform_histogram(data, 
                                                       time_column='date', 
            ...
            chart_data = DataAdapter.transform_line(data, 
                                                  time_column='date', 
                                                  value_column=value_column)
    ...
    def add_indicator_overlay(self, 
                            chart: Dict[str, Any], 
                            data: pd.DataFrame,
                            column: str,
                            color: str = "#2962FF", 
    ...
    def configure_range_slider(self,
        ...
        if not show:
            ...
            chart["configs"] = [c for c in chart["configs"] 
                               if not c.get("is_range_slider", False)]
            return chart
        
        # Check if range slider already exists
        existing_slider = next((c for c in chart["configs"] 
                               if c.get("is_range_slider", False)), None)
        if existing_slider:
    ...
    def save(self, 
        ...
        has_range_slider = any(c.get("is_range_slider", False) for c in chart["configs"])
