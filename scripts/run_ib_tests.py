...
class IbTestRunner:
    ...
    async def _check_ib_availability(self):
        ...
        try:
            ...
            logger.error("Please ensure IB Gateway/TWS is running before running integration tests")
            raise
    ...
    async def _run_integration_tests(self) -> bool:
        ...
        test_files = [
            "tests/integration/test_ib_unified_integration.py"
        ]
        
        return await self._run_pytest_tests(
            "integration", 
            test_files,
            extra_args=["--run-integration"]
        )
    ...
    async def _run_pytest_tests(self, 
                               test_category: str,
                               test_files: List[str],
        ...
        if self.output_file:
            output_path = Path(self.output_file).parent / f"{test_category}_{Path(self.output_file).name}"
    ...
    def _generate_report(self):
        ...
        passed_tests = sum(1 for result in self.test_results["results"].values() if result.get("success"))
        failed_tests = total_tests - passed_tests
        total_duration = sum(result.get("duration", 0) for result in self.test_results["results"].values())
