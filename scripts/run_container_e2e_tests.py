...
class ContainerE2ETestRunner:
    ...
    def run_pytest_command(self, test_file: str, markers: List[str], extra_args: List[str] = None) -> Tuple[bool, str]:
    ...
    def generate_test_report(self) -> str:
        ...
        if "performance_tests" in self.results and "api_endpoints" in self.results["performance_tests"]:
            for endpoint, result in self.results["performance_tests"]["api_endpoints"].items():
                if "response_time" in result:
                    status = "✅" if result["passed"] else "❌"
                    report.append(f"    {endpoint}: {status} {result['response_time']:.2f}s")
        report.append("")
        ...
        report.append(f"  {'🎉 ALL TESTS PASSED' if overall_success else '💥 SOME TESTS FAILED'}")
        report.append("")
        report.append("=" * 80)
