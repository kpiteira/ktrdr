...
def validate_triangular_membership_function():
    ...
    for point in test_points:
        ...
        print(f"{point:5d} | {std_val:7.3f} | {left_val:13.3f} | {right_val:14.3f} | {single_val:9.3f}")
...
def validate_fuzzy_engine():
    ...
    for rsi in rsi_values:
        ...
        print(f"{rsi:8} | {result['rsi_low']:13.3f} | {result['rsi_medium']:17.3f} | {result['rsi_high']:14.3f}")
    ...
    for macd in macd_values:
        ...
        print(f"{macd:9.1f} | {result['macd_negative']:18.3f} | {result['macd_neutral']:18.3f} | {result['macd_positive']:19.3f}")
    ...
    print(f"MACD membership function visualization saved to output/macd_membership_functions.png")
...
def validate_edge_cases():
    ...
    config_dict = {
        "rsi": {
            ...
            "right_shoulder": {"type": "triangular", "parameters": [50.0, 100.0, 100.0]}
        }
    }
    ...
    print("Value   | Standard Low | Standard High | Singleton | Left Shoulder | Right Shoulder")
    print("----------------------------------------------------------------------------")
    ...
    for val in extreme_values:
        ...
        print(f"{val:7} | {std_low:11.3f} | {std_high:12.3f} | {singleton:9.3f} | {left_shoulder:13.3f} | {right_shoulder:14.3f}")
    
    # Test with NaN values
    result_nan = engine.fuzzify("rsi", np.nan)
    print("\nNaN input results in NaN membership degrees:", all(np.isnan(val) for val in result_nan.values()))
    ...
    # Plot membership functions
    plt.plot(x, membership["rsi_standard_low"], label="Standard Low", linestyle="-", color="blue")
    plt.plot(x, membership["rsi_standard_high"], label="Standard High", linestyle="-", color="green")
    plt.plot(x, membership["rsi_singleton"], label="Singleton", linestyle="-.", color="red")
    plt.plot(x, membership["rsi_left_shoulder"], label="Left Shoulder", linestyle="--", color="orange")
    plt.plot(x, membership["rsi_right_shoulder"], label="Right Shoulder", linestyle="--", color="purple")
