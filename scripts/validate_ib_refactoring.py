...
async def validate_component_imports():
    ...
    try:
        ...
        try:
            symbol_validator = IbSymbolValidatorUnified(component_name="validation_test")
...
async def validate_metrics_collector():
    ...
    try:
        ...
        if component_metrics.total_operations != 1:
            logger.error(f"  ❌ Expected 1 operation, got {component_metrics.total_operations}")
            return False
        
        logger.info(f"  ✅ Component metrics: {component_metrics.total_operations} operations")
        ...
        logger.info(f"  ✅ Global metrics: {global_metrics['total_operations']} total operations")
    ...
    try:
        ...
        try:
            ...
            async with await acquire_ib_connection(
                ...
                logger.info(f"  ✅ Successfully acquired connection: {connection.client_id}")
                
        except Exception as e:
            # This is expected if no IB Gateway is running
            logger.info(f"  ⚠️  Connection acquisition failed (expected without IB Gateway): {e}")
...
async def validate_unified_components():
    ...
    try:
        ...
        symbol_validator = IbSymbolValidatorUnified(component_name="validation_test_validator")
...
async def main():
    ...
    if passed == total:
        ...
        logger.error(f"💥 {total - passed} validations failed - please check the issues above")
        return False
